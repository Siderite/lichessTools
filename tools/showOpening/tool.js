	...
	class ShowOpeningTool extends LiChessTools.Tools.ToolBase {
		...
		miniGameOpening = async (el) => {
			...
			for (const el of elems) {
				...
				if (!openingEl.length) {
					openingEl = $('<span class="lichessTools-opening"/>').appendTo(
						container,
					);
				}
				openingEl.text(opening).attr("title", opening);
				fen = "";
				gameId = "";
			}
		};
		miniGameOpeningDebounced = this.lichessTools.debounce(
		...
		withOpening = async (gameId, el, ply, fen, isMini) => {
			...
			if (data) {
				const now = Date.now();
				if (el.maxPly > 14 || now - data.time < 2000)
		...
		refreshOpening = async (ply) => {
			...
			const fen =
				lichess.analysis?.node?.fen || lichess.analysis?.data?.game?.fen;
			const metaSection = $.cached(
				"div.game__meta section, div.analyse__wiki.empty, div.chat__members:not(.none), .analyse__underboard .copyables, main#board-editor .copyables",
				10000,
			);
			const result = await this.withOpening(
				gameId,
				$.cached("main.round, main.analyse, main#board-editor", 10000)[0],
				ply,
				fen,
				false,
			);
		...
		refreshOpeningDebounced = this.lichessTools.debounce(
			this.refreshOpening,
			500,
		);
		...
		async start() {
			...
			if (this.options.showInBoard) {
				...
				const intervalTime = $("main").is("#board-editor") ? 1000 : 3500;
