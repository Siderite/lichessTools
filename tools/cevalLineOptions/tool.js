	...
	class CevalLineOptionsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.cevalLineOptions":
					"Op\u0163iuni linii \uee00n evaluarea computerului",
		...
		handlePvs = () => {
			...
			const demotes = arr
				.map((entry) => entry[1])
				.filter((val) => val.count <= 1 && val.cls);
			arr.forEach((entry) => {
				const key = entry[0];
				const val = entry[1];
			...
			$("div.pv_box span.pv-san").each((i, e) => {
				...
				const cls = val?.count > 1 ? ("pv-san " + val.cls).trim() : "pv-san";
