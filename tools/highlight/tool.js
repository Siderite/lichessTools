(() => {
	class HighlightTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				"options.highlight":
					"Eviden\u0163iaz\u0103 mut\u0103ri \u00een analiz\u0103",
				"highlight.lastMove": "Ultima mutare din fiecare varia\u0163iune",
				"highlight.notCommented": "Ultime mut\u0103ri necomentate",
				"highlight.transposition":
					"Transpozi\u0163iile la mutarea curent\u0103",
				"highlight.mainLine":
					"Eviden\u0163iaz\u0103 tabla c\u00e2nd nu pe linia principal\u0103",
				"highlight.mainLinePieces":
					"Eviden\u0163iaz\u0103 piese c\u00e2nd nu pe linia principal\u0103",
				"highlight.variationDepth":
					"Eviden\u0163iaz\u0103 ad\u00e2ncimea varia\u0163iunii",
		...
		highlightLastMoves = () => {
			...
			$("div.analyse__moves move.lichessTools-lastInLine")
				...
				.removeClass("lichessTools-lastInLine");
		...
		highlightChecks = () => {
			...
			$("div.analyse__moves move.lichessTools-inCheck")
				.filter((i, e) => !toHighlight.includes(e))
				.removeClass("lichessTools-inCheck");
			for (const elem of toHighlight) {
		...
		highlightUncommented = () => {
			...
			$("div.analyse__moves move.lichessTools-uncommented")
				.filter((i, e) => !toHighlight.includes(e))
				.removeClass("lichessTools-uncommented");
			for (const elem of toHighlight) {
		...
		highlightTranspositions = () => {
			...
			if (this.options.transposition) {
				...
				if (parent.transpositionBehavior?.excludeSameLine) {
					transpositions = transpositions?.filter(
						(n) =>
							n === currentNode ||
							(n.path &&
								!n.path.startsWith(currentNode.path) &&
								!currentNode.path.startsWith(n.path)),
					);
				}
			...
			$("div.analyse__moves move.lichessTools-transposition")
				.filter((i, e) => !toHighlight.includes(e))
				.removeClass("lichessTools-transposition");
			for (const elem of toHighlight) {
		...
		highlightMainLinePieces = () => {
			...
			if (onMainline) {
				$(
					"div.main-board cg-board piece.lichessTools-notOnMainline",
				).removeClass("lichessTools-notOnMainline");
			} else {
				const board = parent.getBoardFromFen(analysis.node.fen);
				const mainNode = analysis.nodeList.findLast(
					(n, i) => n == analysis.mainline[i],
				);
		...
		highlightVariationDepth = () => {
			...
			while (nodes.length) {
				...
				if (node.path) {
					dict[node.path] =
						"vd" +
						Math.min(7, node.variationDepth + 1) +
						" vdm" +
						((node.variationDepth % 7) + 1);
				}
				let depth = 0;
		...
		async start() {
			...
			$.cached("body").toggleClass(
				"lichessTools-variationDepth",
				this.options.variationDepth,
			);
