(() => {
	class PinOnMainPageTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				"options.General": "General",
				"options.pinOnMainPage":
					"Pune transmisiuni \u015fi studii pe pagina principal\u0103",
		...
		pinCurrentStudy = () => {
			...
			lichess.storage.set(
				"LichessTools.pinnedStudies",
				parent.global.JSON.stringify(this.options.pinned),
			);
			this.addPin();
		};
		...
		addToHomepage = () => {
			...
			this.options.pinned.forEach((p) => {
				...
				const innerElem = $(
					'<span class="content"><span class="name"></span><span class="lichessTools-unpin"></span></span>',
				);
				innerElem
					.find("span.lichessTools-unpin")
					.attr("data-icon", "\uE071")
		...
		async start() {
			...
			this.options = {
				enabled: value,
				pinned: parent.jsonParse(
					(_) => lichess.storage.get("LichessTools.pinnedStudies"),
					[],
				),
			};
			this.addPin();
