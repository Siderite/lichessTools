		...
		async start() {
			...
			analysis.wiki = parent.wrapFunction(analysis.wiki, {
				...
				before: ($this, nodes) => {
					...
					const plyPrefix = (node) =>
						`${parent.global.Math.floor((node.ply + 1) / 2)}${node.ply % 2 === 1 ? "._" : "..."}`;
					const pathParts = nodes
						.slice(1)
						.map((n) => `${plyPrefix(n)}${n.san}`);
					...
					if (
						!newTitles?.length ||
						newTitles.find((t) => t.replaceAll(" ", "_") == title)
					)
						return;
					const originalFunction =
						analysis.wiki.__originalFunction?.bind($this);
