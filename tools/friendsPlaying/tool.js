	...
	class FriendsPlayingTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"options.friendsPlaying":
					"Sound and voice alert when friends start playing",
				audioNotAllowedTitle:
					"LiChess Tools - Audio allowed only after user action",
			},
			"ro-RO": {
				...
				"options.friendsPlaying":
					"Alert\u0103 sonor\u0103 \u015Fi vocal\u0103 c\u00E2nd joac\u0103 prieteni",
				audioNotAllowedTitle:
					"LiChess Tools - Sunete permise doar dup\u0103 ac\u0163iune utilizator",
			},
		};
		...
		showAudioNotAllowed = () => {
			...
			if (isAudioAllowed) {
				...
				if (!this.audioCheckTimeout) {
					this.audioCheckTimeout = parent.global.setInterval(
		...
		playFriendSound = async (username) => {
			this.lichessTools.debug &&
				this.lichessTools.global.console.debug(username + " playing");
			const now = Date.now();
			const isMuted = (
				this.lichessTools.currentOptions.getValue("mutedPlayers") || []
			...
			if (!silent) {
				let friendSoundTime = +this.lichessTools.jsonParse((_) =>
					this.lichessTools.lichess.storage.get("LiChessTools.friendSound"),
				)?.value?.time;
				if (friendSoundTime) {
			...
			if (!silent && !this.lichessTools.net.slowMode) {
				try {
					const arr = await this.lichessTools.api.user.getUserStatus(
						[username],
						{ withGameMetas: true },
					);
					...
					if (e.toString().includes("Failed to fetch")) {
						...
						this.lichessTools.global.console.warn(
							"playFriendSound game fetch error",
							e,
						);
					}
				}
			...
			if (hasInfo) {
				if (
					!silent &&
					gameType != "unknown" &&
					!this.lichessTools.isOptionSet(
						this.lichessTools.currentOptions.getValue("friendsPlaying"),
						gameType,
					)
				) {
			...
			if (!silent) {
				...
				this.lichessTools.lichess.storage.fire("LiChessTools.friendSound", {
					time: now,
					id: id,
				});
				await this.lichessTools.timeout(200);
				const storedId = this.lichessTools.jsonParse((_) =>
					this.lichessTools.lichess.storage.get("LiChessTools.friendSound"),
				)?.value?.id;
			...
			this.beep?.play();
			let translation =
				this.lichessTools.translator.plural(
					"playing",
					1,
					username?.replace(/[_\-]/g, " "),
				) +
				", " +
				this.lichessTools.translator.noarg("gameType-" + gameType);
			if (!isStandard) {
				translation += " " + variant;
			}
			this.lichessTools.speak(translation, {
				translated: this.lichessTools.isTranslated,
			});
		...
		async start() {
			...
			if (parent.currentOptions.getValue("mutedPlayers")?.length) {
				this.logOption(
					" ... muted",
					parent.currentOptions.getValue("mutedPlayers").join(","),
				);
			}
			if (!parent.getUserId()) {
			...
			if (value !== false && value?.toString()?.replace(/,\s*standard/i, "")) {
				this.beep = await lichess.sound.load(
					"friendPlaying",
					lichess.sound.url("piano/GenericNotify.mp3"),
				);
