	...
	class ExplorerGambitsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.explorerGambits":
					"Arat\u0103 mut\u0103ri \u00een Explorator care duc la gambituri",
			...
			$("tr[data-uci]", container).each((i, e) => {
				if ($("td:has(div.bar)", e).addClass("lichessTools-bar").length) {
					if (!$("td.lichessTools-explorerGambits", e).length) {
						$("<td>").addClass("lichessTools-explorerGambits").appendTo(e);
					}
				}
				...
				const explorerItem = (analysis.explorer.current()?.moves || []).find(
					(i) => i.uci == uci,
				);
				let text = "";
				let title = undefined;
				...
				$("td.lichessTools-explorerGambits", e).text(text).attr("title", title);
			});
		...
		async start() {
			...
			$(
				"th.lichessTools-explorerGambits,td.lichessTools-explorerGambits",
			).remove();
			explorer.setNode = parent.unwrapFunction(
				explorer.setNode,
				"explorerGambits",
