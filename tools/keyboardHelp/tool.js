	...
	class KeyboardHelpTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.keyboardHelp":
					"Sec\u0163iune LiChess Tools \u00een dialogul ajutor pentru taste",
				lichessTools: "LiChess Tools",
				...
				nextOpponentMistake:
					"Urm\u0103toarea gre\u015Feal\u0103 a adversarului",
				nextOpponentInaccuracy: "Urm\u0103toarea inexactitate a adversarului",
				nextGood:
					"Urm\u0103toarea bun\u0103/briliant\u0103/interesant\u0103 a adversarului",
		...
		processHelp = async () => {
			...
			const row = (keys, text) => {
				const row = $(
					'<tr><td class="keys"></td><td class="desc"></td></tr>',
				).appendTo(table);
			...
			if (parent.currentOptions.getValue("keyShortcuts")) {
				...
				if (
					parent.currentOptions.getValue("chapterNavigation") &&
					$('div.lichessTools-chapterControls button[data-act="random"]').length
				) {
			...
			if (
				parent.currentOptions.getValue("obsIntegration") &&
				$("span.lichessTools-obsSetup").length
			) {
