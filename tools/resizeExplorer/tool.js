(() => {
	class ResizeExplorerTool extends LiChessTools.Tools.ToolBase {
		...
		refreshHeight = () => {
			...
			const h = this.isMobile ? this.height?.mobile : this.height?.desktop;
			if (h === undefined) {
		...
		dragDivider = (ev) => {
			...
			const hh =
				$(".ceval", tools).height() + ($(".pv_box", tools).height() || 0);
			const t = tools.offset().top;
			const et = explorerBox.offset().top;
			const h = this.isMobile ? ev.pageY - et : t + th - ev.pageY;
		...
		addDividerDirect = () => {
			...
			divider = $('<div class="lichessTools-resizeExplorer">')
				...
				.on("dragstart", (ev) => {
					...
					$("body").on("dragenter dragover", this.controlCursor);
				})
				.on("dragend", (ev) => {
					$(ev.currentTarget).removeClass("dragging");
					$("body").off("dragenter dragover", this.controlCursor);
				})
		...
		async start() {
			...
			analysis.explorer.setNode = parent.unwrapFunction(
				analysis.explorer.setNode,
				"resizeExplorer",
			);
			if (!value) {
				const explorerBox = $("main.analyse .analyse__tools .explorer-box");
			...
			analysis.explorer.setNode = parent.wrapFunction(
				analysis.explorer.setNode,
				{
					id: "resizeExplorer",
