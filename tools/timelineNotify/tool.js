	...
	class TimelineNotifyTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				timelineNotifyTitle:
					"LiChess Tools - c\u0103tre Activitate recent\u0103",
		...
		async start() {
			...
			switch (value) {
				case true:
					this.types = this.preferences[0].defaultValue.split(",");
					break;
				case false:
					this.types = [];
					break;
			...
			if (/^\/timeline/i.test(parent.global.location.pathname))
				this.setAllRead();
			const notification = {
				getEntries: async () => {
					...
					if (!newEntries.find((e) => e.date > justNotified)) return [];
					const entry = {
						id: "timelineNotify",
						isNew: true,
						icon: "\uE058",
						...
						content: $("<div>")
							...
							.append(
								$("<span>").text(
									trans.pluralSame("timelineNotification", newEntries.length),
								),
							)
							.html(),
