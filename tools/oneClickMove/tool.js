(() => {
	class OneClickMoveTool extends LiChessTools.Tools.ToolBase {
		...
		getDests = async (board, fen, variant) => {
			...
			let destMan =
				analysis?.chessground.state?.movable?.dests || this._cache.get(key);
			if (!destMan) {
				lichess.socket.send("anaDests", {
					variant: variant,
					fen: fen,
					path: key,
				});
		...
		boardClick = async (ev) => {
			...
			const analysis = lichess.analysis;
			if (
				!(this.options.analysis && analysis) &&
				!(this.options.play && !analysis)
			)
				return; //TODO better play detection
			if (!ev.x && !ev.y) return;
			...
			const orientation = board.closest(".cg-wrap").is(".orientation-black")
				? "black"
				: "white";
			const fen = parent.getPositionFromBoard(
				board.closest("cg-container"),
				true,
			);
			...
			const sources = $("piece." + turn, board).filter((i, e) => {
				const sq = e.cgKey;
				if (!sq) return false;
				if (sq == square) {
					pieceExists = true;
						...
						.map((c) => {
							if (c.san?.startsWith("O-O")) {
								switch (c.uci?.slice(-2)) {
									case "h1":
										return c.uci.slice(0, 2) + "g1";
									case "a1":
										return c.uci.slice(0, 2) + "c1";
									case "h8":
										return c.uci.slice(0, 2) + "g8";
									case "a8":
										return c.uci.slice(0, 2) + "c8";
								}
							}
							return c.uci;
						})
		...
		getCoords = (square, board, orientation) => {
			...
			return {
				x: offset.left - win.scrollX + coords.x * q + q / 2,
				y: offset.top - win.scrollY + coords.y * q + q / 2,
			};
		};
		...
		unpackDests = (lines) => {
			...
			const uciChar = (ch) => {
				const x = uciValue(ch);
				return (
					String.fromCharCode(97 + (x % 8)) + String.fromCharCode(49 + x / 8)
				);
			};
		...
		handleBoard = () => {
			...
			if (
				...
				!parent.isWrappedFunction(lichess.socket.handle, "oneClickMove")
			) {
		...
		async start() {
			...
			const board = $("div.main-board cg-board")[0];
			if (board) {
				board.removeEventListener("mousedown", this.boardClick, {
					capture: true,
				});
			...
			if (
				...
				($("main.round,main.puzzle").length && this.options.play)
			) {
