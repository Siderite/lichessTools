	...
	class StockfishTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				"options.general": "General",
				"options.stockfish": "Stockfish",
				"options.stockfish-threads":
					"Thread-uri pentru motorul de analiz\u0103 LiChess Tools",
				"options.stockfish-hash":
					"Hash pentru motorul de analiz\u0103 LiChess Tools (MB)",
	...
	class Stockfish {
		...
		async load() {
			try {
				if (!this._module) {
					this.parent.debug &&
						this.parent.global.console.debug("SF", "loading module...");
					...
					const assetUrl =
						engine && engine.assets?.js
							? engine.assets.root + "/" + engine.assets.js
							: "npm/lila-stockfish-web/" +
								(this._use1670 ? "sf161-70.js" : "sf-nnue-40.js");
					this.parent.global.console.debug(
						"SF",
						"loading engine " +
							engineId +
							(engine ? " (" + engine.name + ")" : "") +
				...
				if (!this._stockfish) {
					this.parent.debug &&
						this.parent.global.console.debug("SF", "loading Stockfish...");
					this._stockfish =
						(await this._module.default) ||
						this.parent.global.exports.Stockfish;
					if (!this._stockfish) {
				...
				if (!this._instance) {
					this.parent.debug &&
						this.parent.global.console.debug("SF", "creating instance...");
					...
					if (this._initialThreads)
						this.setOption("Threads", this._initialThreads);
				...
				this.parent.announce(
					this.parent.translator.noarg("couldNotLoadStockfish"),
				);
		...
		postMessage(message) {
			...
			this.parent.debug &&
				this.parent.global.console.debug("Post SF:", message);
		...
		start() {
				...
				"go" +
					(this._depth
						...
						: this._time
							? " movetime " + this._time
							: " infinite") +
					(this._searchMoves?.length
						? " searchmoves " + this._searchMoves.join(" ")
						: ""),
			);
			...
			this.parent.debug &&
				this.parent.global.console.debug("SF", "Engine started");
		}
		...
		listen(data) {
			...
			if (/^(info|bestmove)/.test(data)) {
				...
				for (const split of splits.slice(1)) {
					if (
						!isString &&
						/^(depth|seldepth|time|nodes|pv|multipv|score|cp|wdl|mate|lowerbound|upperbound|currmove|currmovenumber|hashfull|nps|tbhits|sbhits|cpuload|string|refutation|currline|ponder)$/.test(
							split,
						)
					) {
						arr = [];
