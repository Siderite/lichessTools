	...
	class MoveListOptionsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				bookmarkSplitConfirmationText:
					"Sure you want to split the chapter on this bookmark?",
				bookmarkSplitConfirmationDeleteText:
					"Sure you want to split the chapter on this bookmark?\r\nTHIS WILL DELETE FROM THIS CHAPTER THE MOVES THAT FOLLOW",
				bookmarkSplitText: "Split chapter here",
				bookmarkSplitTitle:
					"LiChess Tools - create a new chapter with following moves from here\r\nPress Shift to also delete them from here",
				chapterLink: "Continue here: %s",
				analysisPopupButtonTitle:
					"LiChess Tools - move list in another window (use SYNC button)",
			...
			"ro-RO": {
				...
				"moveListOptions.fullWidthAnalysis":
					"List\u0103 mut\u0103ri l\u0103rgit\u0103",
				"moveListOptions.hideLeftSide": "Ascunde partea st\u00e2ng\u0103",
				"moveListOptions.analysisPopup":
					"Deschide \u00een alt\u0103 fereastr\u0103",
				...
				bookmarkSplitConfirmationText:
					"Sigur vrei s\u0103 tai un nou capitol de la acest bookmark?",
				bookmarkSplitConfirmationDeleteText:
					"Sigur vrei s\u0103 tai un nou capitol de la acest bookmark?\r\nASTA VA \u015ETERGE MUT\u0102RILE URM\u0102TOARE DIN ACEST CAPITOL",
				bookmarkSplitText: "Taie un nou capitol de aici",
				bookmarkSplitTitle:
					"LiChess Tools - creaz\u0103 un nou capitol din mut\u0103rile urm\u0103toare\r\nApas\u0103 Shift ca s\u0103 la \u015Ftergi de aici",
				chapterLink: "Continu\u0103 aici: %s",
				analysisPopupButtonTitle:
					"LiChess Tools - list\u0103 mut\u0103ri \u00een alt\u0103 fereastr\u0103 (folose\u015Fte butonul SYNC)",
				bookmarkLabelForInteractive: "Bookmark: ",
			},
		};
		...
		setBookmark = (elem, node, bookmark) => {
			...
			if (bookmark) {
				...
				if (!bookmarkElem.length) {
					bookmarkElem = $(
						"<bookmark><button></button><label></label></bookmark>",
					).prependTo(elem);
					$("button", bookmarkElem)
				...
				$("button", bookmarkElem).toggleClass(
					"lichessTools-noChildren",
		...
		setBookmarks = () => {
			...
			const $ = parent.$;
			const r = /bkm:([^\s]+)\s*/s;
			const thereAreBookmarks =
				!!parent.global.document.querySelector("bookmark");
		...
		getCollapsed = (label) => {
			...
			if (!this.bookmarks) {
				this.bookmarks = new Map(
					parent.storage.get("LichessTools.bookmarks") || [],
				);
			}
		...
		addCommentBookmarks = () => {
			...
			const commentNodes = this.getCommentNodes(
				$("div.analyse__moves comment, div.gamebook .comment .content"),
			);
			for (const node of commentNodes) {
				...
				if (comment.length) {
					...
					const isEmpty = !Array.from(comment[0].childNodes || [])
						...
						.find((n) => !!n.nodeValue?.trim());
					comment.toggleClass("lichessTools-empty", isEmpty);
				} else if (divComment.length) {
					node.textContent = text.replace(
						r,
						trans.noarg("bookmarkLabelForInteractive") +
							this.fromBookmarkName(m[1]),
					);
				}
			}
		...
		debouncedAddCommentBookmarks = this.lichessTools.debounce(
		...
		analysisControls = () => {
			...
			if (!analysis) return;
			$(".tview2").toggleClass(
				"lichessTools-indentedVariations",
				this.options.indentedVariations,
			);
      ...
      <label for="abset-indentedVariations">$trans(moveListOptions.indentedVariations)</label>
				...
				$("#abset-indentedVariations").on("change", async () => {
					this.options.indentedVariations = $("#abset-indentedVariations").is(
						":checked",
					);
      ...
      <label for="abset-fullWidthAnalysis">$trans(moveListOptions.fullWidthAnalysis)</label>
				...
				$("#abset-fullWidthAnalysis").on("change", async () => {
					this.options.fullWidthAnalysis = $("#abset-fullWidthAnalysis").is(
						":checked",
					);
					const options = parent.currentOptions;
					options.moveListOptions = this.options.getString();
			...
			$("#abset-fullWidthAnalysis").prop(
				"checked",
				this.options.fullWidthAnalysis,
			);
      ...
      <label for="abset-hideLeftSide">$trans(moveListOptions.hideLeftSide)</label>
				...
				$("#abset-hideLeftSide").on("change", async () => {
			...
			$("#abset-hideLeftSide").prop("checked", this.options.hideLeftSide);
		};
		...
		hashChange = () => {
			...
			const hash = parent.global.decodeURIComponent(
				this.hash?.slice(1)?.toLowerCase(),
			);
			parent.traverse(
				null,
				(node, state) => {
					if (node.bookmark?.label?.toLowerCase() == hash) {
						if (destinationNode) {
							parent.announce(
								"You have multiple bookmarks with the same label: " +
									node.bookmark.label,
							);
						} else {
		...
		addOrRemoveBookmark = () => {
			...
			const label = this.toBookmarkName(
				parent.global.prompt(trans.noarg("addBookmarkPrompt"), oldLabel),
			);
			...
			const comments = (node.comments || []).filter(
				(c) => c.by?.id == myName || r.test(c.text),
			);
			let commentText = comments.map((c) => c.text).join("\r\n\r\n");
			r.lastIndex = 0;
			commentText =
				(label ? "bkm:" + label + " " : "") + commentText.replace(r, "").trim();
		...
		getBookmarkUrl = async (bookmark) => {
			...
			const url =
				parent.global.location.origin +
				"/study/" +
				study.data.id +
				"/" +
				chapterId +
				"#" +
				parent.global.encodeURIComponent(label);
			const result = await parent.global.navigator.permissions.query({
		...
		collapseExpandAll = () => {
			...
			if (anyCollapsed) {
				$(
					"move.lichessTools-bookmark.lichessTools-collapsed bookmark button",
				).trigger("click");
			} else {
				$(
					"move.lichessTools-bookmark:not(.lichessTools-collapsed) bookmark button",
				).trigger("click");
			}
		};
		...
		bookmarkSplit = async (ev) => {
			...
			while (
				...
				study.currentChapter().id == position.chapterId
			) {
		...
		analysisContextMenu = () => {
			...
			if (analysis.contextMenuPath) {
				...
				if (node?.bookmark?.label) {
					...
					menuItem.attr("data-icon", icon).text(text).attr("title", title);
		...
		setupAnalysisPopup = () => {
			...
			if (this.options.analysisPopup && study) {
				...
				if (container.length) {
					...
					if (!container.length) {
						container = $(
							'<div class="lichessTools-moveListOptions-header">',
				...
				if (!button.length) {
					button = $('<a class="lichessTools-analysisPopup">')
						...
						.on("click", (ev) => {
							ev.preventDefault();
							this.popup?.close();
							this.popup = parent.global.open(
								parent.global.location.href,
								"lichessTools-moves",
								"fullscreen=yes,menubar=no,location=no,status=no,titlebar=no,toolbar=no,",
							);
							this.popup.addEventListener("DOMContentLoaded", () =>
								$("body", this.popup.document).addClass(
									"lichessTools-analysisPopup",
								),
							);
		...
		setupCevalToggle = () => {
			const parent = this.lichessTools;
			const $ = parent.$;
			$("main.analyse").toggleClass(
				"lichessTools-fixCevalToggle",
				this.options.fixCevalToggle,
			);
		};
		...
		addMissingIndexes = () => {
			...
			const $ = parent.$;
			$.cached(
				".tview2.lichessTools-indentedVariations inline+move",
				2000,
			).each((i, e) => {
				e = $(e);
				...
				elem.addClass("lichessTools-index").text(elem.text() + "..");
				$(e).prepend(elem);
			});
		};
		...
		async start() {
			...
			lichess.pubsub.off(
				"lichessTools.redraw",
				this.debouncedAddCommentBookmarks,
			);
			lichess.pubsub.off(
			...
			$.cached("body")
				.toggleClass(
					"lichessTools-fullWidthAnalysis",
					this.options.fullWidthAnalysis,
				)
			...
			$(".lichessTools-fixCevalToggle").removeClass(
				"lichessTools-fixCevalToggle",
			);
