	...
	class MovesFromTranspositionsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"options.movesFromTranspositions":
			...
			"ro-RO": {
				...
				"options.movesFromTranspositions":
					"Prezint\u0103 mut\u0103ri urm\u0103toare transpozi\u0163iilor la pozi\u0163ia curent\u0103",
		...
		findTranspositions = () => {
			...
			const transpoData =
				transpositions.map((n) => n.uci).join(",") +
				(noDuplicates ? "ND" : "D");
			...
			const addForkMove = (targetElem, path, child, isNextMove) => {
				...
				if (glyph) {
					forkMove.append(
						$("<glyph>").attr("title", glyph.name).text(glyph.symbol),
					);
				}
				if (isNextMove) {
			...
			for (const node of transpositions) {
				...
				for (const child of node.children) {
					...
					const forkMove = $("move", fork).filter(
						(i, e) => $(e).attr("p") == path,
					);
					if (forkMove.length) continue;
					const targetElem = parent.getElementForPath(path);
					if (
						currNode.children.length == 1 &&
						child.san == currNode.children[0].san &&
						!onlyMoveAdded
					) {
