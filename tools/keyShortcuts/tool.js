(() => {
	class KeyShortcutsTool extends LiChessTools.Tools.ToolBase {
		...
		clearMoveMode = () => {
			...
			$.cached("body").removeClass(
				"lichessTools-keyMode-pgn lichessTools-keyMode-ceval lichessTools-keyMode-explorer lichessTools-keyMode-general",
			);
		};
		...
		toggleSiteHeader = () => {
			...
			const parent = this.lichessTools;
			const $ = parent.$;
			$("body").toggleClass("lichessTools-hideSiteHeader");
		};
		...
		randomChapter = () => {
			...
			const $ = parent.$;
			const button = $(
				'div.lichessTools-chapterControls button[data-act="random"]',
			);
		...
		bindKeysForAnalysis = () => {
			...
			for (let i = 1; i <= 9; i++) {
				...
				if (!this.oldHandlers[combo])
					this.oldHandlers[combo] = parent.getKeyHandler(combo);
			...
			if (this.options.enabled) {
				...
				parent.bindKeyHandler("g", () =>
					parent.jumpToGlyphSymbols(["!", "!?", "!!", "\u2606"]),
				);
				parent.bindKeyHandler("alt+i", () =>
					parent.jumpToGlyphSymbols("?!", true),
				);
				parent.bindKeyHandler("alt+m", () =>
					parent.jumpToGlyphSymbols("?", true),
				);
				parent.bindKeyHandler("alt+b", () =>
					parent.jumpToGlyphSymbols("??", true),
				);
				parent.bindKeyHandler("alt+g", () =>
					parent.jumpToGlyphSymbols(["!", "!?", "!!", "\u2606"], true),
				);
				...
				if (this.oldHandlers) {
					...
					for (let i = 1; i <= 9; i++) {
						const combo = i.toString();
						if (this.oldHandlers[combo])
							parent.bindKeyHandler(combo, this.oldHandlers[combo], true);
					}
				}
			}
		};
		...
		bindKeysForEditor = () => {
			...
			if (this.options.enabled) {
				for (let i = 1; i <= 8; i++) {
					...
					parent.bindKeyHandler("shift+" + combo, () =>
						this.handleEditorDigit(i, false),
					);
				}
