(() => {
	class BoardImageTool extends LiChessTools.Tools.ToolBase {
		...
		drawSvg = async (svgElement) => {
			...
			const $ = parent.$;
			svgElement = $(svgElement).clone().css({
				opacity: 0.6,
				overflow: "visible",
			});
			svgElement.find("svg").css("overflow", "visible");
			const svgURL = new parent.global.XMLSerializer().serializeToString(
				svgElement[0],
			);
			const url =
				"data:image/svg+xml; charset=utf8, " +
				parent.global.encodeURIComponent(svgURL);
		...
		getBoardImage = async (ev) => {
			...
			const backgroundText = window.getComputedStyle(
				board[0],
				":before",
			).backgroundImage;
		...
		enhanceButtonDirect = () => {
			...
			if (analysisPgn.length) {
				if (!analysisPgn.parent().find("a.lichessTools-boardImage").length) {
					...
					const url =
						"/export/fen.gif?fen=" +
						parent.global.encodeURIComponent(analysis.node.fen) +
						"&color=" +
