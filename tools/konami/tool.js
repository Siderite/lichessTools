(() => {
	class KonamiTool extends LiChessTools.Tools.ToolBase {
		...
		async start() {
			...
			parent.global.document.removeEventListener("keydown", this.keyCheck, {
				capture: true,
			});
			if (!value) return;
			parent.global.document.addEventListener("keydown", this.keyCheck, {
				capture: true,
			});
		}
	}
