	...
	class ExplorerPracticeTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.explorerPractice":
					"Antrenament contra mut\u0103ri din Explorator",
				"options.explorerPracticeOptions":
					"Op\u0163iuni pentru Antrenament cu Exploratorul",
				explorerPracticeTitle:
					"LiChess Tools - antrenament contra mut\u0103ri din Explorator",
				outOfMoves: "Nu mai sunt mut\u0103ri \u00een Explorator",
				"explorerPracticeOptions.showSmileys":
					"Arat\u0103 emoji c\u00E2nd nu mai sunt mut\u0103ri",
				"explorerPracticeOptions.sumClick":
					"Click pe \u03A3 \u00een Explorator pentru a muta",
				"explorerPracticeOptions.showNames": "Arat\u0103 numele adversarului",
				"explorerPracticeOptions.showInfoStudy": "Date Explorer \u00een Studii",
				"explorerPracticeOptions.showInfoAnalysis":
					"Date Explorer \u00een Analiz\u0103",
		...
		writePlayerName = async () => {
			...
			if (!label.length) {
				label = $(
					'<label class="lichessTools-explorerPractice"><span></span></label>',
				).appendTo(board);
			}
		...
		playMove = (ignoreSide) => {
			...
			const total = moves
				.map((m) => (m.total = m.black + m.draws + m.white))
			...
			if (ceval) {
				if (!analysis.node.glyphs?.length) {
					...
					const winValue =
						(ceval.cp || Math.sign(ceval.mate) * 1000) * boardSign;
					let symbol = "\uD83D\uDE10";
		...
		process = () => {
			...
			if (this.options.sumClick) {
				...
				sumRow.removeClass("lichessTools-sumClick").removeAttr("title");
			}
			...
			explorerContainer.toggleClass(
				"lichessTools-hideExplorerData",
				!!this.isRunning && hideData,
			);
		...
		async start() {
			...
			$("main.analyse div.analyse__controls").off(
				"click touchend",
				this.process,
			);
			parent.unbindKeyHandler("shift+l");
			analysis.userJump = parent.unwrapFunction(
				analysis.userJump,
				"explorerPractice",
			);
			if (!value) {
			...
			$("main.analyse div.analyse__controls").on(
				"click touchend",
				this.process,
			);
