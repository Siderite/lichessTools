	...
	class StickyAnalysisTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.stickyAnalysis":
					"Salveaz\u0103 automat analiza pentru re\u00eencarcare",
			},
		};
		...
		saveAnalysisPgn = () => {
			...
			if (
				...
				!/\[Orientation|\[StartFlipped/.test(pgn)
			) {
				pgn =
					'[Orientation "Black"]\r\n[StartFlipped "1"]\r\n' +
			...
			if (!parent.isStartFen(analysis.tree.root.fen)) {
				pgn =
					'[FEN "' +
					analysis.tree.root.fen +
					'"]\r\n' +
					pgn?.replace(/\[FEN\s+"[^"]*"\][\r\n]+/g, "");
			}
			if (this.prevPgn === pgn) return;
			const savedPgn = parent.storage.get("LiChessTools.stickyAnalysis.pgn", {
				zip: true,
			});
			if (savedPgn != pgn) {
				parent.storage.set("LiChessTools.stickyAnalysis.pgn", pgn, {
					zip: true,
				});
			}
			this.prevPgn = pgn;
		};
		saveAnalysisPgnLong = this.lichessTools.debounce(
