(() => {
	class PlayerLagTool extends LiChessTools.Tools.ToolBase {
		...
		getLag = async (username) => {
			...
			const data = await parent.api.user.getUserStatus([username], {
				withSignal: true,
			});
		...
		refreshPlayers = () => {
			...
			$(
				...
				if (!isPlayer) {
					if (
						!this.opponentLagFrequency ||
						$(e).parent().find("good,bad").length
					)
						return;
				}
				const lag = isPlayer ? this._lag : await this.getLag(hrefUserId);
		...
		refreshLagBars = (container, lag, latency) => {
			...
			signal
				.attr(
					"title",
					trans
						.noarg("playerLagTitle")
						.replaceAll("%1", Math.round(lag))
						.replaceAll("%2", Math.round(latency)),
				)
		...
		refreshLagChart = (container, lag, latency) => {
			...
			let chart;
			if (!canvas.length) {
				canvas = $('<canvas class="lichessTools-playerLag">').appendTo(
					container,
				);
				chart = { lag: [], latency: [] };
		...
		async start() {
			...
			$(
				".round__app .ruser-top a.user-link .lichessTools-playerLag,.round__app .ruser-bottom a.user-link .lichessTools-playerLag",
			).remove();
