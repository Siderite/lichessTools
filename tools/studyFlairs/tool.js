	...
	class StudyFlairsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				flairButtonTitle:
					"LiChess Tools - adaug\u0103 o pictogram\u0103 studiului",
		...
		processStudy = () => {
					...
					if (!$("button.lichessTools-studyFlairs", container).length) {
						...
						$('<button class="button lichessTools-studyFlairs">')
							...
							.on("click", (ev) => {
								ev.preventDefault();
								const close = () =>
									flairPicker.removeClass("emoji-done").empty();
					...
					if (!parent.isWrappedFunction(tagify.createTagElem, "studyFlairs")) {
						const handleTag = (tag) => {
							...
							if (m) {
								...
								span.replaceWith($("<img>").attr("src", url));
					...
					if (!parent.isWrappedFunction(tagify.dropdown.show, "studyFlairs")) {
						tagify.dropdown.show = parent.wrapFunction(tagify.dropdown.show, {
							...
							before: ($this, term) => {
								...
								parent.arrayRemoveAll(tagify.settings.whitelist, (f) =>
									/^flair\.([^\)]+)/.test(f),
								);
			...
			$(
				'div.study__topics a.topic, nav.page-menu__menu a[href^="/study/topic/flair."], .topic-list a[href^="/study/topic/flair."]',
			).each((i, e) => {
				const m = /^flair\.([^\)]+)/.exec($(e).text());
				if (m) {
					const url = lichess.asset.flairSrc(m[1]);
		...
		processStudyList = async () => {
			...
			while (this._currentPage < page) {
				const json = await parent.api.study.getStudyListPage(
					baseUrl,
					this._currentPage + 1,
				);
			...
			$("div.study.paginated").each((i, e) => {
				...
				if (this.options.topicFlairs && study.flair) {
					flairs.push({
						title: trans.noarg("studyFlairTitle"),
						flair: study.flair,
					});
				}
				...
				if (this.options.authorFlair && study.owner?.flair) {
					flairs.push({
						title: this.getFullname(study.owner),
						flair: study.owner.flair,
						url: "/@/" + study.owner.id,
					});
				}
				...
				if (flairs.length) {
					...
					if (flairs[0].url) {
						...
						elem.on("contextmenu", (ev) => {
							ev.preventDefault();
							parent.global.location =
								"/study/topic/flair." + flairs[0].title + "/" + mode;
						});
					}
					if (flairs.length > 1) {
						const container = $(
							'<div class="lichessTools-bottomFlairs">',
						).appendTo(e);
						for (let i = 1; i < flairs.length; i++) {
							...
							if (flairs[i].url) {
								...
								elem.on("contextmenu", (ev) => {
									ev.preventDefault();
									parent.global.location =
										"/study/topic/flair." + flairs[i].title + "/" + mode;
								});
							}
						}
					}
		...
		processStudyListDebounced = this.lichessTools.debounce(
		...
		async start() {
			...
			if (
				/^\/study\b/.test(parent.global.location.pathname) &&
				$(".studies.list").length
			) {
