	...
	class SkipMoveCommandTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"skipMoveCommand.helpText":
					'/skipmove\r\nMove for both players to "skip" the move',
				cannotSkipMove: "Cannot skip move from this position!",
			},
			"ro-RO": {
				"options.skipMoveCommand":
					"Comand\u0103: mut\u0103 pentru a schimba cine e la r\u00e2nd",
				"skipMoveCommand.helpText":
					"/skipmove\r\nMut\u0103 pentru a schimba cine e la r\u00e2nd",
				cannotSkipMove:
					"Nu pot schimba cine e la r\u00e2nd din aceasta pozi\u0163ie!",
			},
		};
		...
		skipMoveIfPossible = () => {
			...
			const findNextMove = () => {
				...
				const moves = Array.from(ch.board.rook)
					.concat(Array.from(ch.board.bishop))
					.concat(Array.from(ch.board.queen))
					.concat(Array.from(ch.board.king))
					.filter((p) => !lastPiece || p == lastPiece)
					.flatMap((p) =>
						Array.from(ch.dests(p)).map((d) => {
							return { from: p, to: d };
						}),
					)
