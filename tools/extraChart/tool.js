	...
	class ExtraChartTool extends LiChessTools.Tools.ToolBase {
		...
		preferences = [
			{
				...
				defaultValue:
					"material,principled,tension,brilliant,accuracy,smooth,gauge",
				defaultNotLoggedInValue:
					"material,principled,tension,brilliant,accuracy,smooth,gauge,local,moreBrilliant",
				advanced: true,
			},
			{
				name: "christmas",
		...
		intl = {
			"en-US": {
				...
				"options.christmas":
					"Show Christmas lights on chart on the 25th of December",
			},
			"ro-RO": {
				...
				goodMovesTitle:
					"LiChess Tools - mut\u0103ri bune/briliante/interesante",
		...
		colors = {
			...
			localChart: () => (this.lichessTools.isDark() ? "#EFEF00" : "#705800"),
			localChartHover: () =>
				this.lichessTools.isDark() ? "#FFFF00" : "#604800",
			accuracyChart: () => (this.lichessTools.isDark() ? "#FF00FF" : "#700058"),
			maxTensionLine: "#FF0000",
			maxPotentialLine: "#008000",
			interestingMoves: () =>
				this.lichessTools.isDark() ? "#168226" : "#009914",
		};
						...
						for (let dy = -1; dy <= 1; dy++) {
							...
							for (let i = 1; i < 8; i++) {
								...
								if (m != pm) {
									...
									if (
										(isForward && pc.toLowerCase() === "p") ||
										pc.toLowerCase() === "k"
									) {
										const ppc =
											this.onBoard(x + dx * (i + 1), y + dy * (i + 1)) &&
											board[y + dy * (i + 1)][x + dx * (i + 1)];
										if (ppc) {
				...
				case "p":
					if (this.onBoard(x - 1, y - m)) {
						...
						if (pc) {
							...
							board.enpassant &&
							board.enpassant.x == x - 1 &&
							board.enpassant.y == y - m
						) {
							const pm = -m;
							underAttack.push({
								m: m,
		...
		tension = (node) => {
			...
			const result = [
				...new Set(underAttack.map((i) => i.x + "," + i.y + "=" + i.pc)),
			]
		...
		inCheck = (fen) => {
			...
			const moves = this.getAllCapturingMoves(board).filter(
				(m) => m.m != side && m.x == king.x && m.y == king.y,
			);
			return !!moves.length;
		};
		...
		materialWon = (board, x, y) => {
			...
			const moves = this.getAllCapturingMoves(board)
				...
				.sort(
					(m1, m2) =>
						this.pieceMaterial[m1.spc.toLowerCase()] -
						this.pieceMaterial[m2.spc.toLowerCase()],
				);
			const move = moves[0];
			if (!move) return 0;
      ...
      return toRemove.length?this.smooth(points):points;*/
		...
		getAccuracyData = (mainline) => {
			...
			return mainline
				.map((node, x) => {
					...
					const accuracy =
						103.1668 * Math.exp(-0.04354 * (prevWinPerc - winPerc)) - 3.1669;
					prevWinPerc = winPerc;
		...
		setBrilliant = (mainline, forced) => {
			...
			const parent = this.lichessTools;
			const isLocalChart = $("#acpl-chart-container").is(
				".lichessTools-extraChart",
			);
			const Math = parent.global.Math;
				...
				.forEach((v, x) => {
					...
					if (v.good) {
						...
						if (v.bril) {
							...
							symbol = "?";
							name = "Mistake";
						} else if (v.inaccuracy) {
							symbol = "?!";
							name = "Inaccuracy";
						}
					}
					const glyphs = mainline[x].glyphs || [];
					parent.arrayRemoveAll(
						glyphs,
						(g) =>
							g.type == "nonStandard" &&
							["!", "!?", "!!", "\u2606"].includes(g.symbol),
		...
		showGoodMoves = (forced) => {
			...
			const hcElem = $(
				"#acpl-chart-container.lichessTools-extraChart, div.computer-analysis.active #acpl-chart-container, div.study__server-eval.ready",
			)[0];
			...
			const fill = (container, count, color) => {
				...
				if (!elem.length) {
					elem = $("<div></div>")
						...
						.on("click", (ev) => {
							ev.preventDefault();
							parent.jumpToGlyphSymbols(
								this.options.moreBrilliant
									? ["!?", "!!"]
									: ["!", "!?", "!!", "\u2606"],
								color,
							);
						...
						.on("mouseenter", (ev) => {
							...
							if (!this.colors.beforeInterestingMoves)
								this.colors.beforeInterestingMoves =
									dataset.hoverBackgroundColor || this.colors.originalChart;
							dataset.hoverBackgroundColor = this.colors.interestingMoves();
							dataset.pointHoverBackgroundColor =
								this.colors.interestingMoves();
		...
		showChristmasTree = async () => {
			...
			const xElem = $('<span class="lichessTools-christmas">').text(
				trans.noarg("merryChristmas"),
			);
			container.append(xElem);
			const initHoverBackgroundColor = dataset.hoverBackgroundColor;
			const initActiveElements = chart.getActiveElements()?.map((e) => {
				return { datasetIndex: e.datasetIndex, index: e.index };
			});
			const colors = [
				"red",
				"green",
			...
			for (let i = 0; i < 30; i++) {
				const elements = dataset.data
					.filter((d) => parent.random() < 0.3)
					.map((d) => {
						return { datasetIndex: 0, index: d.x - 1 };
					});
		...
		clearCharts = () => {
			...
			if (!chart) return;
			const parent = this.lichessTools;
			const removed = parent.arrayRemoveAll(chart.data.datasets, (d) =>
				[
					"Material",
					"Principled",
					"Local",
			...
			if (removed.length) {
				chart.options.scales.x.max = Math.max.apply(
					null,
					chart.data.datasets.map((ds) => ds.data.map((p) => p.x)).flat(),
				);
				chart.update("none");
		...
		chartClick = (_event, elements, _chart) => {
			...
			const index = this._chart.data.datasets.findIndex(
				(s) => s.label === "Local",
			);
			const data =
				elements[
					elements.findIndex((element) =>
						[0, index].includes(element.datasetIndex),
					)
				];
		...
		generateCharts = async (forced) => {
			...
			if (!chart) {
				...
				if (!chart && this.options.local && !lichess.analysis.study) {
					const underboard =
						$(".analyse__underboard div.computer-analysis")[0] ||
						$(".analyse__underboard")[0];
					if (underboard) {
						container = $(
							'<div id="acpl-chart-container" class="lichessTools-extraChart"><canvas id="acpl-chart"></canvas></div>',
						);
						...
						chart = await mod.acpl(
							$("#acpl-chart")[0],
							lichess.analysis.data,
							mainline,
							lichess.analysis.trans,
						);
					}
				}
					...
					$('<div class="lichessTools-chartInfo">')
						...
						.append(
							$('<a target="_blank">')
								...
								.attr(
									"href",
									"https://siderite.dev/blog/lichess-tools---user-manual#extraChart",
								),
						)
						.appendTo(container);
				}
			...
			let existingMaterial = chart.data.datasets.findIndex(
				(s) => s.label === "Material",
			);
			if (
				existingMaterial >= 0 &&
				(this.prevSmooth != this.options.smooth || !this.options.material)
			) {
			...
			if (this.options.material) {
				if (existingMaterial < 0) {
					chart.data.datasets.push({
						...
						borderWidth: 2,
						borderDash: [3, 3],
						cubicInterpolationMode: this.options.smooth
							? "monotone"
							: "default",
						tension: 0,
						pointRadius: 0,
					...
					if (dataset.data?.length < lichess.analysis.mainline.length) {
						const newData = this.smooth(
							this.getMaterialData(lichess.analysis.mainline),
						);
			...
			let existingPrincipled = chart.data.datasets.findIndex(
				(s) => s.label === "Principled",
			);
			if (
				existingPrincipled >= 0 &&
				(this.prevSmooth != this.options.smooth || !this.options.principled)
			) {
			...
			if (this.options.principled) {
				if (existingPrincipled < 0) {
					chart.data.datasets.push({
						...
						type: "line",
						data: this.smooth(
							this.getPrincipledData(lichess.analysis.mainline),
						),
						borderWidth: 2,
						borderDash: [4, 2],
						cubicInterpolationMode: this.options.smooth
			...
			let existingLocal = chart.data.datasets.findIndex(
				(s) => s.label === "Local",
			);
			if (
				existingLocal >= 0 &&
				(this.prevSmooth != this.options.smooth || !this.options.local)
			) {
			...
			let existingAccuracy = chart.data.datasets.findIndex(
				(s) => s.label === "Accuracy",
			);
			if (
				existingAccuracy >= 0 &&
				(this.prevSmooth != this.options.smooth || !this.options.accuracy)
			) {
			...
			let existingMaxTension = chart.data.datasets.findIndex(
				(s) => s.label === "Max tension",
			);
			...
			let existingMaxPotential = chart.data.datasets.findIndex(
				(s) => s.label === "Max potential",
			);
			...
			if (
				this.options.christmas &&
				new Date().toISOString().includes("-12-25")
			) {
		...
		async start() {
			...
			if (!this.options.gauge) {
				$(
					"div.eval-gauge tick.lichessTools-material,div.eval-gauge tick.lichessTools-principled",
				).remove();
				this.prevFen = null;
			}
			if (!this.options.local) {
			...
			lichess.pubsub.off(
				"lichessTools.chapterChange",
				this.forceGenerateCharts,
			);
			if (this.options.brilliant) {
				lichess.pubsub.on(
					"lichessTools.chapterChange",
