(() => {
	class AutoUnselectPieceTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				"options.play": "Joc",
				"options.autoUnselectPiece":
					"Deselecteaz\u0103 pies\u0103 dup\u0103 un num\u0103r de secunde",
			},
		};
		...
		select = (square) => {
			...
			if (Date.now() - this._selected.time < this.options.seconds * 1000)
				return;
			this.unselectPiece();
			this.select(null);
		};
		...
		async start() {
			...
			this.unselectPiece = () => {
				const isLastMove =
					$("div.tview2 move:last-child").is(".active") ||
					$("div.ruser-top + * div.buttons + *>:last-child")[0]?.className;
				if (isLastMove)
					bindings.down?.find((b) => b.combination == "down")?.callback();
			};
			this.discoverChatInterval = parent.global.setInterval(
				this.checkPieceSelection,
				500,
			);
		}
	}
