	...
	class ExplorerSettingsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				explorerPracticeSettingTitle:
					"Button to practice against Explorer moves",
			...
			"ro-RO": {
				...
				explorerPracticeSettingTitle:
					"Buton pentru antrenament contra mut\u0103ri din Explorator",
		...
		showSettingsDirect = () => {
			...
			if (!section.length) {
				...
				if (parent.getToolByName("ExplorerEval")) {
					choices.append(
						$('<button class="lichessTools-moveEvaluation">')
							.text(trans("moveEvaluationSettingText"))
							.attr("title", trans("moveEvaluationSettingTitle"))
							.on("click", (ev) => {
								ev.preventDefault();
				...
				if (parent.getToolByName("ExplorerGambits")) {
					choices.append(
						$('<button class="lichessTools-gambits">')
							.text(trans("gambitsSettingText"))
							.attr("title", trans("gambitsSettingTitle"))
							.on("click", (ev) => {
								ev.preventDefault();
				...
				if (parent.getToolByName("ExplorerPractice")) {
					choices.append(
						$('<button class="lichessTools-explorerPractice">')
							.text(trans("explorerPracticeSettingText"))
							.attr("title", trans("explorerPracticeSettingTitle"))
							.on("click", (ev) => {
								ev.preventDefault();
								const value =
									parent.currentOptions.getValue("explorerPractice");
				...
				if (parent.getToolByName("OpeningExplorerUsers")) {
					choices.append(
						$('<button class="lichessTools-meButton">')
							.text(trans("meButtonSettingText"))
							.attr("title", trans("meButtonSettingTitle"))
							.on("click", (ev) => {
								ev.preventDefault();
								const value = parent.currentOptions.getValue(
									"openingExplorerUsers",
								);
			...
			$("button.lichessTools-meButton", section).attr(
				"aria-pressed",
				value.toString(),
			);
		};
