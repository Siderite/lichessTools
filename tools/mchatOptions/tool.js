	...
	class MchatOptionsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"mchatOptions.teamChatNotifications":
					"Notific\u0103ri la chat \u00een echip\u0103",
				...
				maximumThreeTeams:
					"Ai atins num\u0103rul maxim de echipe pentru notificare!",
			},
		};
		...
		processChat = () => {
			...
			const createTextNode = parent.global.document.createTextNode.bind(
				parent.global.document,
			);
			if (this.options.urlify) {
				$("li>t", container).each((i, e) => {
					if (!parent.inViewport(e)) return;
					const textNodes = Array.from(e.childNodes).filter(
						(n) => n.nodeType == 3,
					);
					for (const textNode of textNodes) {
						...
						for (const match of matches) {
							...
							if (match.index > p) {
								newNodes.push(
									createTextNode(lineText.substring(p, match.index)),
								);
							}
						...
						if (p < lineText.length) {
							newNodes.push(
								createTextNode(lineText.substring(p, lineText.length)),
							);
						}
			...
			if (this.options.images) {
				$("a.lichessTools-chat-url:not(:has(img))", container).each((i, e) => {
					...
					$("<img>").attr("src", url.toString()).appendTo($(e).empty());
				});
			}
					...
					if (oldHandler) {
						...
						const splitLength = (s, l) => {
							...
							while (p < s.length && p + l - 2 < s.length) {
								result.push(
									(p > 0 ? ellipsis : "") + s.substr(p, l - 2) + ellipsis,
								);
								p += l - 2;
							}
							if (p < s.length) {
		...
		createSocket = (teamId) => {
			...
			socket.pubsub = {
				emit: (type, data) => {
					if (type == "socket.in.message")
						this.receiveChatMessage(teamId, data);
				},
			};
			return socket;
		};
		...
		handleNotificationsDirect = async () => {
			...
			for (const team of this.teamsData) {
				if (
					team.newMessage &&
					JSON.stringify(team.newMessage) != JSON.stringify(team.lastMessage)
				) {
					const teamName =
						this.userTeams.find((t) => t.id == team.teamId)?.name ||
						team.teamId;
					const notification = {
						getEntries: async () => {
							const entry = {
								...
								content: $("<div>")
									.append(
										$("<span>").text(
											trans.pluralSame("newTeamMessagesText", teamName),
										),
									)
									.html(),
		...
		handleNotifications = this.lichessTools.debounce(
		...
		notificationButtonInTeams = () => {
			...
			$("table.slist tr.paginated").each((i, e) => {
				...
				const teamId = /^\/team\/(?<teamId>[^\/]+)$/i.exec(href)?.groups
					?.teamId;
				if (!teamId) return;
				...
				button.toggleClass(
					"lichessTools-enabled",
					!!this.teamsData?.find((t) => t.teamId == teamId),
				);
			});
		};
		...
		async start() {
			...
			this.options = {
				...
				teamChatNotifications: parent.isOptionSet(
					value,
					"teamChatNotifications",
				),
			};
			...
			if (
				$("section.mchat").length &&
				(this.options.urlify || this.options.unlimited || this.options.images)
			) {
			...
			if (this.options.teamChatNotifications) {
				...
				this.teamsData =
					parent.storage.get("LichessTools.chatNotificationTeams") || [];
				const configuredTeamsCount = this.teamsData?.length;
				if (configuredTeamsCount) {
					parent.arrayRemoveAll(
						this.teamsData,
						(t) => !this.userTeams.find((ut) => ut.id == t.teamId),
					);
					...
					const teams = this.teamsData.filter(
						(t) => t.teamId != m?.groups?.teamId,
					);
