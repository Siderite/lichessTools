(() => {
	class InterceptEventHandlersTool extends LiChessTools.Tools.ToolBase {
		...
		removeEventHandlers = (target, type) => {
			const items = this.eventHandlers.filter(
				(eh) => eh.target === target && eh.type === type,
			);
		...
		async init() {
			...
			EventTarget.prototype.addEventListener = wrap(
			...
			EventTarget.prototype.removeEventListener = wrap(
				EventTarget.prototype.removeEventListener,
				{
					...
					before: (target, type, listener, options) => {
						const useCapture =
							typeof options === "boolean" ? options : !!options?.capture;
						this.lichessTools.arrayRemoveAll(
							this.eventHandlers,
							(eh) =>
								eh.target === target &&
								eh.type === type &&
