	...
	class ShapeRankTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.shapeRank":
					"Arat\u0103 ordinea s\u0103ge\u0163ilor \u015Fi cercurilor",
			},
		};
		...
		ensureShapeRank = () => {
			...
			this.chessground =
				analysis?.chessground ||
				$("div.cg-wrap.lichessTools-boardOverlay")[0]?.chessground;
						...
						if (shapes) {
							...
							for (const shape of shapes) {
								...
								const rankShape = {
									...
									customSvg: parent.makeSvg(
										'<text x="10%" y="50%" font-size="200%" fill="black" stroke="' +
											shape.brush +
		...
		waitForChessground = () => {
			...
			const isWrapped = parent.isWrappedFunction(
				this.chessground.state.drawable.onChange,
				"shapeRank",
			);
			if (this.options.enabled) {
				if (!isWrapped && this.chessground.state.drawable.onChange) {
					this.chessground.state.drawable.onChange = parent.wrapFunction(
						this.chessground.state.drawable.onChange,
						{
							id: "shapeRank",
							before: ($this, ...args) => {
								const originalFunction =
									this.chessground.state.drawable.onChange.__originalFunction.bind(
										$this,
									);
								if (args[0]?.length) {
				...
				if (isWrapped) {
					this.chessground.state.drawable.onChange = parent.unwrapFunction(
						this.chessground.state.drawable.onChange,
						"shapeRank",
					);
				}
