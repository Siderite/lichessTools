	...
	class MobileExperienceTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.mobileExperienceRound":
					"Op\u0163iuni pentru joc pe aparate mobile",
				...
				"mobileExperience.shapeDrawing":
					"S\u0103ge\u0163i \u00een analiz\u0103",
				"mobileExperience.randomNextMove": "Buton mutare aleatoare",
				"mobileExperience.selectiveRandom":
					"...doar c\u00e2nd sunt varia\u0163iuni",
				"mobileExperienceRound.shapeDrawingRound":
					"S\u0103ge\u0163i \u00een joc",
				"mobileExperienceRound.standardButtons": "Butoane standard",
				"mobileExperienceRound.invert": "Inverseaz\u0103 user \u015fi ceas",
				shapeDrawingTitle:
					"LiChess Tools - deseneaz\u0103 s\u0103ge\u0163i \u015Fi cercuri",
		...
		handleGesture = (shape) => {
			...
			const existing = drawable.shapes.find(
				(s) =>
					s.orig === shape.orig &&
					s.dest === shape.dest &&
					s.brush === shape.brush,
		...
		initializeOverlayWrap = async () => {
			...
			const cg = Chessground(wrap[0], {
				...
				drawable: {
					enabled: false,
					defaultSnapToValidMove: lichess.storage
						.boolean("arrow.snap")
						.getOrDefault(true),
				},
		...
		toggleBrush = (ev) => {
			...
			this.drawingBrush =
				index >= this.options.colorCount ? null : this.brushes[index];
			...
			for (const brush of this.brushes) {
				$(ev.target).toggleClass(
					"lichessTools-" + brush + "Brush",
		...
		handleRedraw = async () => {
			...
			const isEnabled = !!(
				this.options.shapeDrawing ||
				this.options.shapeDrawingRound ||
				this.options.randomNextMove
			);
			...
			if (isAnalyse) {
				...
				$("main")
					...
					.toggleClass(
						"lichessTools-standardButtons",
						this.options.standardButtons,
					);
			...
			if (isAnalyse) {
				...
				if (this.options.shapeDrawing) {
					if (
						!$(
							"div.analyse__controls div.features button.lichessTools-shapeDrawing",
						).length
					) {
						$('<button class="fbt">')
							.attr("data-icon", "\u21D7")
				...
				if (this.options.randomNextMove) {
					if (
						!$(
							"div.analyse__controls div.jumps button.lichessTools-randomNextMove",
						).length
					) {
						$('<button class="fbt">')
							.attr("data-icon", "\u21C9")
							...
							.insertBefore(
								$('div.analyse__controls div.jumps button[data-act="next"]'),
							);
						addHandler = true;
					}
					const hasVariations =
						!this.options.selectiveRandom ||
						parent.getNextMoves(lichess.analysis.node).length > 1;
					$(
						"div.analyse__controls div.jumps button.lichessTools-randomNextMove",
					).toggle(hasVariations);
				} else {
					$(
						"div.analyse__controls div.jumps button.lichessTools-randomNextMove",
					).remove();
				...
				if (addHandler) {
					...
					if (elem) {
						if (!this.originalHandler) {
							this.originalHandler = parent
								.getEventHandlers(elem, "touchstart")
								?.at(0)
								?.bind(elem);
						}
				...
				if (this.options.shapeDrawingRound) {
					...
					wrap?.remove();
					$(
						"div.rcontrols div.ricons button.lichessTools-shapeDrawing",
					).remove();
				}
			}
		};
		...
		async start() {
			...
			const mobileExperience =
				parent.currentOptions.getValue("mobileExperience");
			const mobileExperienceRound = parent.currentOptions.getValue(
				"mobileExperienceRound",
			);
			...
			this.logOption(
				"... color count",
				parent.currentOptions.getValue("colorCount"),
			);
			this.options = {
				...
				selectiveRandom: parent.isOptionSet(
					mobileExperience,
					"selectiveRandom",
				),
				shapeDrawingRound: parent.isOptionSet(
					mobileExperienceRound,
					"shapeDrawingRound",
				),
				standardButtons: parent.isOptionSet(
					mobileExperienceRound,
					"standardButtons",
				),
			...
			if (isRound) {
				const lockBoardElem = $(
					"#top div.site-buttons div.lichessTools-lockBoard",
				);
				...
				$("main div.cg-wrap:not(.lichessTools-boardOverlay)").off(
					"click",
					this.clearShapes,
				);
				if (this.options.shapeDrawingRound) {
					lichess.pubsub.on("ply", this.clearShapes);
					$("main div.cg-wrap:not(.lichessTools-boardOverlay)").on(
						"click",
						this.clearShapes,
					);
				}
