...
initCheckBoard(startCheckPointIndex) {
      ...
      if (doAdd && row !== "") {
        for (let j = 0; j < row.length; j++) {
          ...
          if (char !== " " && char !== ".") {
            ...
            const pieceCell = this.addPieceAt(lowerChar, currentRowIndex, j);
  ...
  if (startCheckPointIndex === 0) {
    ...
    this.player = this.addPieceAt(this.Piece.HERO_KING, this.progress, 3).piece;
    this.progress -= 4;
  }
}
...
destroyPiece(piece) {
  ...
  if (piece.showThreat) {
    ...
    for (let i = piece.row - this.NUM_CELLS; i < piece.row + this.NUM_CELLS; i++) {
      const rowContent = this.checkBoard[i];
      if (rowContent) {
               
        for (let j = piece.col - this.NUM_CELLS; j < piece.col + this.NUM_CELLS; j++) {
...
computeCellPos(row, col) {
  const res = this.projection.project((col + 0.5) / this.NUM_CELLS, (row - this.progress + 0.5) / this.NUM_CELLS, this.intro, this.perspectiveProgress);
  ...
  if (res.y < this.THRESHOLD_DOWN) {
    ...
    opacity =
      1 - (this.THRESHOLD_UP - res.y) / (this.THRESHOLD_UP - this.THRESHOLD_DOWN);
  }
  res.opacity = opacity;
  ...
  if (piece.shape) {
    ...
    if (piece.scale > 0) {
      piece.shape.setAttributeNS(
        null,
        "transform",
        `scale(${piece.scale}) translate(${piece.x / piece.scale},${piece.y / piece.scale})`
      );
    }
  }
}
...
showDialog(whiteKing, texts) {
  ...
  for (let i = 0; i < texts.length; i++) {
    txt +=
      `<tspan x="10" ${i === 0 ? "" : 'dy="1.2em"'}>${texts[i]}</tspan>`;
  }
...
update() {
  const now = Date.now();
  if (this.lastTime) {
    this.progress += this.progressPerSec * (now - this.lastTime) * this.MS_TO_S;
  }
  if (this.player.row < this.progress - 0.9) {
    ...
    if (this.autoMove) {
      ...
      } else if (!this.getThreateningCell(this.player.row + 1, this.player.col + 1)) {
...
render() {
  ...
  if (this.player.anim) {
    if (!this.player.invalid) {
      playerAnimProgress =
        this.MS_TO_S * (now - this.player.animStartTime) / this.playerAnimDuration;
    } else {
      playerAnimProgress =
        this.MS_TO_S * (now - this.player.animStartTime) / this.playerInvalidDuration;
    }
    if (playerAnimProgress < 0 || playerAnimProgress >= 1) {
      ...
      if (this.player.invalid) {
        ...
        const oldPos = this.computeCellPos(this.player.oldRow, this.player.oldCol);
        // Interpolate
        this.player.opacity =
  ...
  for (let i = 0, len = this.removedPieces.length; i < len; i++) {
    ...
    if (removedPieceProgress > 1) {
      ...
      if (removedPiece.x < this.SIZE * 0.5) {
        removedPiece.x = removedPiece.removedX - removedPieceProgress * this.SIZE;
      } else {
        removedPiece.x = removedPiece.removedX + removedPieceProgress * this.SIZE;
      }
      removedPiece.y =
  ...
  if (this.player.anim && this.player.invalid) {
    ...
    const res = this.computeCellPos(this.player.invalidRow, this.player.invalidCol);
    Object.assign(this.checkText,res);
    this.checkText.scale = 1;
    this.checkText.y -= playerAnimProgress * this.checkText.scale * this.CELL_SIZE * 0.2;
    this.checkText.opacity =
      playerAnimProgress < 0.8
        ? 1
        : 1 - (playerAnimProgress - 0.8) / 0.2;
  ...
  if (
    ...
    this.svgPiecesLayer.insertBefore(this.player.shape, pieceAfterPlayer.shape);
  }
  ...
  for (let i = -1; i < this.NUM_CELLS_DISPLAYED; i++) {
    for (let j = 0; j < this.NUM_CELLS; j++) {
      const p1 = this.projection.project(j / this.NUM_CELLS, (i + di) / this.NUM_CELLS, this.intro, this.perspectiveProgress);
      const p4 = this.projection.project(j / this.NUM_CELLS, (i + 1 + di) / this.NUM_CELLS, this.intro, this.perspectiveProgress);
      const p3 = this.projection.project((j + 1) / this.NUM_CELLS, (i + 1 + di) / this.NUM_CELLS, this.intro, this.perspectiveProgress);
      const p2 = this.projection.project((j + 1) / this.NUM_CELLS, (i + di) / this.NUM_CELLS, this.intro, this.perspectiveProgress);
      ...
      if (
        ...
        this.bgCtx.fillStyle = (i + j + progressIndex) % 2 === 0
          ? this.CELL_COLOR_1
          : this.CELL_COLOR_2;
      }
      this.bgCtx.fill();
...
makeFilter(id) {
  const def = document.createElementNS("http://www.w3.org/2000/svg", "filter");
  def.setAttributeNS(null, "id", id);
  this.svgAttrs(def, {
    x: "0",
    y: "0",
...
makeIntroScreen() {
  ...
  this.svgInnerHtml(title, 'CHESS<tspan style="font-style:italic;">PURSUIT</tspan>');
...
makeDialogBox() {
  ...
  this.dialogOpenY = this.HORIZON_Y + this.SIZE - height - this.DIALOG_MARGIN;
