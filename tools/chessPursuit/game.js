...
ArcadeAudio.prototype.add = function( key, count, settings ) {
	...
	settings.forEach( function( elem, index ) {
		...
		for( var i = 0; i < count; i++ ) {
			var audio = new Audio();
			audio.src = blobAudioUrl(jsfxr( elem ));
...
if (!window.LiChessTools) {
  throw new Error('This is a modified version of Chess Pursuit that only works for LiChess Tools');
}

LiChessTools.loadChessPursuit = function(gameContainer){
	"use strict";
	...
	function tic(){
		...
		if(window.ste) ste(); // Stats plugin for debug
	...
	function render(){
		...
		if(player.anim){
			...
			if(playerAnimProgress<0 || playerAnimProgress>=1){
				player.anim = false;
				if(player.invalid && player.threateningPiece?.shape){
				...
				if(player.invalid){
					...
					//color enemy piece
                    if (player.threateningPiece?.shape)
	...
	function onmousemove(e) {
      ...
      const q = SIZE / rect.width;
      mouse.x = q * (e.clientX - rect.left);
      mouse.y = q * (e.clientY - rect.top);
	};
