(() => {
	class TrapValueCommandTool extends LiChessTools.Tools.ToolBase {
		...
		explorerItem = (node, value) => {
			...
			const key = parent.hash(
				components.map((k) => parent.lichess.storage.get(k) || "").join("|"),
			);
		...
		showTrapValue = async (initialPath) => {
			...
			const potency =
				(orientation != "black" ? explorerItem.white : explorerItem.black) /
				total;
			...
			for (const node of nodes) {
				...
				const isOpponentMove =
					node.ply % 2 === (orientation == "black" ? 1 : 0);
				const total =
					explorerItem.white + explorerItem.draws + explorerItem.black;
			...
			const text = trapScore
				? trans.pluralSame(
						"trapValueCommand.valueText",
						probabilityScore + "% x " + potencyScore + "% = " + trapScore + "%",
					)
				: "?";
