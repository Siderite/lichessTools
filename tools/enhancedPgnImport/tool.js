	...
	class EnhancedPgnImportTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				differentFensError:
					"cannot merge PGNs with different starting positions",
			},
			"ro-RO": {
				...
				differentFensError:
					"nu pot combina PGNuri cu pozi\u0163ii de start diferite",
			},
		};
		...
		async start() {
			...
			analysis.changePgn = parent.unwrapFunction(
				analysis.changePgn,
				"enhancedImport",
			);
			if (!value) return;
			...
			analysis.changePgn = parent.wrapFunction(analysis.changePgn, {
				...
				after: ($this, result, input, andReload) => {
					...
					if (lastError)
						parent.global.console.debug(
							"...merged only " + successfulPGNs + " PGNs",
						);
					return data;
				},
			});
