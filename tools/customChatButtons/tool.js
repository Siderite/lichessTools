(() => {
	class CustomChatButtonsTool extends LiChessTools.Tools.ToolBase {
		...
		getDefaultButtons = () => {
			return {
				start: [...this.defaultButtons.start],
				end: [...this.defaultButtons.end],
			};
		};
		getButtons = () => {
		...
		jiggle = (elem) => {
			...
			parent.global.setTimeout(
				() => elem.removeClass("lichessTools-jiggle"),
				1000,
			);
		};
		...
		deleteButton = (item) => {
			...
			const existingButtons = $(
				"section.mchat div.mchat__presets span:not([data-role])",
			).get();
		...
		setEditMode = (mode) => {
			...
			if (!this.oldPlaceholder)
				this.oldPlaceholder = chatInput.attr("placeholder");
			if (this.editMode == "edit") {
		...
		discoverChat = () => {
			...
			if (
				this.group === "start" &&
				$("span:not([data-role]):first-child", presets).text() ===
					this.getDefaultButtons().end[0].split("/")[0]
			) {
				this.group = "end";
			}
			const buttons = this.getButtons();
			...
			while (index < existingButtons.length) {
				...
				if (!button) {
					...
					index++;
					if (
						elem.text() !== button.short ||
						elem.attr("title") !== button.long
					) {
		...
		async start() {
			...
			if (!value) return;
			this.discoverChatInterval = parent.global.setInterval(
				this.discoverChat,
				1000,
			);
		}
