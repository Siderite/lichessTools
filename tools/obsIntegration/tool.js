	...
	class ObsIntegrationTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				OBSTitle: "LiChess Tools - Open Broadcaster Software",
				OBSTitleDisabled:
					"Blocat - click dreapta sau apas\u0103 O pentru a debloca",
...
`)
			...
			$('label[data-for="password"]', dialog).text(
				trans.noarg("passwordInputLabel"),
			);
			...
			$('label[data-for="options"]', dialog).text(
				trans.noarg("optionsInputLabel"),
			);
			$("#btnSave")
			...
			const scenes = await parent.comm
				.send({
					type: "getScenes",
					options: {
						url: setup.url,
						password: setup.password,
						connectOptions: setup.connectOptions,
					},
				})
				.catch((e) => {
					parent.global.console.error(e);
				});
			const template = $("<select>").append(
				$('<option value=""></option>').text(trans.noarg("defaultOptionText")),
			);
		...
		getSceneName = async (study, setup) => {
			...
			const mapping =
				setup.mappings[currentChapter[this._chapterKey]] ||
				setup.mappings[this._defaultName];
			return mapping;
		};
		...
		async start() {
			...
			if (!value) {
				parent.comm.send({ type: "disconnect" }).catch((e) => {
					parent.global.console.error("Error disconnecting:", e);
				});
				return;
			}
