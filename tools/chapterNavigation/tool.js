(() => {
	class ChapterNavigationTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.chapterNavigation":
					"Butoane navigare pentru capitole de studiu",
		...
		refreshChapterControls = () => {
			...
			if (
				!$(
					"div.study__side .study__chapters,aside.relay-tour__side .relay-games",
				).length
			)
				return;
			...
			let container = $(
				'div.study__side div[role="footer"],aside.relay-tour__side div[role="footer"]',
			);
          ...
          <button class="fbt" data-act="last" data-icon="&#x21A0;"/>
				...
				$("div.study__side,aside.relay-tour__side").addClass(
					"lichessTools-chapterControls",
				);
			}
			const chapterId = study.currentChapter()?.id;
			const index = chapterId ? list.findIndex((c) => c.id == chapterId) : 0;
			$('button[data-act="first"]', container).toggleClass(
				"disabled",
				index == 0,
			);
			$('button[data-act="prev"]', container).toggleClass(
				"disabled",
				index == 0,
			);
			$('button[data-act="random"]', container).toggleClass(
				"disabled",
				list.length == 1,
			);
			$('button[data-act="next"]', container).toggleClass(
				"disabled",
				index == list.length - 1,
			);
			$('button[data-act="last"]', container).toggleClass(
				"disabled",
				index == list.length - 1,
			);
		};
		...
		debouncedRefreshChapterControls = this.lichessTools.debounce(
		...
		async start() {
			...
			lichess.pubsub.off(
				...
				this.debouncedRefreshChapterControls,
			);
			lichess.pubsub.off(
				"lichessTools.redraw",
				this.debouncedRefreshChapterControls,
