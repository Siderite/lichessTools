(() => {
	class AddNotificationsTool extends LiChessTools.Tools.ToolBase {
				...
				for (const entry of entries) {
					if (!notifications.length) {
						const emptyDiv = $("div.empty", app)
							...
							.empty();
						notifications = $('<div class="notifications">').appendTo(emptyDiv);
					}
					let elem = $(
						"a.site_notification.lichessTools-addNotifications",
						notifications,
		...
		forcedProcessNotifications = this.lichessTools.debounce(
		...
		updateNotificationCount = (ev) => {
			...
			if (count === undefined) {
				parent.global.console.warn(
					"Could not read unread value from socket.in.notifications",
					ev,
				);
			}
