	...
	class ForkBehaviorTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.forkBehavior":
					"Comportament la mutare urm\u0103toare pentru varia\u0163iuni",
				"forkBehavior.normal": "Normal",
		...
		showPopup = async (nextMoves, nextTranspos) => {
			...
			const size =
				nextMoves.length + nextTranspos.length + (hasTranspos ? 2 : 0);
			let dlg = null;
			let selectElem = null;
			if ($("body").is(".mobile")) {
				selectElem = $("<ul>");
				...
				if (hasTranspos) {
					container = $("<ul>")
						.text(trans.noarg("transposGroupLabel"))
						.appendTo(selectElem);
				...
				selectElem = $("<select>").attr("size", size);
				let container = hasTranspos
					? $("<optgroup>")
							.attr("label", trans.noarg("movesGroupLabel"))
							.appendTo(selectElem)
					: selectElem;
				for (const move of nextMoves) {
					$("<option>")
				...
				if (hasTranspos) {
					container = $("<optgroup>")
						.attr("label", trans.noarg("transposGroupLabel"))
						.appendTo(selectElem);
					for (const move of nextTranspos) {
						$("<option>")
							.attr("value", move.uci + " " + move.path)
				...
				dlg = parent.dialog({
					html:
						selectElem[0].outerHTML +
						'<span class="dialog-actions"><button class="button submit">' +
						trans.noarg("OK") +
						"</button></span>",
				});
			}
			...
			$(dlg)
				.on("close", () =>
					lichess.analysis.explorer.setHovering(
			...
			const highlight = (ev) => {
				...
				for (let i = state.selected; i < index; i++)
					lichess.analysis.fork.next();
				for (let i = state.selected; i > index; i--)
					lichess.analysis.fork.prev();
			...
			selectElem
				.find("option")
				.on("mousedown", (ev) => {
					if (ev.button == 0 && ev.target?.selected) makeMove();
				})
				.on("dblclick", makeMove)
				.on("contextmenu", makeMove);
			selectElem.find("li").on("click", mobileMakeMove);
		...
		bindFork = () => {
			...
			if (["hybrid", "chessbase"].includes(this.options.value)) {
				...
				if (!board.prop("forkBehavior_init")) {
					this.oldWheelHandler = parent.getEventHandlers(
						$(".main-board")[0],
						"wheel",
					)[0];
				...
				$("div.analyse__fork").each((i, e) => {
					...
					e.addEventListener("click", () => (this.inForkClick = true), {
						capture: true,
					});
					e.addEventListener("click", () => (this.inForkClick = false), {
						capture: false,
					});
				});
				if (!parent.isWrappedFunction(analysis.fork.proceed, "forkBehavior")) {
					analysis.fork.proceed = parent.wrapFunction(analysis.fork.proceed, {
						...
						before: ($this, ...args) => {
							...
							const nextTranspos = parent.getNextMoves(
								analysis.node,
								false,
								true,
							);
								...
								switch (this.options.value) {
									case "hybrid":
										if (this.variationSelect != analysis.path) {
											...
											$(".analyse__tools").addClass(
												"lichessTools-forkBehavior-hybrid",
											);
		...
		async start() {
			...
			const analysis = lichess?.analysis;
			if (!analysis) return;
			analysis.fork.proceed = parent.unwrapFunction(
				analysis.fork.proceed,
				"forkBehavior",
			);
