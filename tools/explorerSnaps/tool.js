	...
	class ExplorerSnapsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				snapSettingsButtonTitle:
					"LiChess Tools - create a snap of current Explorer settings",
				allText: "all",
				sinceText: "since %s",
				untilText: "until %s",
				snapTitle:
			...
			"ro-RO": {
				...
				"options.explorerSnaps":
					"Schimb\u0103 \u00eentre diferite set\u0103ri ale Exploratorului",
				addSnapPrompt: "Titlul noului snap",
				replaceSnapPrompt: 'Suprascrie snapul "%s" ?',
				snapSettingsButtonText: "Snap!",
				snapSettingsButtonTitle:
					"LiChess Tools - creaz\u0103 un snap al set\u0103rilor curente din Explorator",
				allText: "toate",
				sinceText: "de la %s",
				untilText: "p\u00e2n\u0103 la %s",
				snapTitle:
		...
		getSnapElement = (snap) => {
			const parent = this.lichessTools;
			const $ = parent.$;
			return $('section.lichessTools-explorerSnaps button[data-act="shot"]')
				.filter((i, e) => {
		...
		highlightSnap = (snap) => {
			...
			parent.global.setTimeout(
				() => elem.removeClass("lichessTools-highlight"),
				1000,
			);
		};
		...
		getSnapBySettings = (settings) => {
			...
			let snap = this.options.snaps.find(
				(s) => stringify(s.settings) == stringify(settings),
			);
			return snap;
		};
		...
		snapSettings = () => {
			...
			if (!text) return;
			if (
				this.options.snaps.find((s) => s.text == text) &&
				!parent.global.confirm(trans.pluralSame("replaceSnapPrompt", text))
			)
				return;
			snap = {
				text: text,
		...
		saveSnaps = () => {
			...
			const lichess = parent.lichess;
			lichess.storage.set(
				"LiChessTools.explorerSnaps",
				parent.global.JSON.stringify(this.options.snaps),
			);
			parent.emitRedraw();
		};
		...
		getTitle = (settings) => {
			...
			const compress = (allArr, arr) => {
				...
				if (!resultArr.length && start == allArr[0])
					return trans.noarg("allText");
				if (start) {
					resultArr.push(end ? start + "-" + end : start + "+");
				}
			...
			const sinceText = settings.since
				? trans.pluralSame("sinceText", timeString(settings.since))
				: "";
			const untilText = settings.until
				? trans.pluralSame("untilText", timeString(settings.until))
				: "";
			return trans
				.noarg("snapTitle")
		...
		showSnapsDirect = () => {
			...
			const container = $(
				"section.explorer-box div.config >div:has(section.date)",
			);
			if (!container.length) return;
			let section = $("section.lichessTools-explorerSnaps", container);
			if (
				!this.options.enabled ||
				lichess.analysis.explorer.config.data.db() != "lichess"
			) {
				section.remove();
				return;
			}
			if (!section.length) {
				section = $('<section class="lichessTools-explorerSnaps">').appendTo(
					container,
				);
			}
		...
		async start() {
			...
			this.options = {
				enabled: value,
				snaps: parent.jsonParse(
					(_) => lichess.storage.get("LiChessTools.explorerSnaps"),
					[],
				),
			};
			for (const snap of this.options.snaps) {
			...
			explorer.config.toggleOpen = parent.unwrapFunction(
				explorer.config.toggleOpen,
				"explorerSnaps",
			);
			explorer.enabled = parent.unwrapFunction(
				explorer.enabled,
				"explorerSnaps",
			);
			...
			if (!value) {
				if (this._originalText) {
					$(".explorer-title span.lichess").text(this._originalText);
				}
				if (this._originalTitle) {
					$(".explorer-title span.lichess").text(this._originalTitle);
				}
				return;
			}
			explorer.config.toggleOpen = parent.wrapFunction(
				explorer.config.toggleOpen,
				{
					id: "explorerSnaps",
					after: ($this, result) => {
