			...
			parent.chessops = (function () {
				...
				(function () {
					...
					const k = (t) => e[d(t)] + s[l(t)],
						w = (t) => {
							if ("@" === t[1] && 4 === t.length) {
								...
								const e = m(t.slice(0, 2)),
									s = m(t.slice(2, 4));
								let r;
								if (5 === t.length && ((r = f(t[4])), !r)) return;
								if (c(e) && c(s))
									return {
										from: e,
										to: s,
						...
						b = (t) =>
							o(t)
								? `${p(t.role).toUpperCase()}@${k(t.to)}`
								: k(t.from) + k(t.to) + (t.promotion ? p(t.promotion) : ""),
						g = (t, e) =>
							"white" === t ? ("a" === e ? 2 : 6) : "a" === e ? 58 : 62,
						v = (t, e) =>
							"white" === t ? ("a" === e ? 3 : 5) : "a" === e ? 59 : 61,
						y = (t) => (
							(t =
								(858993459 & (t -= (t >>> 1) & 1431655765)) +
								((t >>> 2) & 858993459)),
							Math.imul((t + (t >>> 4)) & 252645135, 16843009) >> 24
						),
						E = (t) =>
							(((t = ((t >>> 8) & 16711935) | ((16711935 & t) << 8)) >>> 16) &
								65535) |
							((65535 & t) << 16),
						C = (t) =>
							E(
					...
					class S {
						...
						shl64(t) {
							return t >= 64
								? S.empty()
								: t >= 32
									? new S(0, this.lo << (t - 32))
									: t > 0
										? new S(
												this.lo << t,
												(this.hi << t) ^ (this.lo >>> (32 - t)),
											)
										: this;
						}
						bswap64() {
						...
						has(t) {
							return (
								0 != (t >= 32 ? this.hi & (1 << (t - 32)) : this.lo & (1 << t))
							);
						}
						set(t, e) {
						...
						with(t) {
							return t >= 32
								? new S(this.lo, this.hi | (1 << (t - 32)))
								: new S(this.lo | (1 << t), this.hi);
						}
						without(t) {
							return t >= 32
								? new S(this.lo, this.hi & ~(1 << (t - 32)))
						...
						withoutFirst() {
							return 0 !== this.lo
								? new S(this.lo & (this.lo - 1), this.hi)
								: new S(0, this.hi & (this.hi - 1));
						}
						moreThanOne() {
							return (
								(0 !== this.hi && 0 !== this.lo) ||
					...
					const q = (t, e) => {
						...
						K = (t, e) =>
							((t, e) => F(S.fromSquare(t), _[t], e))(t, e).xor(
								((t, e) => {
									const s = B[t];
									let r = e.intersect(s),
										i = r.rbit64();
									return (
										(r = r.minus64(S.fromSquare(t))),
						...
						j = (t, e) =>
							$(t, e)
								.intersect(S.full().shl64(t).xor(S.full().shl64(e)))
								.withoutFirst();
					class T {
						constructor() {}
						static default() {
						...
						clear() {
							(this.occupied = S.empty()), (this.promoted = S.empty());
							for (const t of r) this[t] = S.empty();
							for (const t of i) this[t] = S.empty();
						}
						clone() {
							const t = new T();
							(t.occupied = this.occupied), (t.promoted = this.promoted);
							for (const e of r) t[e] = this[e];
							for (const e of i) t[e] = this[e];
							return t;
						}
						getColor(t) {
							return this.white.has(t)
								? "white"
								: this.black.has(t)
									? "black"
									: void 0;
						...
						getRole(t) {
							for (const e of i) if (this[e].has(t)) return e;
						}
						get(t) {
						...
						take(t) {
							...
							return (
								e &&
									((this.occupied = this.occupied.without(t)),
									(this[e.color] = this[e.color].without(t)),
									(this[e.role] = this[e.role].without(t)),
									e.promoted && (this.promoted = this.promoted.without(t))),
								e
							);
						}
						set(t, e) {
							const s = this.take(t);
							return (
								(this.occupied = this.occupied.with(t)),
						...
						*[Symbol.iterator]() {
							for (const t of this.occupied) yield [t, this.get(t)];
						}
						pieces(t, e) {
					...
					class L {
						...
						static fromBoard(t, e) {
							const s = new L();
							for (const r of i) s[r] = t.pieces(e, r).size();
							return s;
						}
						clone() {
							const t = new L();
						...
						add(t) {
							const e = new L();
							for (const s of i) e[s] = this[s] + t[s];
							return e;
						}
						nonEmpty() {
					...
					class Z {
						...
						clone() {
							const t = new Z();
							return (
								(t.unmovedRooks = this.unmovedRooks),
								(t.rook = {
									white: {
										a: this.rook.white.a,
										h: this.rook.white.h,
					...
					class J {
						...
						kingAttackers(t, e, s) {
							return ((t, e, s, r) =>
								s[e].intersect(
									K(t, r)
										.intersect(s.rooksAndQueens())
										.union(V(t, r).intersect(s.bishopsAndQueens()))
										.union(M(t).intersect(s.knight))
										.union(A(t).intersect(s.king))
										.union(P(u(e), t).intersect(s.pawn)),
								))(t, e, this.board, s);
						}
						playCaptureAt(t, e) {
							(this.halfmoves = 0),
								...
								this.pockets &&
									this.pockets[u(e.color)][e.promoted ? "pawn" : e.role]++;
						}
						ctx() {
							...
							const s = K(e, S.empty())
								...
								.union(V(e, S.empty()).intersect(this.board.bishopsAndQueens()))
								.intersect(this.board[u(this.turn)]);
							let r = S.empty();
							for (const t of s) {
						...
						clone() {
							...
							return (
								...
								(s.remainingChecks =
									null === (e = this.remainingChecks) || void 0 === e
										? void 0
										: e.clone()),
						...
						validate(e) {
							if (this.board.occupied.isEmpty()) return t.err(new W(Y.Empty));
							if (2 !== this.board.king.size()) return t.err(new W(Y.Kings));
							...
							return c(s)
								? this.kingAttackers(
									...
									? t.err(new W(Y.OppositeCheck))
									: S.backranks().intersects(this.board.pawn)
										? t.err(new W(Y.PawnsOnBackrank))
										...
										if (
											...
											(s.first() !== r &&
												this.kingAttackers(
													e,
													u(this.turn),
													this.board.occupied.without(r).with(i),
												).nonEmpty())
										)
											return t.err(new W(Y.ImpossibleCheck));
									} else if (
										s.size() > 2 ||
										(2 === s.size() && $(s.first(), s.last()).has(e))
									)
										return t.err(new W(Y.ImpossibleCheck));
							}
							return t.ok(void 0);
						...
						dests(t, e) {
							if ((e = e || this.ctx()).variantEnd) return S.empty();
							const s = this.board.get(t);
							if (!s || s.color !== this.turn) return S.empty();
							let r, i;
							if ("pawn" === s.role) {
						...
						hasInsufficientMaterial(t) {
							...
							if (this.board[t].intersects(this.board.bishop)) {
								return (
									(!this.board.bishop.intersects(S.darkSquares()) ||
										!this.board.bishop.intersects(S.lightSquares())) &&
						...
						hasDests(t) {
							...
							for (const e of this.board[this.turn])
								if (this.dests(e, t).nonEmpty()) return !0;
							return this.dropDests(t).nonEmpty();
						}
						isLegal(t, e) {
							if (o(t))
								return (
									!(!this.pockets || this.pockets[this.turn][t.role] <= 0) &&
									("pawn" !== t.role || !S.backranks().has(t.to)) &&
									this.dropDests(e).has(t.to)
								);
							{
								if ("pawn" === t.promotion) return !1;
								if ("king" === t.promotion && "antichess" !== this.rules)
									return !1;
								if (
									!!t.promotion !==
									(this.board.pawn.has(t.from) && S.backranks().has(t.to))
								)
									return !1;
						...
						isEnd(t) {
							return (
								!!(t ? t.variantEnd : this.isVariantEnd()) ||
						...
						allDests(t) {
							t = t || this.ctx();
							const e = new Map();
							if (t.variantEnd) return e;
							for (const s of this.board[this.turn]) e.set(s, this.dests(s, t));
							return e;
						}
						play(t) {
							const e = this.turn,
							...
							if (
								...
								this.board.set(t.to, {
									...
									this.pockets && this.pockets[e][t.role]--,
									"pawn" === t.role && (this.halfmoves = 0);
							else {
								...
								if ("pawn" === i.role) {
									(this.halfmoves = 0),
										t.to === s &&
											(n = this.board.take(t.to + ("white" === e ? -8 : 8)));
									const r = t.from - t.to;
									16 === Math.abs(r) &&
										8 <= t.from &&
										t.from <= 55 &&
										(this.epSquare = (t.from + t.to) >> 1),
										t.promotion &&
											((i.role = t.promotion), (i.promoted = !!this.pockets));
								} else if ("rook" === i.role) this.castles.discardRook(t.from);
								else if ("king" === i.role) {
									if (r) {
							...
							this.remainingChecks &&
								this.isCheck() &&
								(this.remainingChecks[e] = Math.max(
									this.remainingChecks[e] - 1,
									0,
								));
						}
					}
					class X extends J {
						...
						static fromSetup(t, e) {
							const s = new this();
							return s.setupUnchecked(t), s.validate(e).map((t) => s);
						}
						clone() {
							return super.clone();
						}
					...
					const tt = (t, e) => {
							...
							const s = "white" === t.turn ? 5 : 2,
								r = "white" === t.turn ? 8 : -8;
							if (l(e) !== s) return;
							if (t.board.occupied.has(e + r)) return;
							const i = e - r;
							return t.board.pawn.has(i) && t.board[u(t.turn)].has(i)
								? e
								: void 0;
						},
						et = (t) => {
							if (!c(t.epSquare)) return;
							const e = t.ctx(),
								s = t.board
									.pieces(t.turn, "pawn")
									.intersect(P(u(t.turn), t.epSquare));
							for (const r of s)
								if (t.dests(r, e).has(t.epSquare)) return t.epSquare;
						},
						st = (t, e, s) => {
							if (!c(t.epSquare)) return !1;
							if (!P(t.turn, e).has(t.epSquare)) return !1;
							if (!c(s.king)) return !0;
						...
						rt = (t, e, s) => {
							if (!c(s.king) || s.checkers.nonEmpty()) return S.empty();
							const r = t.castles.rook[t.turn][e];
							if (!c(r)) return S.empty();
							...
							return t.kingAttackers(i, u(t.turn), h).nonEmpty()
								? S.empty()
								: S.fromSquare(r);
						},
						it = (t, e, s) => {
							if (s.variantEnd) return S.empty();
							const r = t.board.get(e);
							if (!r || r.color !== t.turn) return S.empty();
							let i = D(r, e, t.board.occupied);
							if ("pawn" === r.role) {
								let s = t.board[u(t.turn)];
								c(t.epSquare) && (s = s.with(t.epSquare)), (i = i.intersect(s));
								const r = "white" === t.turn ? 8 : -8,
									n = e + r;
								if (0 <= n && n < 64 && !t.board.occupied.has(n)) {
									i = i.with(n);
									const s = n + r;
									("white" === t.turn ? e < 16 : e >= 48) &&
										!t.board.occupied.has(s) &&
										(i = i.with(s));
								}
								return i;
							}
							return (
								(i = i.diff(t.board[t.turn])),
						...
						nt = (t, e) => {
							if (o(e)) return;
							const s = e.to - e.from;
							return (2 === Math.abs(s) || t.board[t.turn].has(e.to)) &&
								t.board.king.has(e.from)
								? s > 0
									? "h"
									: "a"
								: void 0;
						...
						at = (t, e) => {
							const s =
								Math.max(t.pieces(e, "queen").size() - 1, 0) +
								Math.max(t.pieces(e, "rook").size() - 2, 0) +
								Math.max(t.pieces(e, "knight").size() - 2, 0) +
								Math.max(
									t.pieces(e, "bishop").intersect(S.lightSquares()).size() - 1,
									0,
								) +
								Math.max(
									t.pieces(e, "bishop").intersect(S.darkSquares()).size() - 1,
									0,
								);
						...
						chessgroundDests: (t, e) => {
							...
							for (const [i, n] of t.allDests(r))
								if (n.nonEmpty()) {
									...
									(null == e ? void 0 : e.chess960) ||
										i !== r.king ||
										4 !== d(i) ||
										(n.has(0) ? t.push("c1") : n.has(56) && t.push("c8"),
										n.has(7) ? t.push("g1") : n.has(63) && t.push("g8")),
										s.set(k(i), t);
								}
							return s;
						},
						chessgroundMove: (t) => (o(t) ? [k(t.to)] : [k(t.from), k(t.to)]),
						scalachessCharPair: (t) =>
							o(t)
								? String.fromCharCode(
										35 + t.to,
										139 +
											["queen", "rook", "bishop", "knight", "pawn"].indexOf(
												t.role,
											),
									)
								: String.fromCharCode(
					...
					const wt = (t) => (/^\d{1,4}$/.test(t) ? parseInt(t, 10) : void 0),
							...
							for (let n = 0; n < e.length; n++) {
								...
								if ("/" === o && 8 === i) (i = 0), r--;
									...
									if (a > 0) i += a;
										...
										if (i >= 8 || r < 0) return t.err(new kt(mt.Board));
										const a = i + 8 * r,
											h = bt(o);
										if (!h) return t.err(new kt(mt.Board));
						...
						vt = (e) => {
							if (e.length > 64) return t.err(new kt(mt.Pockets));
							const s = Q.empty();
							for (const r of e) {
								const e = bt(r);
								if (!e) return t.err(new kt(mt.Pockets));
								s[e.color][e.role]++;
							}
							return t.ok(s);
						},
						yt = (e, s) => {
							...
							for (const r of s) {
								...
								let a;
								if ("q" === s) a = o;
								else if ("k" === s) a = o.reversed();
								else {
									if (!("a" <= s && s <= "h"))
										return t.err(new kt(mt.Castling));
									a = S.fromFile(s.charCodeAt(0) - "a".charCodeAt(0)).intersect(
										o,
									);
								}
								for (const t of a) {
									if (e.king.has(t)) break;
									if (e.rook.has(t)) {
										i = i.with(t);
										break;
									}
							...
							return r.some((t) => S.backrank(t).intersect(i).size() > 2)
								? t.err(new kt(mt.Castling))
								: t.ok(i);
						},
						Et = (e) => {
							const s = e.split("+");
							if (3 === s.length && "" === s[0]) {
								const e = wt(s[1]),
									r = wt(s[2]);
								return !c(e) || e > 3 || !c(r) || r > 3
									? t.err(new kt(mt.RemainingChecks))
									: t.ok(new H(3 - e, 3 - r));
							}
							if (2 === s.length) {
								const e = wt(s[0]),
						...
						Ct = (e) => {
							...
							if (r.endsWith("]")) {
								const e = r.indexOf("[");
								if (-1 === e) return t.err(new kt(mt.Fen));
								(i = gt(r.slice(0, e))), (o = vt(r.slice(e + 1, -1)));
							} else {
								const t = ((t, e, s) => {
									let r = t.indexOf(e);
									for (; s-- > 0 && -1 !== r; ) r = t.indexOf(e, r + e.length);
									return r;
								})(r, "/", 7);
								-1 === t
									? (i = gt(r))
									: ((i = gt(r.slice(0, t))), (o = vt(r.slice(t + 1))));
							}
							const a = s.shift();
							if (c(a) && "w" !== a) {
								if ("b" !== a) return t.err(new kt(mt.Turn));
								n = "black";
							} else n = "white";
							return i.chain((e) => {
								const r = s.shift(),
								...
								const d = c(l) ? wt(l) : 0;
								if (!c(d)) return t.err(new kt(mt.Halfmoves));
								const p = s.shift(),
									f = c(p) ? wt(p) : 1;
								if (!c(f)) return t.err(new kt(mt.Fullmoves));
								const k = s.shift();
								let w = t.ok(void 0);
								if (c(k)) {
									if (c(u)) return t.err(new kt(mt.RemainingChecks));
									w = Et(k);
								} else c(u) && (w = u);
								return s.length > 0
									? t.err(new kt(mt.Fen))
						...
						At = (t, e) =>
							[
								qt(t.board) + (t.pockets ? `[${Ot(t.pockets)}]` : ""),
								t.turn[0],
								xt(t.board, t.unmovedRooks),
								c(t.epSquare) ? k(t.epSquare) : "-",
								...(t.remainingChecks ? [zt(t.remainingChecks)] : []),
								...((null == e ? void 0 : e.epd)
									? []
									: [
					...
					var Mt = Object.freeze({
						...
						parsePiece: (t) => {
							...
							const e = bt(t[0]);
							if (e) {
								if (2 === t.length && "~" === t[1]) e.promoted = !0;
						...
						Bt = (t, e, s = !1) => {
							...
							}
							{
								...
								for (const [i, o] of t.allDests(n)) {
									const n =
										l(i) === ("white" === t.turn ? 6 : 1) && t.board.pawn.has(i)
											? r
											: [void 0];
									for (const r of o)
										for (const o of n) {
											...
											n.play(h);
											const c = Bt(n, e - 1, !1);
											s && console.log(b(h), c), (a += c);
										}
								}
								if (t.pockets)
									for (const r of i)
										if (t.pockets[t.turn][r] > 0)
											for (const i of "pawn" === r
												? o.diff(S.backranks())
												: o) {
												const n = t.clone(),
													o = {
							...
							for (let s = 7; s >= 0; s--)
								for (let r = 0; r < 8; r++) {
									const i = r + 8 * s,
										n = t.get(i),
										o = n ? Pt(n) : ".";
									e.push(o), e.push(r < 7 ? (o.length < 2 ? " " : "") : "\n");
								}
							return e.join("");
						},
						square: _t,
					...
					const Nt = (t, r) => {
							let i = "";
							if (o(r))
								"pawn" !== r.role && (i = p(r.role).toUpperCase()),
									(i += "@" + k(r.to));
							else {
								...
								if (
									"king" !== n ||
									(!t.board[t.turn].has(r.to) && 2 !== Math.abs(r.to - r.from))
								) {
									const o =
										t.board.occupied.has(r.to) ||
										("pawn" === n && d(r.from) !== d(r.to));
									if ("pawn" !== n) {
										let o;
										if (
											((i = p(n).toUpperCase()),
											(o =
												"king" === n
													? A(r.to).intersect(t.board.king)
													: "queen" === n
														? U(r.to, t.board.occupied).intersect(t.board.queen)
														: "rook" === n
															? K(r.to, t.board.occupied).intersect(
																	t.board.rook,
																)
															: "bishop" === n
																? V(r.to, t.board.occupied).intersect(
																		t.board.bishop,
																	)
																: M(r.to).intersect(t.board.knight)),
											(o = o.intersect(t.board[t.turn]).without(r.from)),
											o.nonEmpty())
										) {
											const n = t.ctx();
											for (const e of o)
											...
											if (o.nonEmpty()) {
												...
												o.intersects(S.fromFile(d(r.from)))
													? (t = !0)
													: (n = !0),
													n && (i += e[d(r.from)]),
													t && (i += s[l(r.from)]);
						...
						Ft = (t, e) => {
							...
							return (
								t.play(e),
								(null === (s = t.outcome()) || void 0 === s ? void 0 : s.winner)
									? r + "#"
									: t.isCheck()
										? r + "+"
										: r
						...
						makeSanVariation: (t, e) => {
							...
							for (let i = 0; i < e.length; i++) {
								0 !== i && r.push(" "),
									"white" === t.turn
										? r.push(t.fullmoves, ". ")
										: 0 === i && r.push(t.fullmoves, "... ");
								const n = Nt(t, e[i]);
								if ((t.play(e[i]), r.push(n), "--" === n)) return r.join("");
								i === e.length - 1 &&
								(null === (s = t.outcome()) || void 0 === s ? void 0 : s.winner)
									? r.push("#")
									: t.isCheck() && r.push("+");
						...
						parseSan: (t, e) => {
							...
							if (!r) {
								...
								if (
									...
									if (!c(s.king) || !c(e) || !t.dests(s.king, s).has(e)) return;
									return {
										from: s.king,
										to: e,
									};
							...
							const i = r[1] ? f(r[1]) : "pawn",
								n = m(r[4]),
								o = r[5] ? f(r[5]) : void 0;
							if (!!o !== ("pawn" === i && S.backranks().has(n))) return;
							if ("king" === o && "antichess" !== t.rules) return;
							let a = t.board.pieces(t.turn, i);
							"pawn" !== i || r[2]
								? r[2] &&
									(a = a.intersect(
										S.fromFile(r[2].charCodeAt(0) - "a".charCodeAt(0)),
									))
								: (a = a.intersect(S.fromFile(d(n)))),
								r[3] &&
									(a = a.intersect(
										S.fromRank(r[3].charCodeAt(0) - "1".charCodeAt(0)),
									));
					...
					const Kt = (t, e) => {
						const s = T.empty();
						(s.occupied = e(t.occupied)), (s.promoted = e(t.promoted));
						for (const i of r) s[i] = e(t[i]);
						for (const r of i) s[r] = e(t[r]);
						return s;
					};
					var Ut = Object.freeze({
						...
						flipHorizontal: (t) => {
							...
							return (t = (t = (t = t
								.shr64(1)
								.intersect(e)
								.union(t.intersect(e).shl64(1)))
								.shr64(2)
								.intersect(s)
								.union(t.intersect(s).shl64(2)))
								.shr64(4)
								.intersect(r)
								.union(t.intersect(r).shl64(4)));
						},
						flipDiagonal: (t) => {
							let e = t.xor(t.shl64(28)).intersect(new S(0, 252645135));
							return (
								(t = t.xor(e.xor(e.shr64(28)))),
								(e = t.xor(t.shl64(14)).intersect(new S(858980352, 858980352))),
								(t = t.xor(e.xor(e.shr64(14)))),
								(e = t
									.xor(t.shl64(7))
									.intersect(new S(1426085120, 1426085120))),
						...
						transformSetup: (t, e) => {
							...
							return {
								board: Kt(t.board, e),
								pockets:
									null === (s = t.pockets) || void 0 === s ? void 0 : s.clone(),
								turn: t.turn,
								unmovedRooks: e(t.unmovedRooks),
								epSquare: c(t.epSquare)
									? e(S.fromSquare(t.epSquare)).first()
									: void 0,
								remainingChecks:
									null === (r = t.remainingChecks) || void 0 === r
										? void 0
										: r.clone(),
					...
					class Dt extends J {
						...
						reset() {
							super.reset(), (this.pockets = Q.empty());
						}
						setupUnchecked(t) {
							super.setupUnchecked(t),
								(this.board.promoted = t.board.promoted
									.intersect(t.board.occupied)
									.diff(t.board.king)
									.diff(t.board.pawn)),
						...
						validate(e) {
							return super.validate(e).chain((e) => {
								var s, r;
								return (
									null === (s = this.pockets) || void 0 === s
										? void 0
										: s.count("king")
								)
									? t.err(new W(Y.Kings))
									: ((null === (r = this.pockets) || void 0 === r
												? void 0
												: r.size()) || 0) +
												this.board.occupied.size() >
											64
										? t.err(new W(Y.Variant))
										: t.ok(void 0);
							});
						}
						hasInsufficientMaterial(t) {
							return this.pockets
								...
								: super.hasInsufficientMaterial(t);
						}
						dropDests(t) {
							var e, s;
							const r = this.board.occupied
								.complement()
								.intersect(
									(
										null === (e = this.pockets) || void 0 === e
											? void 0
											: e[this.turn].hasNonPawns()
									)
										? S.full()
										: (
													null === (s = this.pockets) || void 0 === s
														? void 0
														: s[this.turn].hasPawns()
												)
											? S.backranks().complement()
											: S.empty(),
								);
					...
					class $t extends J {
						...
						validate(e) {
							if (this.board.occupied.isEmpty()) return t.err(new W(Y.Empty));
							if (this.board.king.size() > 2) return t.err(new W(Y.Kings));
							const s = this.board.kingOf(u(this.turn));
							return c(s)
								? this.kingAttackers(
										s,
						...
						kingAttackers(t, e, s) {
							...
							return r.isEmpty() || A(t).intersects(r)
								? S.empty()
								: super.kingAttackers(t, e, s);
						}
						playCaptureAt(t, e) {
							super.playCaptureAt(t, e), this.board.take(t);
							for (const e of A(t)
								.intersect(this.board.occupied)
								.diff(this.board.pawn)) {
								const t = this.board.take(e);
								"rook" === (null == t ? void 0 : t.role) &&
									this.castles.discardRook(e),
									"king" === (null == t ? void 0 : t.role) &&
										this.castles.discardColor(t.color);
							}
						}
						hasInsufficientMaterial(t) {
							if (this.board.pieces(u(t), "king").isEmpty()) return !1;
							if (this.board[t].diff(this.board.king).isEmpty()) return !0;
							if (this.board[u(t)].diff(this.board.king).nonEmpty()) {
								if (
									this.board.occupied.equals(
										this.board.bishop.union(this.board.king),
									)
								) {
									if (
										!this.board.bishop
											.intersect(this.board.white)
											.intersects(S.darkSquares())
									)
										return !this.board.bishop
											.intersect(this.board.black)
											.intersects(S.lightSquares());
									if (
										!this.board.bishop
											.intersect(this.board.white)
											.intersects(S.lightSquares())
						...
						dests(t, e) {
							...
							for (const r of it(this, t, e)) {
								...
								!c(i) ||
									(c(e.board.kingOf(e.turn)) &&
										!e.kingAttackers(i, e.turn, e.board.occupied).isEmpty()) ||
									(s = s.with(r));
							}
							return s;
						}
					...
					class jt extends J {
						...
						reset() {
							super.reset(), (this.castles = Z.empty());
						}
						setupUnchecked(t) {
							super.setupUnchecked(t), (this.castles = Z.empty());
						}
						static default() {
							const t = new this();
							return t.reset(), t;
						...
						ctx() {
							...
							if (
								c(this.epSquare) &&
								P(u(this.turn), this.epSquare).intersects(
						...
						dests(t, e) {
							...
							return s.intersect(
								e.mustCapture
									? c(this.epSquare) && "pawn" === this.board.getRole(t)
										? r.with(this.epSquare)
										: r
									: S.full(),
							);
						}
						hasInsufficientMaterial(t) {
							if (this.board[t].isEmpty()) return !1;
							if (this.board[u(t)].isEmpty()) return !0;
							...
							return (
								!(
									!this.board.occupied.equals(this.board.knight) ||
									2 !== this.board.occupied.size()
								) &&
								(this.board.white.intersects(S.lightSquares()) !==
									this.board.black.intersects(S.darkSquares())) !=
									(this.turn === t)
							);
						}
						isVariantEnd() {
					...
					class Lt extends J {
						...
						reset() {
							super.reset(), (this.remainingChecks = H.default());
						}
						setupUnchecked(t) {
							var e;
							super.setupUnchecked(t),
								(this.remainingChecks =
									(null === (e = t.remainingChecks) || void 0 === e
										? void 0
										: e.clone()) || H.default());
						}
						static default() {
						...
						isVariantEnd() {
							return (
								!!this.remainingChecks &&
								(this.remainingChecks.white <= 0 ||
									this.remainingChecks.black <= 0)
							);
						}
						variantOutcome(t) {
					...
					class Qt extends J {
						...
						validate(e) {
							return this.isCheck() || this.board.pawn.nonEmpty()
								? t.err(new W(Y.Variant))
								: super.validate(e);
						}
						dests(t, e) {
							if (t === (e = e || this.ctx()).king) return super.dests(t, e);
							let s = S.empty();
						...
						isVariantEnd() {
							...
							if (c(s)) {
								...
								for (const r of A(s).intersect(t).diff(this.board.black))
									if (this.kingAttackers(r, "white", e).isEmpty()) return !1;
							}
							return !0;
						}
						variantOutcome(t) {
							if (t ? !t.variantEnd : !this.isVariantEnd()) return;
					...
					class Ht extends J {
						...
						reset() {
							(this.board = (() => {
								const t = T.empty();
								return (
									(t.occupied = new S(4294967295, 4294901862)),
									(t.promoted = S.empty()),
						...
						validate(e) {
							if (this.board.occupied.isEmpty()) return t.err(new W(Y.Empty));
							if (1 !== this.board.king.size()) return t.err(new W(Y.Kings));
							const s = this.board.kingOf(u(this.turn));
							if (
								c(s) &&
								this.kingAttackers(s, this.turn, this.board.occupied).nonEmpty()
							)
								return t.err(new W(Y.OppositeCheck));
							for (const e of r) {
								const s = this.board.pieces(e, "king").isEmpty()
									? S.backrank(u(e))
									: S.backranks();
					...
					var Wt = Object.freeze({
						...
						equalsIgnoreMoves: (t, e) => {
							...
							return (
								t.rules === e.rules &&
								((t, e) =>
									t.white.equals(e.white) &&
									t.promoted.equals(e.promoted) &&
									i.every((s) => t[s].equals(e[s])))(t.board, e.board) &&
								((e.pockets &&
									(null === (s = t.pockets) || void 0 === s
										? void 0
										: s.equals(e.pockets))) ||
									(!t.pockets && !e.pockets)) &&
								t.turn === e.turn &&
								t.castles.unmovedRooks.equals(e.castles.unmovedRooks) &&
								et(t) === et(e) &&
								((e.remainingChecks &&
									(null === (r = t.remainingChecks) || void 0 === r
										? void 0
										: r.equals(e.remainingChecks))) ||
							...
							switch (t.rules) {
								...
								case "crazyhouse": {
									...
									return (
										r.size() +
											t.board.pawn.size() +
											((null === (e = t.pockets) || void 0 === e
												? void 0
												: e.count("pawn")) || 0) <=
											16 &&
										t.board.knight.diff(r).size() +
											((null === (s = t.pockets) || void 0 === s
												? void 0
												: s.count("knight")) || 0) <=
											4 &&
										t.board.bishop.diff(r).size() +
											((null === (i = t.pockets) || void 0 === i
												? void 0
												: i.count("bishop")) || 0) <=
											4 &&
										t.board.rook.diff(r).size() +
											((null === (n = t.pockets) || void 0 === n
												? void 0
												: n.count("rook")) || 0) <=
											4 &&
										t.board.queen.diff(r).size() +
											((null === (o = t.pockets) || void 0 === o
												? void 0
												: o.count("queen")) || 0) <=
											2
									);
								...
								case "horde":
									return r.every((e) =>
										t.board.pieces(e, "king").nonEmpty()
											? at(t.board, e)
											: t.board[e].size() <= 36,
									);
								case "racingkings":
									return r.every(
										(e) =>
											t.board.pieces(e, "knight").size() <= 2 &&
											t.board.pieces(e, "bishop").size() <= 2 &&
											t.board.pieces(e, "rook").size() <= 2 &&
											t.board.pieces(e, "queen").size() <= 1,
									);
							}
						},
					});
					...
					class ce {
								...
								try {
									...
									this.consumeBudget(t.length - s),
										...
										(null == e ? void 0 : e.stream) ||
											(this.handleLine(), this.emit(void 0));
								} catch (t) {
									this.emit(t);
								}
						}
							...
							t: for (;;)
								switch (this.state) {
									...
									case 3: {
										if (t) {
											if (ae(e)) return;
											if (oe(e)) return this.emit(void 0);
										}
										const s =
										...
										for (; (r = s.exec(e)); ) {
											...
											if (i.startsWith("$"))
											...
											else if ("!" === i) this.handleNag(1);
											else if ("?" === i) this.handleNag(2);
											else if ("!!" === i) this.handleNag(3);
											else if ("??" === i) this.handleNag(4);
											else if ("!?" === i) this.handleNag(5);
											else if ("?!" === i) this.handleNag(6);
											else if (
												"1-0" === i ||
												"0-1" === i ||
												"1/2-1/2" === i ||
												...
												1 === this.stack.length &&
													"*" !== i &&
													this.game.headers.set("Result", i);
											else if ("(" === i)
												...
												if ("{" === i) {
													const t = s.lastIndex,
														r = " " === e[t] ? t + 1 : t;
													(e = e.slice(r)), (this.state = 4);
													continue t;
												}
												this.consumeBudget(100),
													"Z0" === i || "0000" === i || "@@@@" === i
														? (i = "--")
														: i.startsWith("0") && (i = i.replace(/0/g, "O")),
									...
									case 4: {
										const s = e.indexOf("}");
										if (-1 === s) return void this.commentBuf.push(e);
										{
						...
						handleNag(t) {
							...
							s.node &&
								((e = s.node.data).nags || (e.nags = []),
								s.node.data.nags.push(t));
						}
						handleComment() {
							var t, e;
							this.consumeBudget(100);
							...
							(this.commentBuf = []),
								s.node
									? ((t = s.node.data).comments || (t.comments = []),
										s.node.data.comments.push(r))
									: s.root
										? ((e = this.game).comments || (e.comments = []),
											this.game.comments.push(r))
										: (s.startingComments || (s.startingComments = []),
											s.startingComments.push(r));
						}
						emit(t) {
							if ((4 === this.state && this.handleComment(), t))
								return this.emitGame(this.game, t);
							this.found && this.emitGame(this.game, void 0), this.resetGame();
						}
					}
					const ue = (t) => {
					...
					var we = Object.freeze({
						...
						transform: (t, e, s) => {
							...
							for (; (n = i.pop()); )
								for (let t = 0; t < n.before.children.length; t++) {
									const e =
											t < n.before.children.length - 1 ? n.ctx.clone() : n.ctx,
						...
						walk: (t, e, s) => {
							...
							for (; (i = r.pop()); )
								for (let t = 0; t < i.node.children.length; t++) {
									const e =
											t < i.node.children.length - 1 ? i.ctx.clone() : i.ctx,
										n = i.node.children[t];
									!1 !== s(e, n.data, t) &&
										r.push({
											node: n,
						...
						makePgn: (t) => {
							...
							for (const e of t.comments || []) s.push("{", ie(e), "}");
							const r = t.headers.get("FEN"),
								i = r
									? Ct(r).unwrap(
											(t) =>
												2 * (t.fullmoves - 1) + ("white" === t.turn ? 0 : 1),
											(t) => 0,
										)
									: 0,
								n = [];
							...
							return (
								s.push(ee(se(t.headers.get("Result")))),
								e.push(s.join(" "), "\n"),
								e.join("")
							);
						},
						...
						parsePgn: (t, e = ne) => {
							const s = [];
							return new ce((t) => s.push(t), e, NaN).parse(t), s;
						},
						parseVariant: ue,
						makeVariant: le,
						startingPosition: (e, s) => {
							const r = ue(e.get("Variant"));
							if (!r) return t.err(new W(Y.Variant));
							const i = e.get("FEN");
						...
						makeComment: (t) => {
							...
							return (
								r.length && e.push(`[%cal ${r.join(",")}]`),
								t.evaluation &&
									e.push(
										`[%eval ${((t) => {
											const e = de(t) ? "#" + t.mate : t.pawns.toFixed(2);
											return c(t.depth) ? e + "," + t.depth : e;
										})(t.evaluation)}]`,
									),
							...
							return {
								text: t
									...
									.replace(
										...
										(t, e) => {
											for (const t of e.split(",")) i.push(ke(t));
											return "  ";
										},
									)
									.replace(
