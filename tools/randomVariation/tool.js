	...
	class RandomVariationTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.randomVariationDepth":
					"Ad\u00E2ncimea probabilit\u0103\u0163ii mut\u0103rii urm\u0103toare",
		...
		populatePercent = (nodes, isInteractive, depth) => {
			...
			if (total > 100) {
				console.warn(
					"Variations are marked with values of prc which total more than 100",
				);
			}
			if (defaultPrc.length) {
				...
				for (const node of defaultPrc) {
					const descendants = getGamebookDescendants(
						node,
						depth,
						1,
						isInteractive,
					...
					const weight =
						1 + descendants.filter((n) => n.children?.length > 1).length;
		...
		getNextMoves = (node, noTranspositions, noMoves) => {
			...
			if (
				noTranspositions ||
				!parent.transpositionBehavior?.consideredVariations ||
				!node.transposition
			)
				return arr;
			let transpositions = node.transposition.filter((n) => n !== node);
			if (
				parent.transpositionBehavior?.excludeSameLine &&
				node.path !== undefined
			) {
		...
		async start() {
			...
			if (Number.isNaN(this.depth))
				this.depth = this.preferences.filter(
					(p) => p.name == "randomVariationDepth",
				)[0].defaultValue;
