	...
	class ExtendedInteractiveLessonTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"extendedInteractiveLesson.returnToPreview":
					"Play again from where you entered Preview",
				...
				extendedInteractiveLessonLong:
					"Extended Interactive lesson - LiChess Tools",
				...
				addDeviationTitle:
					"LiChess Tools - explain why moves from here not in the PGN are wrong",
				"options.extendedInteractiveLessonFlow":
				...
				extendedInteractiveOptionsTitle:
					"LiChess Tools - interactive lesson preferences",
				giveUpButtonText: "Give up",
				giveUpButtonTitle: "Abandons the interactive run",
				giveUpConfirmation:
					"Are you sure you want to abandon the interactive run?",
			},
			"ro-RO": {
				"options.study": "Studiu",
				"options.extendedInteractiveLesson": "Lec\u0163ii interactive extinse",
				"extendedInteractiveLesson.extendedInteractive":
					"Joac\u0103 toate varia\u0163iunile",
				"extendedInteractiveLesson.showFinalScore": "Arat\u0103 scorul final",
				"extendedInteractiveLesson.alwaysShowScore":
					"Arat\u0103 scorul tot timpul",
				"extendedInteractiveLesson.returnToPreview":
					"Joac\u0103 din nou de unde ai intrat \u00een Preview",
				"extendedInteractiveLesson.fastInteractive":
					"Interac\u0163iune rapid\u0103",
				"extendedInteractiveLesson.giveUpButton": "Buton renun\u0163are",
				extendedInteractiveLesson: "Lec\u0163ie Interactiv\u0103 extins\u0103",
				extendedInteractiveLessonLong:
					"Lec\u0163ie Interactiv\u0103 extins\u0103 - LiChess Tools",
				...
				addDeviationText:
					"Explic\u0103 de ce alte mut\u0103ri sunt gre\u015Fite",
				addDeviationTitle:
					"LiChess Tools - explic\u0103 de ce mut\u0103ri de aici lips\u0103 din PGN sunt gre\u015Fite",
				"options.extendedInteractiveLessonFlow":
					"Cursul lec\u0163iilor interactive extinse",
				"extendedInteractiveLessonFlow.sequential": "Secven\u0163ial",
				"extendedInteractiveLessonFlow.spacedRepetition":
					"Repeti\u0163ie distan\u0163at\u0103",
				...
				resetButtonTitle:
					"LiChess Tools - resetare progres \u00een varia\u0163uni",
				progressTitle: "LiChess Tools - %s varia\u0163uni",
				extendedInteractiveOptionsTitle:
					"LiChess Tools - preferin\u0163e lec\u0163ie interactiv\u0103",
				giveUpButtonText: "Renun\u0163",
				giveUpButtonTitle: "Abandoneaz\u0103 lec\u0163ia interactiv\u0103",
				giveUpConfirmation:
					"E\u015Fti sigur ca vrei sa abandonezi lec\u0163ia interactiv\u0103?",
			},
		};
		...
		extendedGamebook = {
			...
			makeState: () => {
				...
				const state = {
					...
					showHint: false,
					isNavigateBack:
						gp.path?.length > analysis.path?.length &&
						gp.path.startsWith(analysis.path),
				};
				...
				if (state.init || gp.state?.init) {
					gp.resetStats();
					if (
						this.options.flow.sequential ||
						this.options.flow.spacedRepetition
					) {
						gp.currentPath = this.getCurrentPath();
						if (!gp.currentPath) {
							const nextMoves = parent
								.getNextMoves(node, gp.threeFoldRepetition)
								.filter((c) => this.isPermanentNode(c));
							if (nextMoves.length) {
								if (
									parent.global.confirm(
										trans.noarg("resetQuestionNoVariations"),
									)
								) {
									this.resetDone();
									return gp.makeState();
				...
				const isAcceptedMove =
					this.isPermanentNode(node) &&
					(!(
						this.options.flow.sequential || this.options.flow.spacedRepetition
					) ||
						this.inCurrentPath(analysis.path));
				if (!isAcceptedMove) {
					const position = parent.getNodePosition(node);
					const candidate = parent
						.getNextMoves(parNode, gp.threeFoldRepetition)
						.filter((c) => this.isPermanentNode(c))
						.filter(
							(c) =>
								!(
				...
				if (!isAcceptedMove) {
					...
					state.feedback = "end";
					this.markPathFinished(
						analysis.path,
						gp.goodMoves + (gp.isMyMove() ? 0 : 1),
						gp.badMoves,
						gp.askedForSolution,
					);
				} else if (gp.isMyMove()) {
				...
				if (!state.comment) {
					...
					if (
						!state.isNavigateBack &&
						!gp.isMyMove() &&
						func &&
						this.options.fastInteractive
					) {
						delay = 50;
						const oldFunc = func;
						func = () => {
					...
					$("div.gamebook .comment").removeClass("good bad");
				}
			},
			retry: () => {
				const parent = this.lichessTools;
			...
			next: () => {
				...
				if (!gp.isMyMove()) {
					...
					if (
						...
						child = parent.getRandomVariation(
							analysis.node,
							gp.threeFoldRepetition,
						);
					}
					if (child) {
			...
			solution: () => {
				...
				const gp = analysis.gamebookPlay();
				let children = parent
					.getNextMoves(analysis.node, gp.threeFoldRepetition)
					.filter((c) => this.isPermanentNode(c));
				if (
					this.options.flow.sequential ||
					this.options.flow.spacedRepetition
		...
		getCurrentPath = () => {
			...
			const key =
				analysis.study.data.id + "/" + analysis.study.currentChapter()?.id;
			...
			const traverse = (node, path) => {
				...
				const nextMoves = node.children.filter((c) => this.isPermanentNode(c));
			...
			lichess.storage.set(
				"LichessTools.chapterPaths",
				JSON.stringify(this._paths),
			);
			return paths.currentPath;
		};
		...
		markPathFinished = (path, goodMoves, badMoves, askedForSolution) => {
			...
			const success =
				badMoves == 0 &&
				!askedForSolution &&
				goodMoves >= Math.floor(path.length / 4);
		...
		showGiveUpButton = () => {
			...
			if (!container.length || $(".lichessTools-giveUp", container).length)
				return;
			$('<button class="lichessTools-giveUp">')
				...
				.on("click", (ev) => {
					ev.preventDefault();
					parent.global.setTimeout(() => {
						if (!parent.global.confirm(trans.noarg("giveUpConfirmation")))
							return;
		...
		showScore = (isFinal) => {
			...
			const scoreText = trans.pluralSame(
				isFinal ? "finalScore" : "currentScore",
				Math.round(100 * score),
			);
			const scoreRating =
				score > 0.9 ? 4 : score > 0.75 ? 3 : score > 0.5 ? 2 : 1;
			const el = $("<span/>")
		...
		patchGamebook = () => {
			...
			if (!gp) return;
			if (
				this.options.extendedInteractive &&
				!gp.isExtendedInteractiveLessons
			) {
				gp.makeState = this.replaceFunction(
					gp.makeState,
					this.extendedGamebook.makeState,
					"extendedInteractiveLessons",
				);
				gp.retry = this.replaceFunction(
					gp.retry,
					this.extendedGamebook.retry,
					"extendedInteractiveLessons",
				);
				gp.next = this.replaceFunction(
					gp.next,
					this.extendedGamebook.next,
					"extendedInteractiveLessons",
				);
				gp.solution = this.replaceFunction(
					gp.solution,
					this.extendedGamebook.solution,
					"extendedInteractiveLessons",
				);
				...
				gp.makeState = parent.unwrapFunction(
					gp.makeState,
					"extendedInteractiveLessons",
				);
				gp.retry = parent.unwrapFunction(
					gp.retry,
					"extendedInteractiveLessons",
				);
				gp.next = parent.unwrapFunction(gp.next, "extendedInteractiveLessons");
				gp.solution = parent.unwrapFunction(
					gp.solution,
					"extendedInteractiveLessons",
				);
				gp.isExtendedInteractiveLessons = true;
			...
			if (
				(this.options.showFinalScore || this.options.alwaysShowScore) &&
				!gp.isShowScore
			) {
				gp.fens = {};
				gp.resetStats = this.extendedGamebook.resetStats;
				...
				gp.redraw = parent.wrapFunction(gp.redraw, {
					...
					after: ($this, results, ...args) => {
						if (gp.state.feedback == "end" && this.options.showFinalScore) {
							this.showScore(true);
						} else if (this.options.alwaysShowScore) {
							this.showScore();
						}
		...
		playAgain = () => {
			...
			const analysis = parent.lichess.analysis;
			analysis.userJump(
				(this.options.returnToPreview && this._previewPath) || "",
			);
			analysis.redraw();
		};
		...
		alterUI = () => {
			...
			$.cached("body").toggleClass(
				"lichessTools-extendedInteractiveLesson",
				this.options.extendedInteractive &&
					!!analysis?.study?.data?.chapter?.gamebook,
			);
			...
			if (
				menu.length &&
				analysis?.study?.data?.chapter?.gamebook &&
				!menu.has('a[data-role="addDeviation"]').length
			) {
				const text = trans.noarg("addDeviationText");
			...
			if (!analysis.study?.practice) {
				...
				let optionsElem = gamebookElem.find(
					".lichessTools-extendedInteractiveLesson-options",
				);
				if (!optionsElem.length) {
					optionsElem = $(
						'<div class="lichessTools-extendedInteractiveLesson-options">',
					)
						.append($("<span>"))
						.append(
							$('<a target="_blank">')
								...
								.attr(
									"href",
									"https://siderite.dev/blog/lichess-tools---user-manual#extendedInteractiveLesson",
								),
						)
				...
				if (this.options.extendedInteractive)
					optionsArr.push(
						trans.noarg("extendedInteractiveLesson.extendedInteractive"),
					);
				if (this.options.flow.sequential)
					optionsArr.push(
						trans.noarg("extendedInteractiveLessonFlow.sequential"),
					);
				if (this.options.flow.spacedRepetition)
					optionsArr.push(
						trans.noarg("extendedInteractiveLessonFlow.spacedRepetition"),
					);
				if (this.options.returnToPreview)
					optionsArr.push(
						trans.noarg("extendedInteractiveLesson.returnToPreview"),
					);
				if (this.options.fastInteractive)
					optionsArr.push(
						trans.noarg("extendedInteractiveLesson.fastInteractive"),
					);
		...
		refreshNodeVersion = () => {
			...
			parent.traverse(
				analysis.tree.root,
				(n, s) => (n.version = n.version || this.currentVersion),
				true,
			);
		};
...
`.replace(/\$trans\(([^\)]+)\)/g, (m) => {
				...
				$(
					...
					if ($("#abset-extendedInteractive").is(":checked"))
						arr.push("extendedInteractive");
					if ($("#abset-showScore").is(":checked")) arr.push("showFinalScore");
					if ($("#abset-alwaysShowScore").is(":checked"))
						arr.push("alwaysShowFinalScore");
					if ($("#abset-returnToPreview").is(":checked"))
						arr.push("returnToPreview");
					if ($("#abset-fastInteractive").is(":checked"))
						arr.push("fastInteractive");
			...
			$("#abset-extendedInteractive").prop(
				"checked",
				this.options.extendedInteractive,
			);
			$("#abset-showScore").prop("checked", this.options.showFinalScore);
			$("#abset-alwaysShowScore").prop("checked", this.options.alwaysShowScore);
			$("#abset-returnToPreview").prop("checked", this.options.returnToPreview);
			$("#abset-fastInteractive").prop("checked", this.options.fastInteractive);
		};
		...
		refreshChapterProgress = () => {
			...
			$("div.study__chapters").addClass(
				"lichesstools-extendedInteractiveLessonFlow",
			);
			for (const chapter of list) {
				const container = $(
					'div.study__chapters div[data-id="' + chapter.id + '"]',
				);
				...
				if (total) {
					...
					container.attr(
						"title",
						trans.pluralSame("progressTitle", doneCount + "/" + total),
					);
				} else {
		...
		async start() {
			...
			const flow = parent.currentOptions.getValue(
				"extendedInteractiveLessonFlow",
			);
			...
			if (
				this.options.extendedInteractive &&
				!parent.isWrappedFunction(
					study.setGamebookOverride,
					"extendedInteractive",
				)
			) {
				study.setGamebookOverride = parent.wrapFunction(
					study.setGamebookOverride,
					{
						...
						before: ($this, o) => {
							...
							if (o == "analyse" && study.members.canContribute()) {
								const oldSetGamebookOverride =
									study.setGamebookOverride.__originalFunction;
				...
				study.setGamebookOverride = parent.unwrapFunction(
					study.setGamebookOverride,
					"extendedInteractive",
				);
			}
			...
			analysis.actionMenu.toggle = lichessTools.unwrapFunction(
				analysis.actionMenu.toggle,
				"extendedInteractiveLesson",
			);
			analysis.actionMenu.toggle = lichessTools.wrapFunction(
				analysis.actionMenu.toggle,
				{
			...
			if (
				this.options.extendedInteractive ||
				this.options.showFinalScore ||
				this.options.alwaysShowScore
			) {
			...
			if (analysis.study.onReload) {
				analysis.study.onReload = lichessTools.unwrapFunction(
					analysis.study.onReload,
					"extendedInteractiveLesson",
				);
			}
			...
			if (this.options.extendedInteractive) {
				...
				analysis.study.onReload = lichessTools.wrapFunction(
					analysis.study.onReload,
					{
						id: "extendedInteractiveLesson",
						after: ($this, result, ...args) => {
