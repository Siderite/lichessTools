	...
	class TvOptionsTool extends LiChessTools.Tools.ToolBase {
		preferences = [
			{
				...
				possibleValues: [
					...
					"streamerTv",
					"friendsTv",
					/*'teamTv',*/ "userTvHistory",
					"wakelock",
				],
				defaultValue:
					"link,bookmark,streamerTv,friendsTv,userTvHistory,wakelock",
				advanced: false,
			},
		];
		...
		intl = {
			...
			"ro-RO": {
				...
				"tvOptions.userTvHistory":
					"Dou\u0103 partide precedente \u00een TVul juc\u0103torilor",
				"tvOptions.wakelock": "Prevent screen lock with TV",
		...
		isGamesPage = () => {
			return /^\/games(\/|$)?/i.test(
				this.lichessTools.global.location.pathname,
			);
		};
		...
		isStreamerTvPage = () => {
			return (
				/^\/games\/?$/i.test(this.lichessTools.global.location.pathname) &&
		...
		updateTvOptionsButton = () => {
			...
			if (
				this.options.streamerTv ||
				this.options.friendsTv ||
				this.options.teamTv
			) {
				parent.lichess.pubsub.emit = parent.unwrapFunction(
					parent.lichess.pubsub.emit,
					"tvOptions",
				);
				parent.lichess.pubsub.emit = parent.wrapFunction(
					parent.lichess.pubsub.emit,
					{
						id: "tvOptions",
						before: ($this, name, info) => {
							if (name == "socket.in.finish") {
								if (
									!this.isStreamerTvPage() &&
									!this.isFriendsTvPage() &&
									!this.isTeamTvPage()
								)
									return;
								const gameId = info.id;
								$(
									'main.tv-games div.page-menu__content.now-playing a[data-live="' +
										gameId +
										'"]',
								).remove();
			...
			if (this.options.streamerTv) {
				...
				if (this.isStreamerTvPage()) {
					parent.global.document.title = parent.global.document.title?.replace(
						/^.*?\u2022/,
						trans.noarg("streamers") + " \u2022",
					);

					$("a.active:not(.lichessTools-streamers)", container).removeClass(
						"active",
					);
				}
				if (elem.length) {
					...
					$(
						...
						.insertAfter(
							$("a.lichessTools-friends", container)[0] ||
								$("a.best", container)[0],
						)
						.toggleClass("active", this.isStreamerTvPage())
						.find("strong")
						.text(trans.noarg("streamers"));
				}
			} else {
			...
			if (this.options.friendsTv && parent.getUserId()) {
				...
				if (this.isFriendsTvPage()) {
					...
					$("a.active:not(.lichessTools-friends)", container).removeClass(
						"active",
					);
				}
				if (elem.length) {
			...
			if (this.options.teamTv && parent.getUserId()) {
				...
				if (this.isTeamTvPage()) {
					...
					$("a.active:not(.lichessTools-team)", container).removeClass(
						"active",
					);
				}
				if (elem.length) {
		...
		getUsers = (el) => {
			const parent = this.lichessTools;
			const $ = parent.$;
			return $("span.mini-game__user", el)
				.get()
				.map((e) => {
					const cl = $(e).clone();
				...
				while (p < notFound.length) {
					...
					for (const data of arr.filter((i) => i.playing)) {
						try {
							...
							if (
								!$('a.mini-game[data-userId="' + data.id + '"]', container)
									.length
							) {
		...
		getTeamPlayerIds = async () => {
			...
			if (!teamId) {
				const teams = await await parent.api.team.getUserTeams(
					parent.getUserId(),
				);
				teamId = teams[0]?.id;
			}
			if (!teamId) return [];
			const teamPlayers = (await parent.api.team.getTeamPlayers(teamId))?.map(
				(u) => u.id,
			);
			return teamPlayers;
		};
		...
		updateTvOptionsPageDirect = async () => {
			...
			if (this.isStreamerTvPage()) {
				container.toggleClass(
					"lichessTools-streamerTv",
					this.options.streamerTv,
				);
				const playerIds = (await parent.api.streamer.getLiveStreamers())?.map(
					(s) => s.id,
				);
		...
		updateTvOptionsPage = this.lichessTools.debounce(
			this.updateTvOptionsPageDirect,
			1000,
		);
		...
		requestWakeLock = async () => {
			...
			try {
				if (document.visibilityState === "visible") {
					...
					this.wakelock =
						await parent.global.navigator.wakeLock.request("screen");
		...
		async start() {
			...
			const tvOptions =
				this.options.link || this.options.bookmark || this.options.userTvHistory
					? parent.getTvOptions()
					: null;
			...
			if (this.options.link || this.options.bookmark) {
				...
				if (gameId && gameId !== "synthetic" && gameId !== "broadcast") {
					if (this.options.link && !header.parent().is("a")) {
						const url =
							"/" + gameId + (tvOptions.isBlack ? "/black" : "/white");
						header.wrap(
							$("<a>")
								.attr("href", url)
								.attr("title", "LiChess Tools - " + url),
						);
					}
					if (this.options.bookmark && !header.has("a.bookmark").length) {
						const title = trans.noarg("bookmarkGame");
						$("div.setup", header).prepend(
							$(
								'<a class="bookmark lichessTools-bookmark" href="/bookmark/' +
									gameId +
									'"><i data-icon="\uE067" class="on is3"></i><i data-icon="\uE066" class="off is3"></i><span></span></a>',
							).attr("title", title),
						);
					}
				}
				...
				if (!$("div.tv-history").length) {
					...
					if (text) {
						const matches = [
							...text.matchAll(
								new RegExp(
									'\\[Site.*?\\/([^"\\/]+)"\\][\\s\\S]*?\\[(Black|White)\\s+"' +
										parent.escapeRegex(tvOptions.user) +
										'"\\]',
									"gi",
								),
							),
						...
						if (matches.length) {
							...
							const translationText = trans.plural(
								"previouslyOnTV",
								1,
								tvOptions.user,
							);
							const translationTitle = trans.noarg("fromLiChessTools");
							$(".round__underboard").append(
								$('<div class="tv-history"></div>')
