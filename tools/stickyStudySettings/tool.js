	...
	class StickyStudySettingsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"stickyStudySettings.savePosition":
					"Pozi\u0163ia/mi\u005Fcarea \u00een capitole studiu",
			},
		};
		...
		fillEditFormDirect = () => {
			...
			let settings = parent.currentOptions.getValue(
				"stickyStudySettings.settings",
			);
			if (!settings) return;
		...
		async start() {
			...
			if (!study) return;
			study.form.open = parent.unwrapFunction(
				study.form.open,
				"stickyStudySettings",
			);
			study.form.save = parent.unwrapFunction(
				study.form.save,
				"stickyStudySettings",
			);
			study.setPath = parent.unwrapFunction(
				study.setPath,
				"stickyStudySettings",
			);
			...
			if (this.options.savePosition) {
				study.setPath = parent.wrapFunction(study.setPath, {
					...
					before: ($this, path, node) => {
						...
						const data = parent.jsonParse(
							lichess.storage.get("LichessTools.studyPositions"),
							{},
						);
						data[studyId] = {
							chapterId: chapterId,
						...
						lichess.storage.set(
							"LichessTools.studyPositions",
							parent.global.JSON.stringify(data),
						);
					},
				});
				if (
					!this._pageLoaded &&
					!lichess.analysis.study.gamebookPlay &&
					!parent.isGamePlaying()
				) {
