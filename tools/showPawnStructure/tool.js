(() => {
	class ShowPawnStructureTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				"options.general": "General",
				"options.showPawnStructure":
					"Arat\u0103 structura de pioni \u00een partide",
		...
		getStructure = (board, blackOrientation) => {
			...
			if (isle?.start < 2)
				result.qSide.w = Math.min(3, isle.end) - isle.start + 1;
			isle = result.islands.b.at(0);
			if (isle?.start < 2)
				result.qSide.b = Math.min(3, isle.end) - isle.start + 1;
			isle = result.islands.w.at(-1);
			if (isle?.end > 5)
				result.kSide.w = isle.end - Math.max(4, isle.start) + 1;
			isle = result.islands.b.at(-1);
			if (isle?.end > 5)
				result.kSide.b = isle.end - Math.max(4, isle.start) + 1;
			...
			structureText +=
				result.qSide[me] > result.qSide[they]
					? "M"
					: result.qSide[me] < result.qSide[they]
						? "T"
						: "X";
			structureText +=
				result.kSide[me] > result.kSide[they]
					? "M"
					: result.kSide[me] < result.kSide[they]
						? "T"
						: "X";
		...
		getStructureName = (structure) => {
			...
			const arrItem =
				arr[0].similarity > oppArr[0].similarity ? arr[0] : oppArr[0];
		...
		addStructureAnchor = (el, structureName, structure) => {
			...
			if (
				(isLink && structureElem.is("span")) ||
				(!isLink && structureElem.is("a"))
			) {
				structureElem.remove();
			...
			if (!structureElem.length) {
				const visibleEl = el
					.filter((i, e) => !!lichessTools.inViewport(e))
					.eq(0);
			...
			if (structureName.best)
				title +=
					"\r\n" +
					structureName.best.value.name +
					" " +
					structureName.best.similarity +
					"%";
			structureElem.text(structureName.name).attr("title", title);
			if (isLink) {
		...
		miniGameStructure = async (el) => {
			...
			const elems = $(el)
				.find(
					"a[href].mini-game,div.boards>a[href],.study__multiboard a.mini-game,div.mini-game",
				)
				.get();
			if (
				$(el).is(
			...
			for (const el of elems) {
				fen =
					fen ||
					$(el).attr("data-state") ||
					parent.getPositionFromBoard(el, true);
				if (!fen) {
				...
				const structure = this.getStructure(
					board,
					$(el).attr("data-state").includes("black"),
				);
		...
		miniGameStructureDebounced = this.lichessTools.debounce(
		...
		refreshStructure = async (ply) => {
			...
			const fen =
				...
				parent.getPositionFromBoard($("main"), true);
			if (!fen) return;
			const board = parent.getBoardFromFen(fen);
			const analysisOrientation = lichess.analysis?.getOrientation();
			const isBlackOrientation =
				(analysisOrientation && analysisOrientation == "black") ||
				$(".cg-wrap").eq(0).is(".orientation-black");
		...
		refreshStructureDebounced = this.lichessTools.debounce(
		...
		async start() {
			...
			if (lichess.socket?.settings?.events?.endData) {
				lichess.socket.settings.events.endData = parent.unwrapFunction(
					lichess.socket.settings.events.endData,
					"showPawnStructure",
				);
			}
			...
			if (this.options.enabled) {
				if (lichess.socket?.settings?.events?.endData) {
					lichess.socket.settings.events.endData = parent.wrapFunction(
						lichess.socket.settings.events.endData,
						{
			...
			if (this.isGamesPage() || this.isBroadcastPage()) {
				$.cached("body").toggleClass(
					"lichessTools-structureMiniGames",
