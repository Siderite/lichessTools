		...
		preferences = [
			{
				...
				defaultValue:
					"side,app,table,tv,blog,puzzle,support,feed,tours,leader,winner,about",
				advanced: true,
			},
		];
		...
		async start() {
			...
			if (this.options.allSet || this.options.noneSet) {
				if (this.initialGrid) {
					...
					$("main")
						.find(
							".lobby__side,.lobby__timeline,.lobby__app,main .lobby__table,.lobby__tv,.lobby__blog,.lobby__puzzle,.lobby__support," +
								".lobby__feed,.lobby__tournaments-simuls,.lobby__leaderboard,.lobby__winners,.lobby__about",
						)
			...
			if (isPlay) {
				parent.global.document.title =
					$("#topnav > section:first-child span.play").text() +
					" \u2022 lichess.org";
			} else {
				const grid = this.initialGrid.replace(/[a-z]+/g, (t) => {
					const ft = t === "timeline" ? "side" : t;
					const res = this.options[ft] ? t : ".".padEnd(t.length, " ");
					return res;
				});
				$("main").css("grid-template-areas", grid);
				$("main .lobby__side,main .lobby__timeline").toggleClass(
					"lichessTools-hideElement",
					!this.options.side,
				);
				$("main .lobby__app").toggleClass(
					"lichessTools-hideElement",
					!this.options.app,
				);
				$("main .lobby__table").toggleClass(
					"lichessTools-hideElement",
					!this.options.table,
				);
				$("main .lobby__tv").toggleClass(
					"lichessTools-hideElement",
					!this.options.tv,
				);
				$("main .lobby__blog").toggleClass(
					"lichessTools-hideElement",
					!this.options.blog,
				);
				$("main .lobby__puzzle").toggleClass(
					"lichessTools-hideElement",
					!this.options.puzzle,
				);
				$("main .lobby__support").toggleClass(
					"lichessTools-hideElement",
					!this.options.support,
				);
				$("main .lobby__feed").toggleClass(
					"lichessTools-hideElement",
					!this.options.feed,
				);
				$("main .lobby__tournaments-simuls").toggleClass(
					"lichessTools-hideElement",
					!this.options.tours,
				);
				$("main .lobby__leaderboard").toggleClass(
					"lichessTools-hideElement",
					!this.options.leader,
				);
				$("main .lobby__winners").toggleClass(
					"lichessTools-hideElement",
					!this.options.winner,
				);
				$("main .lobby__about").toggleClass(
					"lichessTools-hideElement",
					!this.options.about,
				);
			}
