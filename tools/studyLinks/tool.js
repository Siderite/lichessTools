	...
	class StudyLinksTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"studyLinks.studyLinksSameWindow":
					"Open links to studies in same window",
			},
			"ro-RO": {
				...
				"studyLinks.studyLinksSameWindow":
					"Deschide linkuri la studii \u00een aceea\u015Fi fereastr\u0103",
			},
		};
		...
		getYoutubeId = (e) => {
			...
			const m =
				/(?:youtube(?:\.\w+)+|youtu\.be)(?:\/watch|\/embed|\/shorts|\/live|\/v|\/e|)(?:\b.*?[\?&]v=|\/|)(?<id>[^\?&#\r\n]+)/i.exec(
					url.toString(),
				);
			return m
				? {
...
/>`,
				...
				const dialogPlacement = parent.storage.get(
					"LichessTools.dialogPlacement",
				);
				if (dialogPlacement) {
		...
		alterStudyLinks = this.lichessTools.debounce(
			this.alterStudyLinksDirect,
			100,
		);
