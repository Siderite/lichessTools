	...
	class AnalysisContextActionsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"analysisContextActions.removeSuperfluous":
				...
				extractVariationTitle:
					"LiChess Tools - copy branch and continuations to clipboard\r\nShift/Ctrl/Alt-click for options",
				...
				setCustomEngineDepth:
					"You need to set a custom engine depth for this feature!",
				evaluateTerminationsText: "Evaluate terminating moves",
				evaluateTerminationsTitle:
					"LiChess Tools - add evaluation comment to all branch terminating moves",
			...
			"ro-RO": {
				...
				"options.analysisContextActions":
					"Op\u0163iuni \u00een plus \u00een meniul context",
				"analysisContextActions.copyPgn":
					"Copiaz\u0103 varia\u0163iunea ca PGN",
				"analysisContextActions.moveEval": "Evaluare mut\u0103ri finale",
				"analysisContextActions.showTranspos":
					"Arat\u0103 toate transpozi\u0163iile",
				"analysisContextActions.removeSuperfluous":
					"Elimin\u0103 ce e \u00een plus",
				"analysisContextActions.showOnEmpty":
					"Arat\u0103 meniul context c\u00E2nd nu sunt mut\u0103ri",
				...
				extractVariationTitle:
					"LiChess Tools - copiaz\u0103 varia\u0163iunea \u015Fi continu\u0163rile ca PGN\r\nShift/Ctrl/Alt-click pentru op\u0163iuni",
				...
				setCustomEngineDepth:
					"Trebuie s\u0103 setezi un nivel personalizat pentru motorul de analiz\u0103!",
				evaluateTerminationsText: "Evalueaz\u0103 mut\u0103rile finale",
				evaluateTerminationsTitle:
					"LiChess Tools - adaug\u0103 comentarii cu evaluarea mut\u0103rilor finale din fiecare ramur\u0103",
				evaluateTerminationsStarted:
					"Comentarea cu evalu\u0103ri pornit\u0103: %s",
		...
		extractVariationAsPGN = (ev) => {
			...
			parent.exportPgn(analysis.contextMenuPath, {
				copyToClipboard: true,
				fromPosition: ev.shiftKey,
				toPosition: ev.altKey,
				separateLines: ev.ctrlKey,
			});
		};
		...
		addEvalComment = (node, ceval) => {
			...
			const evalText =
				...
				(ceval.mate
					? "#" + ceval.mate
					: (ceval.cp > 0 ? "+" : "") + (ceval.cp / 100).toFixed(decimals));
		...
		doEvaluation = () => {
			...
			const nodes = this.state.lastMoves.filter(
				(n) =>
					n.id &&
					!n.isCommentedOrMate &&
					(!n.terminationEvaluated ||
						Date.now() - n.terminationEvaluated > 10000),
			);
			if (!this.evaluateTerminationsTotal)
				this.evaluateTerminationsTotal = nodes.length;
			const percent =
				this.evaluateTerminationsTotal -
				nodes.length +
				"/" +
				this.evaluateTerminationsTotal;
			const liveStatus = trans.pluralSame(
				"evaluateTerminationsStarted",
				percent,
			);
		...
		evaluateTerminations = async (ev) => {
			...
			const analysis = lichess.analysis;
			const study = analysis?.study;
			const customEngineDepth =
				parent.currentOptions.getValue("customEngineLevel");
		...
		analysisContextMenu = (ev) => {
			...
			if (this.options.removeSuperfluous) {
				$(
					'a[data-icon="\uE056"],a[data-icon="\uE070"],a.glyph-icon',
					menu,
				).remove();
			}
		};
		...
		ensureShowOnEmpty = () => {
			...
			if (this.options.showOnEmpty) {
				$("div.tview2").addClass("lichessTools-showOnEmpty").attr("p", "*"); //lichess checks this against empty so we must add something ...
				...
				$("div.tview2").removeClass("lichessTools-showOnEmpty").removeAttr("p");
			}
		};
		...
		checkEngineLevel = () => {
			...
			if (this.evaluateTerminationsStarted) {
				...
				if (
					node?.ceval?.depth > customEngineDepth ||
					(node?.ceval?.depth == customEngineDepth && isIdle)
				) {
		...
		async start() {
			...
			if (
				this.options.moveEval &&
				analysis.study &&
				!$("div.lichessTools-liveStatus").length
			) {
				$("main.analyse div.analyse__controls.analyse-controls").after(
					'<div class="lichessTools-liveStatus analyse__controls"><label></label></div>',
				);
			}
