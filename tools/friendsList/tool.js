	...
	class FriendsListTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				friendsMenu:
					"LiChess Tools - prieteni pe care \u00eei urm\u0103re\u015Fti",
		...
		updateFriendsButton = () => {
			...
			if (parent.global.document.hidden) {
				parent.global.requestAnimationFrame(
					parent.debounce(this.updateFriendsButton, 500),
				);
				return;
			}
			const $ = parent.$;
			...
			if (!container.length) {
				...
				container = $('<div class="lichessTools-onlineFriends"/>')
					.append(
						$('<button class="toggle link">')
							.attr("title", title)
							.on("mouseover click", () => {
								this.requestOnlines();
							})
							.append(
								$('<span class="data-count">').attr("data-icon", "\uE059"),
							),
					)
					.append(
				...
				const prev = $("div.pager.prev", container).on("click", (ev) => {
					ev.preventDefault();
					if (prev.is("disabled")) return;
					this.buttonStartIndex -= this.buttonPageSize;
					this.updateFriendsButton();
				});
				const next = $("div.pager.next", container).on("click", (ev) => {
					ev.preventDefault();
			...
			container
				...
				.toggleClass(
					"lichessTools-somePlaying",
					!!this.user_data.playing.length,
				);
			const span = $("button.toggle > span.data-count", container).attr(
				"data-count",
				this.user_data.online.length,
			);
			$("div.pager.prev", container).toggleClass("disabled", atStart);
			$("div.pager.next", container).toggleClass("disabled", atEnd);
			const notifs = $("div.notifications", container).empty();
			const displayedItems = items.slice(
				this.buttonStartIndex,
				this.buttonStartIndex + this.buttonPageSize,
			);
			for (const userId of displayedItems) {
				...
				const playingClass = timeControl
					? "lichessTools-playing-" + timeControl
					: "lichessTools-playing";
				const elem = $('<a class="user-link ulpt">')
					...
					.append(
						$('<span class="content">').text(
							this.user_data.names[userId] || userId,
						),
					)
					.appendTo(notifs);
		...
		updateFriendsMenu = () => {
			...
			if (parent.global.document.hidden) {
				parent.global.requestAnimationFrame(
					parent.debounce(this.updateFriendsMenu, 500),
				);
				return;
			}
			const $ = parent.$;
			...
			if (!$("section.lichessTools-onlineFriends", $(this.menuParent)).length) {
				...
				const title = trans.noarg("friendsMenu");
				$(this.menuParent).append(
					$('<section class="lichessTools-onlineFriends"/>')
						.append(
							$("<a/>")
			...
			$("section.lichessTools-onlineFriends > a").attr(
				"data-count",
				this.user_data.playing.length,
			);
			...
			friends.each((i, e) => {
				...
				let friendMenu = group
					.find("a")
					.filter((i, e2) => sameUser($(e2).attr("href"), href));
				if (friendMenu.is(".temp")) {
					friendMenu.remove();
					friendMenu = null;
				}
				if (!friendMenu?.length) {
					friendMenu = $(e).clone().attr("data-pt-pos", "e");
				...
				if (isPlaying) {
					...
					friendMenu
						...
						.attr("href", "/@/" + user + "/tv");
					if (timeControl)
						friendMenu.addClass("lichessTools-playing-" + timeControl);
				} else {
					friendMenu
			...
			if (this.followingOnlinesRequests > 5 || !friends.length) {
				this.user_data.online.forEach((user) => {
					...
					let friendMenu = group
						.find("a")
						.filter((i, e2) => sameUser($(e2).attr("href"), "/@/" + user));
					if (!friendMenu.length) {
		...
		updateFriendsPage = async () => {
			...
			if (parent.global.document.hidden) {
				parent.global.requestAnimationFrame(
					parent.debounce(this.updateFriendsPage, 500),
				);
				return;
			}
			if (!$(".lichessTools-liveButtons").length) {
				$("<div>")
					...
					.append(
						$('<i data-icon="&#xE012;" data-role="hideOffline">')
					...
					.append(
						$('<i data-icon="&#xE025;" data-role="hideNotPlaying">')
							.attr("title", trans.noarg("hideNotPlayingTitle"))
							.on("click", () => {
					...
					.append(
						$('<i data-icon="&#xE00F;" data-role="hideMuted">')
			...
			if (!isFavorites && !hasPages) {
				for (const user of this.user_data.online) {
					...
					row = $(
						`<tr class="paginated">
      <td><a class="user-link ulpt" href="/@/` +
							user +
							`"><i class="line"></i>` +
							user +
							`</a></td>
      ...
      <td><div class="relation-actions btn-rack"></div></td>
			...
			for (const user in this.rows) {
				...
				$("a.lichessTools-mute", row).attr(
					"title",
					isMuted ? enablePlayingAlertTitle : mutePlayingAlertTitle,
				);
				const isOnline = this.user_data.online.includes(user);
				const isPlaying = this.user_data.playing.includes(user);
				row
					.toggleClass("lichessTools-online", isOnline)
					.toggleClass("lichessTools-playing", isPlaying);
		...
		enters = (userName, data) => {
			...
			if (!this.user_data.online.includes(userId))
				this.user_data.online.push(userId);
			const isPlaying = data?.playing;
			if (isPlaying) {
				if (!this.user_data.playing.includes(userId))
					this.user_data.playing.push(userId);
			} else {
		...
		playing = (user) => {
			user = this.getUserId(user);
			if (!this.user_data.online.includes(user))
				this.user_data.online.push(user);
			if (!this.user_data.playing.includes(user))
				this.user_data.playing.push(user);
			let friend = this.friends[user];
			if (!friend) {
				friend = { userId: user };
				this.friends[user] = friend;
		...
		getFollowingOnlinesByApi = async () => {
			...
			parent.global.console.debug(
				"Sent following-onlines too many times. Giving up.",
			);
		};
		...
		requestOnlinesApi = async () => {
			...
			if (this.user_data.playing.length) {
				const arr = await parent.api.user.getUserStatus(
					this.user_data.playing,
					{ withGameMetas: true },
				);
		...
		async start() {
			...
			if (
				friendsBoxMode == "menu" ||
				friendsBoxMode == "button" ||
				(liveFriendsPage && parent.isFriendsPage())
			) {
				lichess.pubsub.on(
					"socket.in.following_onlines",
					this.following_onlines,
				);
			...
			clearInterval(this.onlinesInterval);
			if (
				this.options.friendsBoxMode ||
				(this.options.liveFriendsPage && parent.isFriendsPage()) ||
				this.options.friendsPlaying
			) {
