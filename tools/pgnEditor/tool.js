	...
	class PgnEditorTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				btnCutStuffText: "Cut",
				btnCutStuffTitle:
					"Cut to ply number, remove junk, annotations, comments, tags, found results or branches",
				...
				searchPattern:
					'Enter partial FEN or PGN string (*,? wildcards supported) or Tag(=,*=)Value or "Index"=Value or "Invalid" or "Ply"(>,=,<)Value or "Eval"(>,=,<)Value"',
				foundGames: "%s games found",
				"foundGames:one": "One game found",
				cutStuffPrompt:
					'"Tags", "Annotations", "Comments", "Result", "Ply "Value,"Junk","Eval"(>,=,<)Value in any combination (i.e. junk, tags, ply 10, eval<0)',
				sendToPgnEditorText: "PGN Editor",
				sendToPgnEditorTitle: "LiChess Tools - send to PGN Editor",
				evaluateNeedsAnalysis:
					"Evaluate can only be used on the analysis or study pages - Lichess limitation",
			},
			"ro-RO": {
				...
				btnNormalizeTitle:
					"Grupeaz\u0103 mut\u0103ri f\u0103cute din aceea\u015fi pozi\u0163ie",
				btnDenormalizeText: "Denormalizeaz\u0103",
				btnDenormalizeTitle: "Extinde transpozi\u0163ii \u00een mut\u0103ri",
				btnSplitText: "Sparge",
				btnSplitTitle:
					"Sparge \u00een mai multe PGNuri f\u0103r\u0103 varia\u0163iuni",
				btnCountText: "Num\u0103r\u0103",
				...
				btnSearchText: "Caut\u0103",
				btnSearchTitle:
					"Caut\u0103 cu FEN par\u0163ial, etichete, index, invalid, jum\u0103t\u0103\u0163i de mutare",
				...
				btnCutStuffTitle:
					"Taie la un nu\u0103ar de jum\u0103t\u0103\u0163i de mutare, elimin\u0103 gunoi, adnot\u0103ri, comentarii, etichete sau rezultatele g\u0103site",
				...
				searchPattern:
					'Introdu un text FEN sau PGN par\u0163ial (suport\u0103 \u00eenlocuitori *,?) sau Tag(=,*=)Valoare sau "Index"=Valoare sau "Invalid" sau "Ply"(>,=,<)Valoare sau "Eval"(>,=,<)Valoare"',
				foundGames: "%s jocuri g\u0103site",
				"foundGames:one": "Un joc g\u0103sit",
				cutStuffPrompt:
					'"Tags", "Annotations", "Comments", "Result", "Ply "Valoare, "Junk", "Eval"(>,=,<)Valoare \u00een orice combina\u0163ie (ex: junk, tags, ply 10, eval<0)',
				sendToPgnEditorText: "Editor PGN",
				sendToPgnEditorTitle: "LiChess Tools - trimite la Editor PGN",
				evaluateNeedsAnalysis:
					"Evaluarea poate fi folosit\u0103 doar pe paginile de analiz\u0103 sau studiu - limitare Lichess",
			},
		};
		...
		addTextToHistory = (text) => {
			...
			parent.storage.set("LichessTools.pgnEditor.history", this.history, {
				session: true,
				zip: true,
			});
			parent.storage.set(
				"LichessTools.pgnEditor.historyIndex",
				this.historyIndex,
				{ session: true },
			);
		};
		...
		copyToClipboard = async (text) => {
			...
			const permission = await parent.global.navigator.permissions.query({
		...
		loadHistory = () => {
			const parent = this.lichessTools;
			this.history =
				parent.storage.get("LichessTools.pgnEditor.history", {
					session: true,
					zip: true,
				}) || [];
			let index = parent.storage.get("LichessTools.pgnEditor.historyIndex", {
				session: true,
			});
...
`)
			...
			$("button.close-button", dialog).on("click", (ev) => {
				ev.preventDefault();
				this.stopOperations();
				dialog.remove();
		...
		countPgn = async () => {
			...
			const games = co.pgn
				.parsePgn(text)
				.filter((g) => g.headers.get("FEN") || g.moves?.children?.length);
			...
			this.writeNote(
				trans.pluralSame("gameCount", games.length).replace(/%2/g, moveCount),
			);
		};
		...
		enhanceGameWithFens = (game) => {
			...
			const traverse = (pos, node, ply = 0) => {
				...
				for (const child of node.children) {
					...
					if (!move) {
						const err = new Error(
							"Move " + child?.data?.san + " at ply " + ply + " is illegal!",
						);
		...
		mergeNodes = (n1, n2) => {
			...
			const comments = (n1.data?.comments || []).concat(
				n1.data?.comments || [],
			);
			if (comments.length) {
		...
		mergePgn = async (textarea) => {
			...
			const mergeGames = (dest, node, src) => {
				...
				if (dest.fenDict || src.fenDict) {
					if (!dest.fenDict || !src.fenDict)
						throw new Error(
							"Cannot merge games that have different enhancement (fenDict)",
						);
		...
		evaluatePosition = async (textarea) => {
			...
			for (const game of games) {
				...
				this.writeNote(
					trans.pluralSame("preparingGames", games.length - gameIndex),
				);
				await parent.timeout(0);
				if (this._cancelRequested) {
					break;
					...
					for (const node of game.lastMoves) {
						...
						const evalText =
							...
							(info.mate
								? "#" + side * info.mate
								: (side * info.cp > 0 ? "+" : "") +
									((side * info.cp) / 100).toFixed(decimals));
		...
		splitPgn = async (textarea) => {
			...
			while (games.length && !this._cancelRequested) {
				...
				games.splice(0, 1);
				this.writeNote(
					trans.pluralSame("splittingGames", newGames.length + games.length),
				);
				await parent.timeout(0);
			}
			games = newGames;
		...
		cutTags = async (textarea) => {
			...
			for (const game of games) {
				const keys = game.headers
					.keys()
					.filter((k) => !["FEN", "SetUp"].includes(k));
			...
			const traverse = (node, lastBranch = null, san = null) => {
				...
				if (!node.children?.length) {
					...
					const match = comments
						.map((c) =>
							/(?:\beval:|%eval) (?:#(?<mate>[\-\+]?\d+)|(?<eval>[\-\+]?\d+(?:\.\d+)?))/.exec(
								c,
							),
						)
						.find((m) => !!m);
		...
		searchPgn = async (textarea) => {
			...
			if (m) {
				...
				m =
					/^eval\s*(?<operator>[\<\>=])\s*(?<value>[\-\+]?\d+(?:\.\d+)?)/i.exec(
						search,
					);
				if (m) {
					searchMode = "eval";
					...
					if (/^invalid[s]?$/i.test(search)) {
						...
						m =
							/^\s*(?<tag>\w+)\s*(?<operator>=|\*=)\s*["]?(?<value>.*?)["]?$/.exec(
								search,
							);
						if (m) {
							searchMode = "tag";
				...
				try {
					...
					switch (searchMode) {
						...
						case "fenOrMoves":
							...
							this.enhanceGameWithFens(game);
							this.enhanceGameWithFenDict(game);
							found = Array.from(game.fenDict).find((pair) =>
								reg.test(pair[0]),
							);
							break;
						case "tag":
							const val =
								tagName.toLowerCase() == "index"
									? gameIndex.toString()
									: game.headers
											.entries()
											.find(
												(p) => p[0]?.toLowerCase() == tagName?.toLowerCase(),
											)[1];
							switch (tagOperator) {
								case "=":
									found =
										val?.replace(/\s+/g, "") == tagValue?.replace(/\s+/g, "");
									break;
								case "*=":
									found = val
										?.replace(/\s+/g, "")
										?.includes(tagValue?.replace(/\s+/g, ""));
									break;
							}
							break;
						case "plyNumber":
		...
		writeGames = (textarea, games) => {
			...
			games.forEach((game) => {
				if (
					games.length > 1 &&
					game.moves?.children?.length &&
					!game.headers.entries().find((e) => !/^[\?\.\*\s]*$/.test(e[1]))
				) {
			...
			const regChessMove =
				/(?<move>\b(?<piece>[NBRQK])?(?<p1>([a-h])?([1-8])?(x)?([a-h][1-8]))(=(?<promotion>[NBRQK]))?(?<p2>\+|#)?\b)(?<nags>(\s+\$\d+)+)/g;
			const annos = ["!", "?", "!!", "??", "!?", "?!"];
			newText = newText.replace(regChessMove, (...m) => {
				const g = m.at(-1);
				let nags = g.nags;
				const m2 = /\s+\$([1-6])\b/.exec(nags);
				if (m2) {
					nags =
						nags.substr(0, m2.index) + nags.substr(m2.index + m2[0].length);
		...
		clear = async (textarea) => {
			...
			parent.storage.remove("LichessTools.pgnEditor.history", {
				session: true,
			});
			parent.storage.remove("LichessTools.pgnEditor.historyIndex", {
				session: true,
			});
		};
		...
		async start() {
			...
			const trans = parent.translator;
			const container = $(
				'#topnav section a[href="/analysis"]+div[role="group"]',
			);
			...
			if (lichess.analysis) {
				...
				lichess.analysis.actionMenu.toggle = lichessTools.unwrapFunction(
					lichess.analysis.actionMenu.toggle,
					"pgnEditor",
				);
				lichess.analysis.actionMenu.toggle = lichessTools.wrapFunction(
					lichess.analysis.actionMenu.toggle,
					{
						id: "pgnEditor",
