	...
	class SaveOptionsForIncognitoTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"options.saveOptionsForIncognito":
					"Also save options for browser private/incognito mode",
			},
			"ro-RO": {
				"options.general": "General",
				"options.saveOptionsForIncognito":
					"Salveaz\u0103 op\u0163iunile \u015Fi pentru modul privat/incognito al browserului",
			},
		};
		...
		getStorageGameId = async (myName) => {
			...
			let text = "";
			try {
				text = await parent.net.fetch({
					url: "/api/games/user/{myName}?max=1&moves=false&sort=dateAsc",
					args: { myName: myName },
				});
			} catch (e) {
