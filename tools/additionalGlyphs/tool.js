(() => {
	class AdditionalGlyphsTool extends LiChessTools.Tools.ToolBase {
		...
		drawGlyphsDirect = () => {
			...
			if (!glyph) return;
			if (
				this.isStandardGlyph(glyph) ||
				lichess.storage.get("analyse.show-move-annotation") === "false"
			) {
				const shapes =
					analysis.chessground.state.drawable.autoShapes?.filter(
						(s) => s.type !== "glyph",
					) || [];
		...
		async start() {
			...
			this.interval = parent.global.setInterval(() => {
				const autoShapes = parent.global.JSON.stringify(
					analysis.chessground?.state.drawable.autoShapes,
				);
