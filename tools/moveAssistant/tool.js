	...
	class MoveAssistantTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				assistantButtonTitle:
					"LiChess Tools - evaluarea mut\u0103rilor piesei selectate",
			},
		};
		...
		evaluate = async () => {
			...
			const dests = selected
				...
				: null;
			const isInteractiveOrPractice = !!(
				analysis.study?.gamebookPlay ||
				analysis.practice?.running() ||
				analysis.study?.practice
			);
			const isActive = !!(
			...
			$("main.analyse div.cg-wrap").toggleClass(
				"lichessTools-moveAssistant",
				isActive,
			);
		...
		refreshSquares = () => {
			...
			$("square.move-dest").each((i, e) => {
				...
				const deltaColor = parent.getGradientColor(delta, [
					{ q: 0, color: "#00FF00" },
					{ q: 100, color: "#FFFF00" },
					{ q: 200, color: "#FF8000" },
					{ q: 300, color: "#FF0000" },
				]);
				const percColor = parent.getGradientColor(qPerc, [
					{ q: 0, color: "#FF0000" },
					{ q: 0.5, color: "#FFFF00" },
					{ q: 1, color: "#00FF00" },
				]);
				const gradientColor = parent.getGradientColor(0.66, [
					{ q: 0, color: percColor },
					{ q: 1, color: "#20202040" },
				]);
				$(e)
					.css(
						"background",
						"radial-gradient(" + gradientColor + " 19%, rgba(0, 0, 0, 0) 20%)",
					)
				...
				if (wdl) {
					...
					if (!bar.length) {
						bar = $('<div class="lichessTools-wdl">').appendTo(e);
					}
					bar
						.css(
							"--deg",
		...
		getInfo = (info) => {
			...
			const cp = mate ? Math.sign(mate) * 10000 - mate : +info.cp?.at(0);
		...
		clearSquares = () => {
			...
			$("square.move-dest").css("background", "").css("border-color", "");
		...
		get isEnabled() {
			...
			const lichess = parent.lichess;
			this._isEnabled =
				lichess.storage.get("LichessTools.moveAssistant") == "true";
