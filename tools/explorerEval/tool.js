	...
	class ExplorerEvalTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"options.explorerEval":
					"Arat\u0103 evaluarea mut\u0103rilor \u00een Explorator",
				...
				fromCevalTitle:
					"LiChess Tools - din evaluare computer, ad\u00e2ncime %s",
		...
		showEvaluations(result) {
						...
						.append(
							$("<tbody>")
								...
								.on("mouseover", (ev) => {
									...
									analysis.explorer.setHovering(
										$(ev.currentTarget).attr("data-fen"),
										uci,
									);
								})
								.on("mouseout", (ev) => {
			...
			if (this.options.evalRows && moves?.length) {
				...
				const newRows = moves.filter(
					(m) => !$('tr[data-uci="' + m.uci + '"]', container).length,
				);
				...
				for (const newRow of newRows) {
					...
					if ($("td", container).filter((i, e) => $(e).text() == san).length)
						continue; //castling can be identified by multiple ucis (i.e. e1g1, e1h1)
					const newTr = $("<tr>")
						.attr("data-uci", uci)
						.append($("<td>").text(san))
			...
			$("tr[data-uci],tr.sum", container).each((i, e) => {
				...
				let explorerItem = (analysis.explorer.current()?.moves || []).find(
					(i) => i.uci == uci,
				);
				if (!explorerItem) {
					if (this.options.evalRows && moves?.length) {
				...
				const total =
					explorerItem.white + explorerItem.draws + explorerItem.black;
				...
				const sharpness = Math.round(
					(((((Math.min(explorerItem.white, explorerItem.black) * q) / 50) *
						333) /
						(explorerItem.draws * q + 1)) *
						1) /
						(1 +
							Math.exp(
								(-(explorerItem.white + explorerItem.black) * q) / 1000,
							)),
				);
				if (sharpness && Number.isFinite(sharpness)) {
					...
					const tdTitle =
						tdBar.attr("title")?.split(" / ")?.at(0) + " / " + sharpnessTitle;
				...
				if (move) {
					text = move.mate
						? "M" + move.mate
						: (move.cp / 100).toFixed(decimals);
					rank = move.rank;
					switch (rank) {
						case null:
							title = trans.pluralSame("fromCevalTitle", move.depth);
							break;
						case 0:
						case 1:
						case 2:
						case 3:
							...
							break;
						case 5:
							title = trans.pluralSame("fromLichessTitle", move.depth);
							break;
					}
					...
					if (total >= 100) {
						const moveCp = move.mate
							? Math.sign(move.mate) * (10000 - Math.abs(move.mate) * 100)
							: move.cp;
						const sim = Math.round(
							(Math.abs(moveCp - cp) / (Math.abs(moveCp) + Math.abs(cp))) * 100,
						);
						if (sim >= 20) {
							explorerItem.diff = Math.abs(moveCp - cp);
							explorerItem.signVal =
								orientation * (Math.abs(moveCp) > Math.abs(cp) ? moveCp : cp);
						}
					}
				...
				$("td.lichessTools-evalRow", e).text(title);
				$(e)
				...
				if (explorerItem.diff > 200) {
					$(e)
						.addClass(
							explorerItem.signVal < 0
								? "lichessTools-warning-red"
								: "lichessTools-warning-green",
						)
		...
		cache404 = {};
		setCached404 = (path) =>
			path ? this._setCached404(path, this.cache404) : false;
		getCached404 = (path) =>
			path ? this._getCached404(path, this.cache404) : false;
		...
		doEvaluation = async () => {
			...
			let newMoves = [];
			if (
				(this.options.db || this.options.lichess) &&
				!parent.net.slowMode &&
				result === undefined &&
				(!this.options.ceval || !analysis.ceval.enabled())
			) {
				result = { moves: [] };
				if (this.options.db && !newMoves?.length) {
					...
					newMoves = obj?.moves?.map((m) => {
						return {
							...
							mate: m.winrate
								? null
								: whosMove * Math.sign(m.score) * (30000 - Math.abs(m.score)),
							rank: m.rank,
						};
					});
				}
			...
			result = result || { moves: [] };
			const ceval =
				analysis.ceval?.curEval ||
				analysis.ceval?.lastStarted?.steps?.at(-1)?.ceval;
			const pvs =
				this.options.ceval && ceval?.fen == analysis.node.fen
					? ceval?.pvs
					: null;
			if (pvs?.length) {
				pvs.forEach((p) => {
		...
		rebind = () => {
			...
			if (!this.options.isSet) {
				explorer.setNode = parent.unwrapFunction(
					explorer.setNode,
					"explorerEval",
				);
			} else {
			...
			if (!this.options.ceval) {
				analysis.onNewCeval = parent.unwrapFunction(
					analysis.onNewCeval,
					"explorerEval",
				);
			} else {
		...
		async start() {
			...
			this.options = {
				...
				db:
					parent.isOptionSet(value, "db") ||
					parent.isOptionSet(value, "chessdb"),
