	...
	class ReadGameCommandTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"readGameCommand.helpText":
					"/readgame [speed] [voice] [instrument]\r\n default speed = " +
					this.defaultSpeed +
					", voice=0, instrument=0\r\n Esc to stop\r\nRead game moves from here",
			},
			"ro-RO": {
				"options.readGameCommand":
					"Comand\u0103: arat\u0103 valoare capcan\u0103 pentru pozi\u0163ie",
				"readGameCommand.helpText":
					"/readgame [vitez\u0103] [voce] [instrument]\r\n viteza standard = " +
					this.defaultSpeed +
		...
		getReadText = (node) => {
			...
			if (
				glyphText &&
				!commentText.toLowerCase().includes(glyphText.toLowerCase())
			) {
				text += ", " + glyphText;
			}
			if (commentText) {
			...
			text = text.replace(/[\r\n\.]+/g, ",");
			const r =
				/(?:\b(?<piece>[NBRQK])?(?<start>[a-h])?(?:[1-8])?(?<takes>x)?(?<end>[a-h][1-8])(?:=(?<promotion>[NBRQK]))?|\b(?<castles>^O-O(?<long>-O)?))(?:(?<check>\+)|(?<mate>#))?/g;
		...
		readGame = async (speed, voiceIndex, instrument) => {
			...
			while (this.reading && node) {
				...
				if (instrument && evl) {
					...
					lichessTools.play(
						"instrument/" + instrument + "/c" + sndIndex + ".mp3",
						0.05,
					);
				}
				await parent.speak(text, {
					rate: speed ? speed / 100 : 0,
			...
			if (value) {
				...
				parent.registerCommand &&
					parent.registerCommand("readGameCommand", {
						handle: (val) => {
							const m =
								/^\s*readgame(?:\s+(?<speed>\d+))?(?:\s+(?<voice>\d+))?(?:\s+(?<instrument>\d+))?/.exec(
									val,
								);
							if (!m) return;
							const speed = +m.groups.speed || this.defaultSpeed;
							const voice =
								m.groups.voice === undefined ? undefined : +m.groups.voice || 0;
							const instrument = +m.groups.instrument || 0;
							parent.global.setTimeout(
								() => this.readGame(speed, voice, instrument),
								100,
							);
							return true;
						},
