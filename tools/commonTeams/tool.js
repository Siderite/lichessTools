	...
	class CommonTeamsTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			...
			"ro-RO": {
				...
				"commonTeamsTitlePrefix:one":
					"LiChess Tools - o echip\u0103 comun\u0103",
		...
		refreshTeams = async () => {
			...
			const teamsArr = [];
			const promises = [];
			$(".game__meta__players a.user-link").each((i, e) => {
			...
			for (const teams of teamsArr) {
				commonTeams =
					commonTeams?.filter((t) => teams.find((tt) => tt.id == t.id)) ||
					teams;
			}
			if (!commonTeams.length) return;
			const teamId = commonTeams[0].id;
			const prefix = trans.plural(
				"commonTeamsTitlePrefix",
				commonTeams.length,
				commonTeams.length,
			);
			const isLichessToolsTeam = !!commonTeams.find(
				(t) => t.id == "l1chess-tools-users-team",
			);
			const linkElement = $(
