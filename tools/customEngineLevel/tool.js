	...
	class CustomEngineLevelTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				"customEngineOptions.noCloudExternal":
			...
			"ro-RO": {
				...
				"options.customEngineLevel":
					"Nivel personalizat pentru motorul de analiz\u0103",
				"options.customEngineOptions":
					"Op\u0163iuni motor analiz\u0103 personalizat",
				"customEngineOptions.noCloud": "Ignor\u0103 cloud pentru evalu\u0103ri",
				"customEngineOptions.noCloudExternal":
					"Ignor\u0103 cloud pentru motoare externe",
				"customEngineOptions.practice": "Aplic\u0103 \u00een mod Practic\u0103",
				"customEngineOptions.fix503":
					"Repar\u0103 erori 503 la motoare analiz\u0103 externe",
				applyInPractice:
					"Set\u0103ri motor personalizat \u00een mod Practic\u0103",
			},
		};
		...
		analysisControls = () => {
			...
			if (!analysis.practice) {
				const customDepth =
					analysis.ceval?.isInfinite ||
					(analysis.ceval?.isDeeper() && !analysis.node.autoDeeper)
						? 99
						: this.options.depth;
				if (
					customDepth &&
					analysis.ceval.enabled() &&
					!analysis.ceval.showingCloud
				) {
					const elem = $("div.ceval div.engine span.info")[0];
					if (elem) {
						// lichess keeps a reference to the actual node
						const textNode = Array.from(elem.childNodes).find(
							(n) => n.nodeType == 3,
						);
						if (textNode) {
							const infoText = textNode.textContent;
							const newText = infoText.replace(
								/(\d+)(?:\/\d+)?/,
								"$1/" + customDepth,
							);
							if (infoText != newText) {
			...
			const isExternal = /external/i.test(
				analysis.ceval?.engines?.active?.tech,
			);

			$(".tview2").toggleClass(
				"lichessTools-noCloud",
				this.options.noCloud || (isExternal && this.options.noCloudExternal),
			);
      ...
      <label for="abset-noCloud">$trans(customEngineOptions.noCloud)</label>
			...
			$("#abset-noCloud").prop("checked", this.options.noCloud);
      ...
      <label for="abset-noCloudExternal">$trans(customEngineOptions.noCloudExternal)</label>
				...
				$("#abset-noCloudExternal").on("change", async () => {
					const options = parent.currentOptions;
					options.customEngineOptions = [
			...
			$("#abset-noCloudExternal").prop("checked", this.options.noCloudExternal);
      ...
      <label for="abset-practice">$trans(applyInPractice)</label>
				...
				$("#abset-practice").on("change", async () => {
					const options = parent.currentOptions;
					options.customEngineOptions = [
						this.options.noCloud ? "noCloud" : "",
						this.options.noCloudExternal ? "noCloudExternal" : "",
			...
			$("#abset-practice").prop("checked", this.options.practice);
		};
		...
		determineCevalState = () => {
			...
			if (
				(analysis.practice?.running() || analysis.study?.practice) &&
			...
			if (analysis.ceval.canGoDeeper && isIdle) {
				if (
					(analysis.ceval.showingCloud && noCloud) ||
					(this.options.depth &&
						curDepth < (node.autoDeeper || this.options.depth))
				) {
			...
			if (
				!analysis.ceval.showingCloud &&
				(node.autoDeeper || !analysis.ceval.isDeeper()) &&
				this.options.depth &&
				curDepth >= this.options.depth
			) {
		...
		wrapEval = () => {
			...
			if (
				this.options.depth ||
				this.options.noCloud ||
				this.options.noCloudExternal
			) {
				if (
					!parent.isWrappedFunction(
						analysis.evalCache.onLocalCeval,
						"customEngineOptions",
					)
				) {
					analysis.evalCache.onLocalCeval = parent.wrapFunction(
						analysis.evalCache.onLocalCeval,
						{
				...
				analysis.evalCache.onLocalCeval = parent.unwrapFunction(
					analysis.evalCache.onLocalCeval,
					"customEngineOptions",
				);
			}
			...
			if (this.options.noCloud || this.options.noCloudExternal) {
				if (
					!parent.isWrappedFunction(
						analysis.evalCache.fetch,
						"customEngineOptions",
					)
				) {
					analysis.evalCache.fetch = parent.wrapFunction(
						analysis.evalCache.fetch,
						{
							id: "customEngineOptions",
							before: ($this, ...args) => {
								...
								const noCloud =
									this.options.noCloud ||
									(isExternal && this.options.noCloudExternal);
								if (!noCloud) return;
								if (
									(analysis.practice?.running() || analysis.study?.practice) &&
									!this.options.practice
				...
				if (
					!parent.isWrappedFunction(
						analysis.explorer.fetchTablebaseHit,
						"customEngineOptions",
					)
				) {
					analysis.explorer.fetchTablebaseHit = parent.wrapFunction(
						analysis.explorer.fetchTablebaseHit,
						{
							id: "customEngineOptions",
							before: ($this, ...args) => {
				...
				analysis.evalCache.fetch = parent.unwrapFunction(
					analysis.evalCache.fetch,
					"customEngineOptions",
				);
				analysis.explorer.fetchTablebaseHit = parent.unwrapFunction(
					analysis.explorer.fetchTablebaseHit,
					"customEngineOptions",
				);
			}
			...
			if (this.options.fix503) {
				if (
					!parent.isWrappedFunction(
						analysis.ceval.engineFailed,
						"customEngineOptions",
					)
				) {
					analysis.ceval.engineFailed = parent.wrapFunction(
						analysis.ceval.engineFailed,
						{
							id: "customEngineOptions",
							before: ($this, errorMessage) => {
								...
								if (errorMessage?.includes("Status 503")) {
									if (
										!this.lastGoDeeper ||
										Date.now() - this.lastGoDeeper > 5000
									) {
										if (analysis.ceval.enabled()) {
											analysis.ceval.goDeeper();
											parent.global.console.log(
												"503 error. Trying to restart engine",
											);
											this.lastGoDeeper = Date.now();
										}
									} else {
										parent.global.console.log(
											"503 error. Waiting 5s before retrying.",
										);
									}
									return false;
								}
				...
				analysis.ceval.engineFailed = parent.unwrapFunction(
					analysis.ceval.engineFailed,
					"customEngineOptions",
				);
			}
		...
		async start() {
			...
			const customEngineOptions = parent.currentOptions.getValue(
				"customEngineOptions",
			);
			...
			this.options = {
				...
				noCloudExternal: parent.isOptionSet(
					customEngineOptions,
					"noCloudExternal",
				),
			...
			this.wrapEval();
			analysis.instanciateEvalCache = parent.wrapFunction(
				analysis.instanciateEvalCache,
				{
					id: "customEngineOptions",
					after: ($this, result, ...args) => {
						this.wrapEval();
