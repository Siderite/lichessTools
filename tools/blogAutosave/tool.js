	...
	class BlogAutosaveTool extends LiChessTools.Tools.ToolBase {
		...
		saveBlog = async (forced) => {
			...
			const arr = form
				...
				.filter((a) => !!a.name);
			const bodyContent = arr
				.map((a) => a.name + "=" + parent.global.encodeURIComponent(a.value))
				.join("&");
			...
			try {
				...
				parent.global.setTimeout(
					() => $("body").removeClass("lichessTools-blogAutosave"),
					2000,
				);
			}
		...
		async start() {
			...
			const isBlogEdit = /^\/ublog\/(?<blogId>[^\/]+)\/edit/.exec(
			...
			if (this.options.savebutton) {
				$('<button class="button lichessTools-blogAutosave">')
					...
					.insertBefore(
						"form.ublog-post-form__main div.form-actions button[type=submit]",
					);
			}
			if (value) {
