	...
	class PreferencesTool extends LiChessTools.Tools.ToolBase {
		...
		intl = {
			"en-US": {
				...
				resetButtonWarning:
					"Are you sure? This will restore all settings to defaults. I recommend a backup first.",
				minimalButtonText: "All off",
				minimalButtonTitle:
					"LiChess Tools - turn all features individually off",
				minimalButtonWarning:
					"Are you sure? You will have to reenable each feature one by one. I recommend a backup first.",
				...
				defaultValueLegend:
					"*Blue bordered preferences are the ones enabled by default",
			},
			"ro-RO": {
				yes: "Da",
				no: "Nu",
				...
				advancedPreferences:
					"Preferin\u0163e avansate (arat\u0103 mult mai multe!)",
				...
				resetButtonWarning:
					"Sigur? Toate op\u0163iunile vor fi resetate pe valori standard. Recomand un backup \u00eenainte.",
				minimalButtonText: "Toate oprite",
				minimalButtonTitle:
					"LiChess Tools - opre\u015fte toate op\u0163iunile individual",
				minimalButtonWarning:
					"Sigur? Va trebui sa porne\u015fti fiecare op\u0163iune una c\u00e2te una. Recomand un backup \u00eenainte.",
				backupButtonText: "Backup",
				backupButtonTitle:
					"LiChess Tools - Descarc\u0103 preferin\u0163ele \u00eentr-un fi\u015Fier",
				restoreButtonText: "Restaurare",
				restoreButtonTitle:
					"LiChess Tools - \u00CEncarc\u0103 preferin\u0163ele dintr-un fi\u015Fier",
				defaultValueLegend:
					"*Preferin\u0163ele cu margine alb\u0103strie sunt cele implicite",
			},
		};
		...
		openPreferences = () => {
			...
			const checkGlobalSwitch = () => {
				$.cached("body").toggleClass(
					"lichessTools-globalDisable",
					!parent.currentOptions.enableLichessTools,
				);
			};
			const checkAdvanced = () => {
				this.options.advanced = !!parent.currentOptions.getValue(
					"advancedPreferences",
				);
				$.cached("body").toggleClass(
					"lichessTools-advancedPreferences",
...
<div class="feedback">
  ...
  <button data-icon="&#xE03A;" title="$trans(feedbackButtonTitle)"></button>
			...
			for (const key in categs) {
				...
				html +=
					'<div><h3><label for="chk_' +
					key +
					'">$trans(options.' +
					key +
					')</label></h3><input type="checkbox" id="chk_' +
					key +
					'" class="categoryToggle">';
				for (const pref of categ) {
					const defaultValue =
						!isLoggedIn && pref.defaultNotLoggedInValue !== undefined
							? pref.defaultNotLoggedInValue
							: pref.defaultValue;
					...
					if (pref.author) {
						html +=
							'<span class="lichessTools-author">$trans(author,' +
							htmlEncode(pref.author) +
							")</span>";
					}
					html += "</h2>";
					switch (pref.type) {
						case "single":
							{
								...
								for (const val of pref.possibleValues) {
									...
									html +=
										`<div` +
										(parent.isOptionSet(defaultValue, val)
											? ' class="defaultValue"'
											: "") +
										`>
                ...
                <div>
                  <input class="form-control" type="text" name="${pref.name}"/>
						...
						default:
							throw new Error(
								"Preference type " + pref.type + " not supported",
							);
					}
					html += `</section>`;
				}
...
<button id="btnRestore" type="button" class="btn button" title="$trans(restoreButtonTitle)">$trans(restoreButtonText)</button>
			...
			html = html.replace(
				...
				function (m, name, value) {
					return htmlEncode(
						value ? trans.pluralSame(name, value) : trans.noarg(name),
					);
				},
			);
			...
			saved = saved.length
				? saved.clone()
				: $('<p class="saved text none" data-icon="\ue038"></p>').text(
						trans.noarg("preferencesSaved"),
					);
			container.empty().append(html).addClass("lichessTools-preferences");
			...
			$("form input:not(.categoryToggle)", container)
				.each((i, e) => {
					...
					const preferences = this.lichessTools.tools
						.find((t) => t.preferences?.find((p) => p.name == optionName))
						?.preferences.find((p) => p.name == optionName);
					if (currentValue !== undefined) {
						if (isCheckable) {
							const defaultValue =
								!isLoggedIn &&
								preferences?.defaultNotLoggedInValue !== undefined
									? preferences?.defaultNotLoggedInValue
									: preferences?.defaultValue;
							const checked = isOptionSet(
								currentValue,
								optionValue,
								defaultValue,
							);
				...
				.on(
					"change keyup paste",
					parent.debounce(async function () {
			...
			const feedbackTextarea = $("div.feedback textarea", container).on(
				"input keypress paste send",
				function (ev) {
			...
			$("div.feedback button", container).on("click", (ev) => {
				ev.preventDefault();
			...
			$("div.actionButtons #btnReset", container).on("click", async (ev) => {
				ev.preventDefault();
				...
				const data = parent.tools
					...
					.map((p) => {
						const defaultValue =
							!isLoggedIn && p.defaultNotLoggedInValue !== undefined
								? p.defaultNotLoggedInValue
								: p.defaultValue;
			...
			$("div.actionButtons #btnMinimal", container).on("click", async (ev) => {
				ev.preventDefault();
				if (!parent.global.confirm(trans.noarg("minimalButtonWarning"))) return;
				const options = await parent.getOptions();
				const keys = parent.tools
					.map((t) => t.preferences)
					.flat()
			...
			$("div.actionButtons #btnBackup", container).on("click", async (ev) => {
				ev.preventDefault();
				const options = await parent.getOptions();
			...
			$("div.actionButtons #btnRestore", container).on("click", async (ev) => {
				ev.preventDefault();
				$('<input type="file">')
					.on("change", async (e) => {
		...
		addInfo() {
			...
			$("div.page-menu__content section[data-pref]").each((i, e) => {
				...
				$("<a>")
					...
					.attr(
						"href",
						"https://siderite.dev/blog/lichess-tools---user-manual/#" + pref,
					)
					.attr("target", "_blank")
					.prependTo(e);
			});
		...
		addPreferencesMenu = (isOpening) => {
			...
			if (!$("#dasher_app a.lichessTools-preferences").length) {
				...
				if (!links.length) {
					links = $('<div class="links">').insertBefore("#dasher_app div.subs");
				}
		...
		async start() {
			...
			const prefElem = $("<a>")
				...
				.attr(
					"href",
					isLoggedOutTeams
						? "/team/all#lichessTools"
						: "/account/preferences/display#lichessTools",
				)
