		...
		exportPgn = async (path, options) => {
			...
			function renderComments(node) {
				...
				for (const glyph of node.glyphs || []) {
					if (glyph.id) {
						// tools like Explorer Practice don't set id
						s +=
							glyph.id >= 1 && glyph.id <= 6 ? glyph.symbol : " $" + glyph.id;
					}
				}
				...
				for (const shape of node.shapes || []) {
					...
					const code =
						shape.brush[0].toUpperCase() + shape.orig + (shape.dest || "");
			...
			const regChessMove =
				/\b(?<piece>[NBRQK])?(?<p1>([a-h])?([1-8])?(x)?([a-h][1-8]))(=(?<promotion>[NBRQK]))?(?<p2>\+|#)?\b/g;
			const unicodePiece = {
				N: "\u2658",
				B: "\u2657",
				R: "\u2656",
			...
			function translateUnicode(s) {
				...
				return s.replace(regChessMove, (...m) => {
					...
					return (
						(unicodePiece[g.piece] || "") +
						(g.p1 || "") +
						(unicodePiece[g.promotion] || "") +
						(g.p2 || "")
					);
				});
			}
					...
					for (const item of arr) {
						...
						switch (nrChildren) {
							...
							case 1:
								newArr.push({
									root: item.root,
									current: item.current.children[0],
								});
								loop = true;
								break;
							default:
			...
			try {
				...
				const startIndex = options.fromPosition
