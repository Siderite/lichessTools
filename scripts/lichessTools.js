(() => {
	class LiChessTools {
		constructor(global, cash) {
			if (!cash || !global)
				throw new Error("usage: new LiChessTools(window, cash)");
			this.$ = cash;
			this.global = global;
			this.comm.init();
		}
		...
		get debug() {
			if (this._debug === undefined) {
				...
				this._debug = +debug ? +debug : debug === "true";
			}
			return this._debug;
		}
		set debug(value) {
			...
			this.global.localStorage.setItem(
				"LiChessTools2.debug",
				this._debug.toString(),
			);
			if (this._debug) {
				this.global.console.debug(
					"%c Debug mode is reserved for developers. Might lead to undesired consequences.",
					"color: red;",
				);
			}
		}
				...
				if (executeOriginal && func) {
					if (options?.ignoreErrors) {
						(async () => {
							...
							.catch((e) =>
								parent.global.console.log("Wrapped function error:", e),
							);
					} else {
		...
		unwrapFunction(func, id) {
			...
			func.__originalFunction = this.unwrapFunction(
				func.__originalFunction,
				id,
			);
			return func;
		}
		...
		unbindKeyHandler(combo, onlyMine) {
			...
			for (const key in mousetrap.bindings) {
				...
				this.arrayRemoveAll(
					arr,
					(b) => (!onlyMine || b.lichessTools) && b.combination === combo,
				);
			}
		}
		...
		bindKeyHandler(combo, func, notMine) {
			...
			if (handler) {
				this.global.console.warn(
					"Key handler for " + combo + " already bound!",
				);
				return false;
			}
		...
		getPgnTag(text, tagName) {
			if (!text) return null;
			const m = new RegExp(
				"\\[s*" + this.escapeRegex(tagName) + '\\s+"([^"]+)"s*\\]',
			).exec(text);
			return m && m[1];
		}
		...
		isOptionSet = (optionValues, searchValue, defaultValue) => {
			...
			if (
				new RegExp(
					"," + this.escapeRegex(searchValue.toString()) + ",",
					"i",
				).test("," + optionValues + ",")
			)
				return true;
		...
		getTvOptions = () => {
			...
			const mTv =
				!inAnalysisMode &&
				/\/tv(\/([^\/]+))?/.exec(this.global.location.pathname);
		...
		inViewport = (element) => {
			...
			if (!element?.offsetParent && $(element).css("position") != "fixed")
				return 0;
		...
		computeCollapsedRegex = () => {
			...
			const collapsedRegex = collapsedPaths.length
				? new RegExp(
						"^(" +
							collapsedPaths.map((p) => this.escapeRegex(p)).join("|") +
							")",
					)
				: null;
		...
		computeCollapsedRegexDebounced = this.debounce(
		...
		getElementForPath(path) {
			...
			if (!elem?.offsetParent) {
				...
				if (this.debug && this.collapsedRegex?.test(path)) {
					if (!this.lichess.analysis.tree.pathIsMainline(path)) {
						this.global.console.debug(
							"Elem for non mainline path " +
								path +
								" exists, but is marked as collapsed!",
						);
					}
				}
			}
			if (path && !elem) {
				if (this.isTreeviewVisible(true)) {
					this.debug &&
						this.global.console.warn(
							"Could not find elem for path " + path,
		...
		getUserId = () => {
			return (
				this.lichess?.analysis?.opts.userId ||
				this.$.cached("body").attr("data-user")
			);
		};
		...
		findGlyphNode = (color, symbols) => {
			...
			const arr = [].concat.apply(
				[],
				symbols.map((s) => state.glyphs[s]).filter((a) => !!a?.length),
			);
			if (!arr.length) return;
			...
			return (
				arr.find((n) => n.ply % 2 === plyColor && n.nodeIndex > index) ||
				arr.find((n) => n.ply % 2 === plyColor)
			);
		};
		...
		getPositionFromBoard = (el, asFen) => {
			...
			const elem = $(el).is("cg-container") ? el : $("cg-container", el)[0];
			const container = $(elem);
			if (!container.length) return;
			const variantElem = container.closest(
				"div.round__app, main, a.mini-game",
			);

			const orientation = container.closest(".cg-wrap").is(".orientation-black")
				? "black"
				: "white";
			const getKey =
				orientation == "white"
			...
			$("piece", container).each((i, p) => {
				...
				const type = Array.from(p.classList).find(
					(c) => !["black", "white"].includes(c),
				);
				res.p = map[type];
				if (piece.is(".white")) {
			...
			if (!turn) {
				if (lastMove["4,7"] && (lastMove["0,7"] || lastMove["7,7"])) {
					turn = "black";
				} else if (lastMove["4,0"] && (lastMove["0,0"] || lastMove["7,0"])) {
					turn = "white";
				}
			}
		...
		speak = async (text, options) => {
			options = {
				...
				voiceIndex:
					options?.voiceIndex === undefined
						? this.speechVoiceIndex
						: options.voiceIndex,
		...
		play = async (path, volume) => {
			const sound = await this.lichess.sound.load(
				"sound",
				this.lichess.sound.url(path),
			);
		...
		isDark = () => {
			...
			return (
				...
				this.global.matchMedia("(prefers-color-scheme: dark)").matches
			);
		};
		...
		jsonParse = (funcOrText, defaultValue) => {
			...
			let json = "unknown";
			try {
				json = typeof funcOrText == "function" ? funcOrText() : funcOrText;
		...
		ndjsonParse = (funcOrText, defaultValue) => {
			...
			try {
				...
				const result = json
					.split(/\r?\n/)
					.filter((s) => s?.trim())
					.map((s) => this.global.JSON.parse(s));
				return result || defaultValue;
			} catch (ex) {
				console.warn("Error parsing JSON: ", json, ex);
				return defaultValue;
		...
		getColor = (text) => {
			const m =
				/^#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})?$/.exec(
					text,
				);
		...
		getGradientColor = (q, gradient) => {
			...
			for (const gr of gradient) {
				if (q >= prev?.q && q <= gr.q) {
					...
					const color =
						"#" +
						Math.round(c1.R + (c2.R - c1.R) * localQ)
							.toString(16)
							.padStart(2, "0") +
						Math.round(c1.G + (c2.G - c1.G) * localQ)
							.toString(16)
							.padStart(2, "0") +
						Math.round(c1.B + (c2.B - c1.B) * localQ)
							.toString(16)
							.padStart(2, "0") +
						Math.round(c1.A + (c2.A - c1.A) * localQ)
							.toString(16)
							.padStart(2, "0");
					return color;
				}
		...
		isGamePlaying() {
			return (
				this.lichess.analysis?.ongoing || this.lichess.analysis?.studyPractice
			);
      ...
      return true;*/
		...
		isTouchDevice() {
			return !this.global.matchMedia("(hover: hover) and (pointer: fine)")
				.matches;
		}
		...
		intl = {
			...
			"ro-RO": {
				serverOverload:
					"LiChess crede c\u0103 le supra\u00eenc\u0103rc\u0103m sistemul!",
			},
			"zh-TW": {
			...
			get lang() {
				let lang =
					lichessTools.global.document.documentElement.lang ||
		...
		net = {
			...
			logNetwork: function (url, size, status) {
				const now = Date.now();
				if (!this.networkLog) {
					this.networkLog = this.lichessTools.jsonParse(
						(_) =>
							this.lichessTools.global.localStorage.getItem(
								"LiChessTools2.fetch",
							),
						{ size: 0, count: 0, arr: [], minTime: now },
					);
				}
				...
				if (this.lichessTools.debug) {
					const rate = this.networkLog.size
						? Math.round(
								(8 * this.networkLog.size) / (now - this.networkLog.minTime),
							)
						: 0;
					const avgSize = this.networkLog.size
						? Math.round((8 * this.networkLog.size) / this.networkLog.count)
						: 0;
					const logSize = this.lichessTools.global.JSON.stringify(
						this.networkLog,
					).length;
					this.lichessTools.global.console.debug("Fetch log size:", logSize);
					this.lichessTools.global.console.debug(
						"  ... Bandwith logged:",
						this.networkLog.size,
						"Rate:",
						rate,
						"kbps",
						"Avg call size:",
			...
			storeLog: function () {
				...
				this.lichessTools.global.localStorage.setItem(
					"LiChessTools2.fetch",
					text,
				);
			},
				...
				try {
					...
					if (args) {
						for (const k in args) {
							url = url.replace(
								"{" + k + "}",
								this.lichessTools.global.encodeURIComponent(args[k]),
							);
						}
					}
					...
					if (status >= 400) {
						...
						if (status == 429) {
							...
							const translation =
								this.lichessTools.translator.noarg("serverOverload");
							this.lichessTools.announce(translation);
							this.slowMode = true;
							this.lichessTools.global.clearTimeout(this.slowModeTimeout);
							this.slowModeTimeout = this.lichessTools.global.setTimeout(
								() => (this.slowMode = false),
								60000,
							);
						}
					...
					this.logNetwork(
						url,
						(options?.body?.length || 0) + (text?.length || 0),
						status,
					);
					return text;
				} catch (e) {
		...
		comm = {
			...
			sendResponses: [],
			init: function () {
				this.lichessTools.global.addEventListener(
					"LichessTools.receive",
					(ev) => {
		...
		cache = {
			...
			init: function () {
				const sessionData =
					this.lichessTools.storage.get("LichessTools.GeneralCache", {
						session: true,
						zip: true,
					}) || [];
				const localData =
			...
			saveDirect: function () {
				...
				let data = [
					...this._cache.entries().filter((e) => e[1].persist == "session"),
				];
				if (data.length) {
					this.lichessTools.storage.set("LichessTools.GeneralCache", data, {
						session: true,
						zip: true,
					});
				}
				data = [
					...this._cache.entries().filter((e) => e[1].persist == "local"),
				];
				if (data.length) {
					this.lichessTools.storage.set("LichessTools.GeneralCache", data, {
						session: false,
		...
		api = {
			...
			init: function () {
				...
				parent.cache.memoizeAsyncFunction(parent.api.team, "getUserTeams", {
					persist: "session",
					interval: 10 * 86400 * 1000,
				});
				parent.cache.memoizeAsyncFunction(parent.api.team, "getTeamPlayers", {
					persist: "session",
					interval: 10 * 86400 * 1000,
				});
				parent.cache.memoizeAsyncFunction(parent.api.evaluation, "getChessDb", {
					persist: "session",
					interval: 1 * 86400 * 1000,
				});
				parent.cache.memoizeAsyncFunction(parent.api.evaluation, "getLichess", {
					persist: "session",
					interval: 1 * 86400 * 1000,
				});
			},
			blog: {
				...
				save: async function (blogId, data) {
					...
					const bodyContent = data
						.map(
							(a) => a.name + "=" + parent.global.encodeURIComponent(a.value),
						)
						.join("&");
			...
			study: {
				...
				getStudyListPage: async function (baseUrl, page) {
					const parent = this.lichessTools;
					const mm = /\/(hot|newest|updated|popular)$/.exec(
						parent.global.location.pathname,
					);
			...
			user: {
				...
				getUserStatus: async function (userIds, options) {
					...
					const query = options
						? "&" +
							Object.keys(options)
								.map(
									(k) => k + "=" + parent.global.encodeURIComponent(options[k]),
								)
								.join("&")
						: "";
				...
				getUserPerfStats: async function (userId, timeControl) {
					...
					const data = await parent.net.json({
						url: "/@/{userId}/perf/{timeControl}",
						args: { userId, timeControl },
					});
					return data;
				},
			},
			game: {
					...
					const query = options
						? "?" +
							Object.keys(options)
								...
								.join("&")
						: "";
					const pgn = await parent.net.fetch("/api/games/export/_ids" + query, {
						method: "POST",
						body: gameIds.join(","),
						cache: "default",
					});
				...
				getMini: async function (gameId, color) {
					...
					const html = await parent.net.fetch({
						url:
							"/{gameId}" + (color == "White" ? "/white" : "/black") + "/mini",
						args: { gameId },
					});
					return html;
				},
			...
			flair: {
				...
				getList: async function () {
					const parent = this.lichessTools;
					const text = await parent.net.fetch(
						parent.assetUrl("flair/list.txt"),
					);
					return text;
				},
			},
			timeline: {
				...
				get: async function (lastRead) {
					const parent = this.lichessTools;
					const timeline = await parent.net.json({
						url: "/api/timeline?nb=100&since={lastRead}",
						args: { lastRead },
					});
					return timeline;
				},
		...
		async start(lichess) {
			...
			const age = lichess.info?.date
				...
				: 0;
			this.global.console.debug(
				"%c site code age: " + Math.round(age * 10) / 10 + " days",
				age < 7 ? "background: red; color:white;" : "",
			);
		...
		fireReloadOptions = () =>
			this.lichess.storage.fire("lichessTools.reloadOptions");
		...
		getDefaultOptions() {
			...
			for (const tool of this.tools) {
				...
				for (const preference of tool.preferences) {
					const defaultValue =
						!isLoggedIn && preference.defaultNotLoggedInValue !== undefined
							? preference.defaultNotLoggedInValue
		...
		applyOptions = async (options) => {
			...
			this.$.cached("body").toggleClass(
				"lichessTools",
				options.enableLichessTools,
			);
	...
	class ToolBase {
		...
		logOption(label, value) {
			this.lichessTools.global.console.log(
				label + " %c" + (value === undefined ? "" : value),
				"color:#9980FF",
			);
		}
