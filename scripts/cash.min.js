...
(function () {
	...
	F.length = 0;
	F.splice = ja;
	"function" === typeof Symbol && (F[Symbol.iterator] = w[Symbol.iterator]);
	F.map = function (a) {
		return G(
			da.apply(
				[],
				fa.call(this, function (b, c) {
					return a.call(b, c, b);
				}),
			),
		);
	...
	function O(a, b, c, d) {
		for (var h = [], f = E(b), q = d && L(d), r = 0, k = a.length; r < k; r++)
			if (f) {
				var l = b(a[r]);
				l.length && ha.apply(h, l);
			} else
				for (l = a[r][b]; !(null == l || (d && q(-1, l))); )
					h.push(l), (l = c ? l[b] : null);
		return h;
	}
	function P(a) {
		return 1 < a.length
			? x.call(a, function (a, c, d) {
		...
		F[c] = function (d) {
			...
			if (!arguments.length)
				return K(this[0])
					...
					: A(this[0])
						? Aa(this[0], b)
						: this[0].getBoundingClientRect()[c] - Ba(this[0], !a);
			var h = parseInt(d, 10);
			return this.each(function (b, d) {
				B(d) &&
					((b = Q(d, "boxSizing")),
					(d.style[c] = xa(c, h + ("border-box" === b ? Ba(d, !a) : 0))));
			});
		};
	});
	var U = {};
	...
	var V = { focus: "focusin", blur: "focusout" },
		...
		Da = /^(mouse|pointer|contextmenu|drag|drop|click|dblclick)/i;
	function Ea(a, b, c, d, h) {
			...
			f[b] &&
				(f[b] = f[b].filter(function (f) {
					var q = f[0],
						k = f[1];
					f = f[2];
					if ((h && f.guid !== h.guid) || !Ca(q, c) || (d && d !== k))
						return !0;
	...
	F.on = function (a, b, c, d, h) {
		var f = this;
		if (!D(a)) {
			for (var q in a) this.on(q, b, c, a[q], h);
			return this;
		}
		D(b) ||
						...
						(a = function Ka(a) {
							...
							if (!a.namespace || Ca(q, a.namespace.split(".")))
								if (
									b ||
									!(
										(Ja && (a.target !== f || a.___ot === r)) ||
										(Ia && a.relatedTarget && f.contains(a.relatedTarget))
									)
								) {
									var k = f;
									if (b) {
										for (var l = a.target; !qa(l, b); ) {
											if (l === f) return;
											l = l.parentNode;
											if (!l) return;
										}
									...
									l = d.call(k, a, a.___td);
									h && Y(f, r, q, b, Ka);
									!1 === l && (a.preventDefault(), a.stopPropagation());
								}
						}),
							(a.guid = d.guid = d.guid || G.guid++),
							Ea(f, r, q, b, a);
				});
		});
		return this;
	...
	F.trigger = function (a, b) {
		if (D(a)) {
			...
			var h = W[d] || V[d] || d;
			if (!h) return this;
			var f = Da.test(h) ? "MouseEvents" : "HTMLEvents";
			a = m.createEvent(f);
			a.initEvent(h, !0, !0);
			a.namespace = c.join(".");
			a.___ot = d;
	...
	var Ga = /%20/g,
		Ha = /\r?\n/g,
		La = /file|reset|submit|button|image/i,
		Ma = /radio|checkbox/i;
					...
					void 0 !== b &&
						...
						I(b, function (b, d) {
							...
							d =
								...
								"=" +
								encodeURIComponent(d.replace(Ha, "\r\n")).replace(Ga, "+");
							a = b + d;
						})));
			});
		});
	...
	F.val = function (a) {
		return arguments.length
			? this.each(function (b, c) {
					if ((b = c.multiple && c.options) || Ma.test(c.type)) {
						var d = v(a) ? fa.call(a, String) : null === a ? [] : [String(a)];
						b
							? I(
									c.options,
									function (a, b) {
	...
	var Na = /^\s*<(\w+)[^>]*>/,
		Oa = /^<(\w+)\s*\/?>(?:<\/\1>)?$/,
		Pa = { "*": aa, tr: ba, td: ca, th: ca, thead: u, tbody: u, tfoot: u };
	function oa(a) {
		if (!D(a)) return [];
		if (Oa.test(a)) return [t(RegExp.$1)];
		var b = Na.test(a) && RegExp.$1;
		b = Pa[b] || Pa["*"];
		b.innerHTML = a;
	...
	F.offset = function () {
		...
		if (a)
			return (
				...
				{ top: a.top + n.pageYOffset, left: a.left + n.pageXOffset }
			);
	};
	...
	var Qa = /^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)>\s*$/g,
		Ra = /^$|^module$|\/(java|ecma)script/i,
		Sa = ["type", "src", "nonce", "noModule"];
	function Ta(a, b) {
		a = G(a);
		a.filter("script")
			.add(a.find("script"))
			.each(function (a, d) {
				if (Ra.test(d.type) && p.contains(d)) {
					var c = t("script");
					c.text = d.textContent.replace(Qa, "");
					I(Sa, function (a, b) {
						d[b] && (c[b] = d[b]);
					});
					...
					function (a, f) {
						I(
							...
							function (b, k) {
								...
								b = !b;
								h
									? k.insertBefore(l, d ? k.firstChild : null)
									: k.parentNode.insertBefore(l, d ? k : k.nextSibling);
								b && Ta(l, k.ownerDocument);
							},
							r,
						);
					},
	...
	F.is = function (a) {
		var b = L(a);
		return y.call(this, function (a, d) {
			return b.call(a, d, a);
		});
	};
	...
	"undefined" !== typeof exports ? (module.exports = G) : (n.cash = G);
})();
