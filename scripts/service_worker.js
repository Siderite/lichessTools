...
const getObs = async (options) => {
	...
	if (!obs.socket) {
		...
		try {
			connectionOptions = options.connectOptions
				? JSON.parse(options.connectOptions)
				: undefined;
		} catch {}
		try {
...
const handlers = {
	...
	sceneChange: async (data) => {
		...
		if (!obs) return;
		await obs.call("SetCurrentProgramScene", { sceneName }).catch((e) => {
			console.debug("Could not set scene", data);
		});
	},
	getScenes: async (data) => {
		...
		const obs = await getObs(options);
		if (!obs) return;
		const response = await obs.call("GetSceneList").catch((e) => {
			console.debug("Could not get scenes", data);
		});
